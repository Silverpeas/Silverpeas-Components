...
public class ProjectManagerInstanciator implements ComponentsInstanciatorIntf {
  ...
  public void create(Connection con, String spaceId, String componentId, String userId) throws InstanciationException {
	SilverTrace.info("projectManager","ProjectManagerInstanciator.create()","root.MSG_GEN_ENTER_METHOD","space = "+spaceId+", componentId = "+componentId+", userId ="+userId);
    
	SilverTrace.info("projectManager","ProjectManagerInstanciator.create()","root.MSG_GEN_EXIT_METHOD");   
  }

  public void delete(Connection con, String spaceId, String componentId, String userId) throws InstanciationException 
  {
	SilverTrace.info("projectManager","ProjectManagerInstanciator.delete()","root.MSG_GEN_ENTER_METHOD","space = "+spaceId+", componentId = "+componentId+", userId ="+userId);
	...
	try 
	{
		...
		deleteStatement.append("delete from ").append(PROJECTMANAGER_TASKS_TABLENAME).append(" where instanceId = ? ");
		...
		holidaysStatement.append("delete from ").append(PROJECTMANAGER_CALENDAR_TABLENAME).append(" where instanceId = ? ");
	...
	{
		throw new InstanciationException("ProjectManagerInstanciator.delete()",InstanciationException.ERROR, "root.EX_RECORD_DELETION_FAILED", e);
	}
	finally
	{
		DBUtil.close(stmt);
	...
	SilverTrace.info("projectManager","ProjectManagerInstanciator.delete()","root.MSG_GEN_EXIT_METHOD");
  }
}