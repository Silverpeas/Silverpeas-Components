 ...
 public class ProjectManagerBmEJB implements ProjectManagerBmBusinessSkeleton, SessionBean {
	...
	public void updateTask(TaskDetail task, String userId) throws RemoteException
	{
		...
		try
		{
			...
			//- soit des tâches suivantes (ie tâches qui ont comme précédence la tâche modifiée) - niveau N
			//- soit des sous tâches (sans précédence) de la tâche modifiée - niveau N-1
			...
			Date		saveEndDate 		= null;
  			...
  			for (int t=0; t<nextTasks.size(); t++)
  			{
				...
				saveEndDate = endDateLinked;
  				...
  				{
	  				//La date de fin de la tâche précédente est supérieure ou égale à la tâche liéée
				...
				// si on est dans un cas de modif de date, faire la mise à jour seulement si les dates changent		
				...
				{//c'est une tache, pas le projet
					...
					{
						//La date de fin de la tâche fille est supérieure à celle de la mère
						//cette tâche doit être décalée
	...
	private void alertResource(TaskDetail task, boolean onCreation)
	{
		...
		if (onCreation)
		{
			...
			body.append("Une nouvelle tâche nommée \"").append(task.getNom()).append("\" vient de vous être affecté.\n");
		}
		else
		{
			subject = "Modification d'une tâche";
			body.append("La tâche \"").append(task.getNom()).append("\" dont vous êtes responsable vient d'être modifiée.\n");
		}
	...
	{
	    try {
		    //la tache est une sous-tache -> on recalcule les montants de charges de la tache mère
