...
public class ProjectManagerRequestRouter extends ComponentRequestRouter
{
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("projectManager", "ProjectManagerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "User=" + projectManagerSC.getUserId() + " Function=" + function);
        ...
        try
        {
            ...
            else if (function.equals("ToProject"))
            {
            	...
            	String[] projectManagerIds = projectManagerSC.getOrganizationController().getUsersIdsByRoleNames(projectManagerSC.getComponentId(), profileNames);
			...
			else if (function.equals("ToAddTask"))
			{
				...
				/*if (currentTask == null)
				{
					currentTask = projectManagerSC.getCurrentProject();
				...
				if (projectManagerSC.isProjectDefined())
            	{
				    ...
				    request.setAttribute("DateDebProject", projectManagerSC.date2UIDate(currentProject.getDateDebut()));
            	}
            ...
            else if (function.equals("ToChooseDate"))
            {
				...
				destination = URLManager.getURL(URLManager.CMP_AGENDA)+"calendar.jsp?JSCallback="+jsFunction+"&indiceForm=0&indiceElem="+inputId;
            }
			else if (function.equals("ToUserPanel"))
			{
				try{
					...
					SilverTrace.warn("projectManager","ProjectManagerRequestRouter.getDestination()","root.EX_USERPANEL_FAILED","function = "+function, e);
				}
			}
			else if(function.equals("FromUserPanel"))
			{
				//récupération des valeurs de userPanel par userPanelPeas
				SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","FromUserPanel:");
				...
				SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","userIds:"+userIds.toString());
				if (userIds.length != 0)
				{
					SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","userIds.length():"+userIds.length);

					UserDetail[] userDetails = SelectionUsersGroups.getUserDetails(userIds);
					SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","userDetails:"+userDetails.toString());
					if (userDetails != null)
					{
						SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","userDetails[0].getId():"+userDetails[0].getId().toString());
						UserDetail userDetail = userDetails[0];
						
						request.setAttribute("ResponsableFullName", projectManagerSC.getUserFullName(userDetail));
			...
			{
				//récupération des valeurs de userPanel par userPanelPeas
				SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","FromUserSelect:");
				Selection sel = projectManagerSC.getSelection();
				...
				if (userIds.length != 0)
				{
					...
					{
						SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","userDetails["+i+"].getId():"+userDetails[i].getId().toString());
						...
						{
							SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","currentResource = "+currentResources.size());
							boolean trouve = false;
							Iterator it = currentResources.iterator();
							while(it.hasNext() && !trouve)
							{
								TaskResourceDetail res = (TaskResourceDetail) it.next();
								SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","currentResource userId:"+res.getUserId()+" / " + res.getUserName());
								...
								if (res.getUserId().equals(userDetail.getId()))
								{
									...
									trouve = true;
									SilverTrace.debug("projectManager","ProjectManagerRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","currentResource charge:"+res.getCharge());
								}
							}
						}
			...
			else if(function.equals("ToTaskComments"))
			{
				...
				request.setAttribute("UserId", projectManagerSC.getUserId());
				request.setAttribute("AbleToAddAttachments", isOrganiteurOrResponsable(projectManagerSC, projectManagerSC.getCurrentTask()));
			...
			else if (function.equals("ToGantt"))
			{
				...
				if (id == null || id.length()==0 || id.equals("-1")) {
					id = new Integer(projectManagerSC.getCurrentProject().getId()).toString();
				} else {
        ...
        SilverTrace.info("projectManager", "ProjectManagerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
    
	private void updateActionCourante(HttpServletRequest request, ProjectManagerSessionController projectManagerSC) throws RemoteException, ParseException
	{
		TaskDetail task = projectManagerSC.getCurrentTask();
	
		if ("admin".equals(projectManagerSC.getRole()))
		{
			task.setResponsableId(new Integer(request.getParameter("ResponsableId")).intValue());
			...
			task.setPreviousTaskId(new Integer(request.getParameter("PreviousId")).intValue());
    ...
    private TaskDetail request2TaskDetail(HttpServletRequest request, ProjectManagerSessionController projectManagerSC) throws ParseException
    {
    	...
    	if (responsableId != null && responsableId.length()>0 && !"null".equals(responsableId))
			task.setResponsableId(new Integer(request.getParameter("ResponsableId")).intValue());
    ...
    private Collection request2Resources(HttpServletRequest request)
    {
		...
		if (allResources != null && !allResources.equals(""))
		{
			...
			for(int i=0; i<tabResources.length; i++)
			{
		        ...
		        SilverTrace.info("projectManager", "ProjectManagerRequestRouter.request2Resources()", "root.MSG_GEN_PARAM_VALUE", "Resource=" + resource.toString());
			}
		}
		return resources;
    }
            
	private TaskDetail request2Project(HttpServletRequest request, ProjectManagerSessionController projectManagerSC) throws ParseException
	{
		TaskDetail task = new TaskDetail();
	...
	private void updateCurrentProject(HttpServletRequest request, ProjectManagerSessionController projectManagerSC) throws RemoteException, ParseException
	{
		TaskDetail project = projectManagerSC.getCurrentProject();
    ...
    private Boolean isOrganiteurOrResponsable(ProjectManagerSessionController projectManagerSC, TaskDetail task)
    {
		String role = projectManagerSC.getRole();           	
		return new Boolean("admin".equals(role) || ("responsable".equals(role) && new Integer(projectManagerSC.getUserId()).intValue()==task.getResponsableId()));
    }
}