...
public class ProjectManagerSessionController extends AbstractComponentSessionController
{
	...
	public ProjectManagerSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext)
	{
		super(mainSessionCtrl, componentContext,  "com.silverpeas.projectManager.multilang.projectManagerBundle", "com.silverpeas.projectManager.settings.projectManagerIcons");
	}
	...
	{
		List tasks = getProjectManagerBm().getAllTasks(getComponentId(), getFiltre());
	...
	public List getTasks() throws RemoteException
	{
		...
		List tasks = getProjectManagerBm().getTasksByMotherId(getComponentId(), getCurrentProject().getId(), getFiltre());
	...
	private List buildArbo(List arbo, TaskDetail task, TaskDetail actionMere, int level) throws RemoteException
	{
		SilverTrace.info("projectManager", "ProjectManagerSessionController.buildArbo()", "root.MSG_GEN_ENTER_METHOD", "arbo.size()="+arbo.size()+", actionId = "+task.getId()+", level = "+level);
		...
		{
			if (actionMere != null && getRole().equals("responsable") && actionMere.getResponsableId()==new Integer(getUserId()).intValue())
			{
				task.setDeletionAvailable(true);
				task.setUpdateAvailable(true);
			} 
			else if (getRole().equals("responsable") && task.getResponsableId()==new Integer(getUserId()).intValue())
			{
				task.setUpdateAvailable(true);
			}
		}
	  	...
	  	if (isUnfoldTask(task.getId()))
	  	{
		  	...
		  	List sousActions = getProjectManagerBm().getTasksByMotherId(getComponentId(), task.getId(), getFiltre());
		  	level++;
	...
	{
		List tasks = getProjectManagerBm().getTasksByMotherId(getComponentId(), new Integer(id).intValue(), getFiltre());
		TaskDetail task = null;
		for (int a=0; a < tasks.size(); a++)
		{
			task = (TaskDetail) tasks.get(a);
	...
	{
		List tasks = getProjectManagerBm().getTasksNotCancelledByMotherId(getComponentId(), new Integer(id).intValue(), getFiltre());
		TaskDetail task = null;
		for (int a=0; a < tasks.size(); a++)
		{
			task = (TaskDetail) tasks.get(a);
	...
	public List getPotentialPreviousTasks(boolean onCreation) throws RemoteException
	{
	...
	public Date processEndDate(String taskId, String charge, Date dateDebut) throws RemoteException
	{
	...
	public Date processEndDate(String charge, Date dateDebut, String instanceId) throws RemoteException
	{
		return getProjectManagerBm().processEndDate(Float.parseFloat(charge), instanceId , dateDebut);
	}
	
	/**
	 * Vérifie la date de début d'une tâche. Si la date de début est un jour non travaillé, 
	 * la date de début sera le prochain jour travaillé. 
	...
	private Collection getAttachments(String id)
	{
		...
		return AttachmentController.searchAttachmentByPKAndContext(foreignKey, null);
	}
	...
	{
		SilverTrace.info("projectManager", "ProjectManagerSessionController.removeTask()", "root.MSG_GEN_ENTER_METHOD", "id="+id);
		
		getProjectManagerBm().removeTask(new Integer(id).intValue(), getComponentId());
	...
	public String initUserPanel() throws RemoteException {
		...
		String hostUrl			= m_context+URLManager.getURL(getSpaceId(), getComponentId())+"FromUserPanel";
	...
	{
		TaskDetail mostDistantTask = getProjectManagerBm().getMostDistantTask(getComponentId(), getCurrentProject().getId());
	...
	public void changeDateStatus(String date, String nextStatus) throws RemoteException, ParseException
	{
		...
		HolidayDetail 	holiday = new HolidayDetail(uiDate2Date(date), getCurrentProject().getId(), getComponentId());
	...
	{
		SilverTrace.info("projectManager", "ProjectManagerSessionController.changeDayOfWeekStatus()", "root.MSG_GEN_ENTER_METHOD", "year="+year+", month="+month+", day="+day);
		...
		while (getCalendar().get(Calendar.DAY_OF_WEEK)!=new Integer(day).intValue())
		{
			getCalendar().add(Calendar.DATE, 1);
		}
		...
		SilverTrace.info("projectManager", "ProjectManagerSessionController.changeDayOfWeekStatus()", "root.MSG_GEN_PARAM_VALUE", "date="+date.toString());
		
		HolidayDetail holidayDate = new HolidayDetail(date, getCurrentProject().getId(), getComponentId());
		...
		{
			holidayDates.add(new HolidayDetail(getCalendar().getTime(), getCurrentProject().getId(), getComponentId()));
	...
	{
		getProjectManagerBm().calculateAllTasksDates(getComponentId(), getCurrentProject().getId(), getUserId());
	}
	...
	public String getRole() {
		...
		for (int i=0; i < roles.length; i++) {
			...
			if (role.equals("admin")) return role;
	...
	/*public SimpleDateFormat getUiFormatter() {
		return uiFormatter;
	}*/
	...
	public void updateOccupation(TaskDetail task)
	{
		try
		{
			...
			throw new ProjectManagerRuntimeException("ProjectManagerSessionController.updateOccupation()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
		}
	}
	...
	public int checkOccupation(String taskId, String userId, Date beginDate, Date endDate)
	{
		try
		{
			return getProjectManagerBm().getOccupationByUser(userId, beginDate, endDate, Integer.parseInt(taskId));
		}
		catch (Exception e) {
			throw new ProjectManagerRuntimeException("ProjectManagerSessionController.updateOccupation()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
		}
