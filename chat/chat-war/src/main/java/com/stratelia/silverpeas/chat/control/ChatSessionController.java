...
public class ChatSessionController extends AbstractComponentSessionController
{
    ...
    public ChatSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext, String multilangBaseName, String iconBaseName)
    {
	...
	public void InsertChatroom( int ID, String name , String comment ) throws ChatException
	{
		...
		ChatRoomDetail roomDetail = new ChatRoomDetail(getComponentId(), String.valueOf(ID), name, comment, getUserId());
	...
	private void deleteIndexChatroom( Integer ID ) {
		IndexEntryPK indexEntry = new IndexEntryPK(getComponentId(), "Chat", ID.toString() );
	...
	{
		return new ChatRoomDetail(getComponentId(), id, room.getParams().getName(), room.getParams().getSubject(), null);
	}
	...
	public String initUserPanel() throws RemoteException, jChatBox.Chat.ChatException {
		...
		sel.setNotificationMetaData(getAlertNotificationMetaData()); // set NotificationMetaData contenant les informations à notifier
	...
	/** SCO - 26/12/2002 Integration AlertUser et AlertUserPeas **/
	/*************************************************************/
    public NotificationMetaData getAlertNotificationMetaData() throws RemoteException, jChatBox.Chat.ChatException {

		String url = "/Rchat/" + this.componentName + "/Main";
		String senderName = getUserDetail().getDisplayedName();
		Chatroom chatroom = ChatroomManager.getInstance().getChatroom(Integer.parseInt(currentRoomId));
		String roomName = chatroom.getParams().getName();
		
		ResourceLocator	message		= new ResourceLocator("com.stratelia.silverpeas.chat.multilang.chatBundle", "fr");
		ResourceLocator	message_en	= new ResourceLocator("com.stratelia.silverpeas.chat.multilang.chatBundle", "en");
		...
		NotificationMetaData notification_message = new NotificationMetaData(NotificationParameters.NORMAL, subject, body);
		notification_message.addLanguage("en", subject_en, body_en);

		//German notifications
		ResourceLocator	message_de	= new ResourceLocator("com.stratelia.silverpeas.chat.multilang.chatBundle", "de");
		if (message_de != null)
		{
   ...
   /*************************************************************/
	...
	private String getNotificationBody(ResourceLocator message, String senderName, String roomName)
    {
	...
	public int getSilverObjectId(String objectId) {
		...
		{
			SilverTrace.error("chat", "ChatSessionController.getSilverObjectId()", "root.EX_CANT_GET_LANGUAGE_RESOURCE", "objectId=" + objectId, e);
		} 
