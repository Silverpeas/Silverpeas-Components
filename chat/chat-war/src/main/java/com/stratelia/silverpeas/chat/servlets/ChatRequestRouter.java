...
public class ChatRequestRouter extends ComponentRequestRouter
{
	...
	private String getFlag(String[] profiles) {
		  ...
		  for (i=0 ; i < profiles.length; i++) 
		  {
				...
				if (profiles[i].equals("publisher")) return profiles[i];
		  }
		  return flag;
	}
    ...
    {
        return new ChatSessionController(mainSessionCtrl, componentContext, "com.stratelia.silverpeas.chat.multilang.chatBundle", "com.stratelia.silverpeas.chat.settings.chatIcons");
    }
	
	public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
		SilverTrace.debug("chat", "ChatRequestRouter.getDestination()","root.MSG_GEN_ENTER_METHOD","function = "+function);
		...
		try
		{
			...
			// Redirect to the correct first page when user click on the domain's barre
			if  ( function.startsWith("Main") || function.startsWith("login.jsp") || function.startsWith("searchResult") )
			{
			...
			else if ( function.startsWith("GoAdmin") || function.startsWith("index.jsp") )
			{
			...
			else if ( function.startsWith("menu.jsp") )
			{
				...
				if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("closechatroom") ) )
				{
				...
				if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("openchatroom") ) )
				{
					int next_ID = chatroomManager.getChatroomUID()+1 ;				// next channel ID (reverse from original object...)
					chatSC.InsertChatroom( next_ID, request.getParameter("name") , request.getParameter("subject") );										// insert database entry
				}
			...
			else if ( function.startsWith("chatroom.jsp") )
			{
				...
				if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("updatechatroom") ) )
				{
					// update the setting of a chatroom
					chatSC.UpdateChatroom( ID, request.getParameter("chatroomName") , request.getParameter("subject") );
				}
				else if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("remove") ))
				{
					// removed from the banned list
					chatSC.DeleteBanned( request.getParameter("ip") , ID.intValue() );
				}
				else if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("ban") ))
				{
					// banned a guys
					chatSC.InsertBanned( request.getParameter("username") , ID.intValue() );
				}
				else if ( (request.getParameter("todo") != null ) && ( request.getParameter("todo").equals("clear") ))
				{
					// banned a guys
			...
			else if ( function.startsWith("ToUserPanel") )
			{
				...
				SilverTrace.debug("chat", "ChatRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","ToUserPanel: function = "+function);
				SilverTrace.debug("chat", "ChatRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","ToUserPanel: function = "+function+" spaceId="+chatSC.getSpaceId()+" componentId="+ chatSC.getComponentId()+" room="+chatSC.getCurrentSilverObjectId());

				destination = chatSC.initUserPanel();

				SilverTrace.debug("chat", "ChatRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","ToUserPanel: function = "+function+"=> destination="+destination);
			}
			...
			{
				request.setAttribute("silverObjectId", chatSC.getCurrentSilverObjectId());
			...
			// Special route for GIF files (function = images/***.gif or styles/***.css )
        ...
        SilverTrace.info("chat","ChatRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE","destination" + destination);
		return destination;
