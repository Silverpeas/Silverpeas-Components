  ...
  <script language="javascript">
    ...
    function clipboardCopy() {
      top.IdleFrame.location.href =
          '../..<%=gallerySC.getComponentUrl()%>copy?Object=Image&Id=<%=photo.getId()%>';
    }

    function clipboardCut() {
      top.IdleFrame.location.href =
          '../..<%=gallerySC.getComponentUrl()%>cut?Object=Image&Id=<%=photo.getId()%>';
    }
      ...
      <div class="rightContent">
        ...
        <div class="fileName">
          <c:choose>
            <c:when test="${requestScope.ViewLinkDownload or photo.downloadable}">
              <a href="<%=lien%>" target="_blank">${photo.fileName}
                <img src="${downloadIconUrl}" alt="<%=EncodeHelper.javaStringToHtmlString(resource.getString("gallery.download.photo"))%>" title="<fmt:message key='gallery.originale'/>"/></a>
        ...
        <div class="fileCharacteristic  bgDegradeGris">
          <p>
            ...
            <span class="fileCharacteristicSize"><fmt:message key="gallery.dimension"/> <b>${photo.resolutionW}
              x ${photo.resolutionH} <fmt:message key="gallery.pixels"/></b></span>
            <br class="clear"/>
          </p>
        </div>
        ...
        <div class="bgDegradeGris" id="suggestionInfoPublication">
          ...
          <c:if test="${not empty photo.lastUpdater}">
            <div class="paragraphe" id="infoModification">
              <b><fmt:message key="GML.updatedAt"/></b>${silfn:formatDate(lastUpdateDate, _language)}
              <fmt:message key="GML.by"/>
              <view:username userId="${photo.lastUpdater.id}"/>
              <div class="profilPhoto">
                <img src='<c:url value="${photo.lastUpdater.avatar}" />' alt="" class="defaultAvatar"/>
              </div>
            </div>
          </c:if>
          ...
          <c:if test="${not empty photo.creator}">
            <div class="paragraphe" id="infoCreation">
              <b><fmt:message key="GML.createdAt"/></b>${silfn:formatDate(createDate, _language)}
              <fmt:message key="GML.by"/>
              <view:username userId="${photo.creator.id}"/>
              <div class="profilPhoto">
                <img src='<c:url value="${photo.creator.avatar}" />' alt="" class="defaultAvatar"/>
              </div>
            </div>
          </c:if>
          <p id="permalinkInfo">
