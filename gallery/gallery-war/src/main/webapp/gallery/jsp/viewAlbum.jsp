...
function uploadCompleted(s)  {
	...
	<%
	  ...
	  // favoris
	  if (!isGuest) {
	    operationPane.addOperation(resource.getIcon("gallery.addFavorite"), resource.getString("gallery.addFavorite"), "javaScript:addFavorite('" +
	                URLManager.getServerURL(request) + "','" + m_context + "','" + EncodeHelper.javaStringToHtmlString(EncodeHelper.javaStringToJsString(galleryName)) +
	                "','" + EncodeHelper.javaStringToHtmlString(EncodeHelper.javaStringToJsString(albumDescription)) + "','" + albumUrl + "')");
	  }
