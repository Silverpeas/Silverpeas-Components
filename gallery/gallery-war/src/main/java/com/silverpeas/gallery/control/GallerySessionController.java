...
public class GallerySessionController extends AbstractComponentSessionController
{
	 ...
	 * @see
	 */
	public GallerySessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext)
	{
		super(mainSessionCtrl, componentContext, "com.silverpeas.gallery.multilang.galleryBundle", "com.silverpeas.gallery.settings.galleryIcons", "com.silverpeas.gallery.settings.gallerySettings");
		...
		if (StringUtil.isDefined(xmlFormName))
		{
			...
			try
			{
				PublicationTemplateManager.addDynamicPublicationTemplate(getComponentId()+":"+xmlFormShortName, xmlFormName);
			}
			catch (PublicationTemplateException e)
			{
				throw new GalleryRuntimeException("GallerySessionController.super()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
			}
		}
		...
		{
			xmlOrderFormShortName = xmlOrderFormName.substring(xmlOrderFormName.indexOf("/")+1, xmlOrderFormName.indexOf("."));
			try
			{
				PublicationTemplateManager.addDynamicPublicationTemplate(getComponentId()+":"+xmlOrderFormShortName, xmlOrderFormName);
			}
			catch (PublicationTemplateException e)
			{
				throw new GalleryRuntimeException("GallerySessionController.super()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
	...
	public Collection getDernieres()
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.getDernieres()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return photos;
	}
	...
	public int getSilverObjectId(String objectId) 
	{
		...
		try
		{
			silverObjectId = getGalleryBm().getSilverObjectId(new PhotoPK(objectId, getSpaceId(), getComponentId()));
		} 
		catch (Exception e)
		{
			SilverTrace.error("gallery", "GallerySessionController.getSilverObjectId()", "root.EX_CANT_GET_LANGUAGE_RESOURCE", "objectId=" + objectId, e);
		} 
	...
	public AlbumDetail getAlbum(String albumId)
	{
		...
		try
		{
			...
			while (it.hasNext())
			{
				...
				{
					SilverTrace.info("gallery", "GallerySessionController.getAlbum", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
				}
			}
		}
		catch (Exception e)
		{
			throw new GalleryRuntimeException("GallerySessionController.getAlbum()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return album;
	}
	...
	public Collection getNotVisible()
	{
		...
		try
		{
			...
			while (it.hasNext())
			{
				...
				{
					SilverTrace.info("gallery", "GallerySessionController.getNotVisible", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
				}
			}
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.getNotVisible()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void setPhotoPath(String photoId, String[] albums)
	{
		try 
		{
			if (albums != null)
			{
				SilverTrace.debug("gallery","GallerySessionController.addAlbumPath()","root.MSG_GEN_PARAM_VALUE","photoId = "+photoId);
				getGalleryBm().setPhotoPath(photoId, albums, getComponentId());
			}
		} 
		catch (RemoteException e) 
		{
			throw new GalleryRuntimeException("GallerySessionController.addAlbumPath()", SilverpeasRuntimeException.ERROR, "gallery.MSG_ERR_GENERAL", e);
		}
	}
	...
	public void createAlbum(AlbumDetail album)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.createAlbum()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void updateAlbum(AlbumDetail album)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.updateAlbum()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void deleteAlbum(String albumId)
	{
		...
		{
			throw new GalleryRuntimeException("GGallerySessionController.deleteAlbum()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public ResourceLocator getMetadataSettings()
	{
		if (metadataSettings == null)
			metadataSettings = new ResourceLocator("com.silverpeas.gallery.settings.metadataSettings", getLanguage());
	...
	public ResourceLocator getMetadataResources()
	{
		if (metadataResources == null)
			metadataResources = new ResourceLocator("com.silverpeas.gallery.multilang.metadataBundle", getLanguage());
	...
	public PhotoDetail getPhoto(String photoId) throws RemoteException 
	{
		...
		PhotoDetail photo = null;
			...
			{
				SilverTrace.info("gallery", "GallerySessionController.getPhoto", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
			}
						
			// Mise à jour de l'album courant
			//String albumId = photo.getAlbumId();
			Collection albumIds = getGalleryBm().getPathList(photo.getPhotoPK().getInstanceId(), photo.getId());
			SilverTrace.info("gallery", "GallerySessionController.getPhoto", "root.MSG_GEN_PARAM_VALUE", "albumIds = "+albumIds+", currentAlbumId =  " + getCurrentAlbumId());
			...
			while (it.hasNext())
			{
				...
				SilverTrace.info("gallery", "GallerySessionController.getPhoto", "root.MSG_GEN_PARAM_VALUE", "albumId = "+albumId);
				if (first)
				{
			...
			SilverTrace.info("gallery", "GallerySessionController.getPhoto", "root.MSG_GEN_PARAM_VALUE", "currentAlbumId fin = "+currentAlbumId);
	...
	public PhotoDetail getPrevious()
	{
		...
		try
		{
			...
			{
				SilverTrace.info("gallery", "GallerySessionController.getPrevious", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
			}
		...
		catch (Exception e)
		{
			...
			throw new GalleryRuntimeException("GallerySessionController.getPrevious()", SilverpeasException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return photo;
	}
	...
	public PhotoDetail getNext()
	{
		...
		try
		{
			...
			{
				SilverTrace.info("gallery", "GallerySessionController.getNext", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
			}
	...
	public String getPhotoId(int rang)
	{
		...
		{
			// traitement des exceptions
			throw new GalleryRuntimeException("GallerySessionController.getPhotoId()", SilverpeasException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return photoId;
	}
	...
	public synchronized String createPhoto(PhotoDetail photo, String albumId)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.createPhoto()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void updatePhotoHeader(PhotoDetail photo)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.updatePhoto()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void updatePhoto(PhotoDetail photo)
	{
		try
		{
			...
			{
				SilverTrace.info("gallery", "GallerySessionController.updatePhoto", "gallery.MSG_NOT_ADD_METADATA", "photoId =  " + photo.getId());
			}
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.updatePhoto()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_ADD_METADATA", e);
		}
	}
	...
	public void deletePhoto(String photoId)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.deletePhoto()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	private void removeXMLContentOfPhoto(String photoId) throws RemoteException
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.removeXMLContentOfPhoto()", SilverpeasRuntimeException.ERROR, "gallery.EX_IMPOSSIBLE_DE_SUPPRIMER_LE_CONTENU_XML", e);
		} 
	...
	public Collection getPath(NodePK nodePK)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.getPath()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return path;
	}
	...
	/*public Collection getPathPhoto(String photoId)
	{
		String albumId = getPhoto(photoId).getAlbumId();
		NodePK nodePK = getAlbum(albumId).getNodePK();
		return getPath(nodePK);
	} */
	...
	public synchronized Collection getPathList(String photoId)  
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.getPathList()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
    }
	...
	{
		SilverTrace.info("gallery", "GallerySessionController.setCurrentAlbumId", "root.MSG_GEN_ENTER_METHOD", "currentAlbumId =  " + currentAlbumId);
	...
	public String initAlertUser(String photoId) throws RemoteException 
	{
		...
		SilverTrace.debug("gallery","GallerySessionController.initAlertUser()","root.MSG_GEN_PARAM_VALUE","name = "+hostComponentName+" componentId="+ getComponentId());
		sel.setNotificationMetaData(getAlertNotificationMetaData(photoId)); // set NotificationMetaData contenant les informations à notifier
	...
	public Collection search(QueryDescription query)
	{
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.getResultSearch()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
			
	public void sendAskPhoto(String order)
	{
		// envoyer une notification au gestionnaire de la photothèque concernant la demande de photo
		...
		ResourceLocator	message		= new ResourceLocator("com.silverpeas.gallery.multilang.galleryBundle", "fr");
		ResourceLocator	message_en	= new ResourceLocator("com.silverpeas.gallery.multilang.galleryBundle", "en");
		...
		messageBody 	= messageBody.append(user).append(" ").append(message.getString("gallery.notifBodyAsk")).append("\n").append(order);
			
		//english notifications
		String subject_en	= message_en.getString("gallery.notifAskSubject");
		messageBody_en 	= messageBody_en.append(user).append(" ").append(message.getString("gallery.notifBodyAsk")).append("\n").append(order);

		NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL, subject, messageBody.toString());
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.createMessage()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	
	private synchronized NotificationMetaData getAlertNotificationMetaData(String photoId) throws RemoteException  
	{
	...
	private String getNotificationBody(PhotoDetail photoDetail, String htmlPath, ResourceLocator message, String senderName)
    {
    	StringBuffer messageText = new StringBuffer();
	    messageText.append(senderName).append(" ");
	    messageText.append(message.getString("gallery.notifInfo")).append("\n\n");
	    messageText.append(message.getString("gallery.notifName")).append(" : ").append(photoDetail.getName()).append("\n");
	    if (isDefined(photoDetail.getDescription()))
	    	messageText.append(message.getString("gallery.notifDesc")).append(" : ").append(photoDetail.getDescription()).append("\n");
	...
	public Collection getListSelected()
	{
		...
		SilverTrace.info("gallery", "GallerySessionControler.getListSelected()", "", "listSelected (taille) = (" + listSelected.size() + ") "+ listSelected.toString());
		return listSelected;
	}
	...
	public boolean isAlbumAdmin(String profile, String albumId, String userId)
	{
		...
		else
		{
			...
			return ("admin".equals(profile) || ("publisher".equals(profile) && album.getCreatorId().equals(userId)));
		}
	}
	...
	public boolean isPhotoAdmin(String profile, String photoId, String userId) throws RemoteException
	{
		...
		else
		{
			...
			return (isAdminOrPublisher(profile) || ("writer".equals(profile) && photo.getCreatorId().equals(userId)));
		}
	}
	...
	public void copyImage(String photoId) throws RemoteException
	{
		...
		SilverTrace.info("gallery", "GallerySessionController.copyImage()", "root.MSG_GEN_PARAM_VALUE","clipboard = " + getClipboard().getName() + "' count=" + getClipboard().getCount());
	...
	public void cutImage(String photoId) throws RemoteException
	{
		...
		SilverTrace.info("gallery", "GallerySessionController.cutPhoto()", "root.MSG_GEN_PARAM_VALUE","clipboard = " + getClipboard().getName() + "' count=" + getClipboard().getCount());
		getClipboard().add((ClipboardSelection) photoSelect);
	}
	...
	public void copyAlbum(String albumId) throws RemoteException
	{
		...
		SilverTrace.info("gallery", "GallerySessionController.copyAlbum()", "root.MSG_GEN_PARAM_VALUE","clipboard = " + getClipboard().getName() + "' count=" + getClipboard().getCount());
		SilverTrace.info("gallery", "GallerySessionController.copyAlbum()", "root.MSG_GEN_PARAM_VALUE","nodeSelect = " + nodeSelect.toString() + "' albumId =" + album.getId());
	...
	public void cutAlbum(String albumId) throws RemoteException
	{
		...
		SilverTrace.info("gallery", "GallerySessionController.cutAlbum()", "root.MSG_GEN_PARAM_VALUE","clipboard = " + getClipboard().getName() + "' count=" + getClipboard().getCount());
		getClipboard().add((ClipboardSelection) nodeSelect);
	}
	...
	{
		try {
			SilverTrace.info("gallery","GalleryRequestRooter.paste()", "root.MSG_GEN_PARAM_VALUE","clipboard = " + getClipboard().getName() + " count=" + getClipboard().getCount());
			...
			while (clipObjectIterator.hasNext()) {
				...
				if (clipObject != null) 
				{
					...
					{
						PhotoDetail photo = (PhotoDetail) clipObject.getTransferData(PhotoSelection.PhotoDetailFlavor);
						pastePhoto(photo, clipObject.isCutted());
						if (clipObject.isCutted())
							CallBackManager.invoke(CallBackManager.ACTION_CUTANDPASTE, Integer.parseInt(getUserId()), getComponentId(), photo.getPhotoPK());
					} 
					if (clipObject.isDataFlavorSupported(NodeSelection.NodeDetailFlavor))
					{
						AlbumDetail album = (AlbumDetail) clipObject.getTransferData(NodeSelection.NodeDetailFlavor);
						SilverTrace.info("gallery","GalleryRequestRooter.paste()", "root.MSG_GEN_PARAM_VALUE", "albumId = " + album.getId());
						pasteAlbum(album, currentAlbum, clipObject.isCutted());
						if (clipObject.isCutted())
							CallBackManager.invoke(CallBackManager.ACTION_CUTANDPASTE, Integer.parseInt(getUserId()), getComponentId(), album.getNodePK());
					}
				}
			}
		} 
		catch (Exception e) {
			throw new GalleryRuntimeException("GallerySessionController.paste()", SilverpeasRuntimeException.ERROR, "gallery.EX_PASTE_ERROR", e);
		}
		getClipboard().PasteDone();
	}
	
	private void pasteAlbum(AlbumDetail albumToPaste, AlbumDetail father, boolean isCutted) throws RemoteException
	{
		NodePK nodeToPastePK = albumToPaste.getNodePK();

		SilverTrace.info("gallery","GalleryRequestRooter.pasteAlbum()", "root.MSG_GEN_PARAM_VALUE", " ENTREE albumToPaste = " + albumToPaste.getId() + " father = " + father.getId());
		...
		if (isCutted)
		{
			...
			SilverTrace.info("gallery","GalleryRequestRooter.pasteAlbum()", "root.MSG_GEN_PARAM_VALUE", " AVANT nodeToPastePK = " + nodeToPastePK.getId() + " father = " + father.getNodePK().getId());
			getNodeBm().moveNode(nodeToPastePK, father.getNodePK());
			SilverTrace.info("gallery","GalleryRequestRooter.pasteAlbum()", "root.MSG_GEN_PARAM_VALUE", " APRES nodeToPastePK = " + nodeToPastePK.getId() + " father = " + father.getNodePK().getId());
			...
			for (int i=0; i<treeToPaste.size(); i++)
			{
				...
				if (fromNode != null)
				{
					...
					SilverTrace.info("gallery","GalleryRequestRooter.pasteAlbum()", "root.MSG_GEN_PARAM_VALUE", "fromNode = " + fromNode.toString() + " toNode = " + toNodePK.toString());
	...
	public ProfileInst getAlbumProfile(String role, String topicId)
	{
		...
		//List profiles = getAdmin().getProfilesByObject(topicId, getComponentId());
	...
	private void pasteImagesOfAlbum(NodePK fromPK, NodePK toPK, boolean isCutted, List nodePKsToPaste) throws RemoteException
	{
		...
		while (itPhotos.hasNext())
		{
			...
			SilverTrace.info("gallery","GalleryRequestRooter.pasteAlbum()", "root.MSG_GEN_PARAM_VALUE", "photo = " + photo.toString() + " toPK = " + toPK.toString());
		...
		try {
			...
			if (isCutted)
			{
				...
				else
				{
					...
					//String id = createImageIntoAlbum(fromPhotoPK, photo, nodePK.getId());
					...
					try {
						if (isDefined(xmlFormName))
						{
							...
							//paste xml content
							GenericRecordSetManager.cloneRecord(recordTemplateFrom, fromId, recordTemplate, id, null);
						}
					}
					catch (PublicationTemplateException e) {
						SilverTrace.info("gallery","GallerySessionController.pastPhoto()","gallery.DIFERENT_FORM_COMPONENT",e);
					}
			...
			else
			{
				...
				else
				{
					...
					List fatherPKs = (List) getGalleryBm().getPathList(getComponentId(), photo.getPhotoPK().getId());
				...
				getPdcBm().copyPositions(fromSilverObjectId, fromPhotoPK.getInstanceId(), toSilverObjectId, getComponentId());
	...
	public synchronized void moveImage(PhotoPK fromPhotoPK, PhotoDetail photoDetail, String fatherId) throws RemoteException {
		...
		SilverTrace.debug("gallery","GallerySessionController.moveImage()","root.MSG_GEN_PARAM_VALUE","photoId = " + photoDetail.getPhotoPK().getId() + " componentFrom = " +  fromPhotoPK.getInstanceId() + " componentTo = " + getComponentId());
		String[] albums = new String[1];
		albums[0] = fatherId;
		getGalleryBm().updatePhotoPath(photoDetail.getPhotoPK().getId(), albums, fromPhotoPK.getInstanceId(), getComponentId());
	    
		ImageHelper.pasteImage(fromPhotoPK, photoDetail, false);
		
		String id = photoDetail.getPhotoPK().getId();
        SilverTrace.spy("gallery", "GallerySessionController.createPhotoIntoAlbum(photoDetail, fatherId)", getSpaceId(), getComponentId(), id, getUserDetail().getId(), SilverTrace.SPY_ACTION_CREATE);
    }
	
	public synchronized String createImage(PhotoPK fromPhotoPK, PhotoDetail photoDetail, String fatherId) throws RemoteException {
    	photoDetail.getPhotoPK().setSpace(getSpaceId());
	    photoDetail.getPhotoPK().setComponentName(getComponentId());
	    photoDetail.setCreatorId(getUserId());
	    photoDetail.setCreationDate(new Date());
	...
	{
		// envoyer une notification au gestionnaire pour le prévenir de la demande de l'utilisateur
		// 1. création du message
		...
		messageBody 	= messageBody.append(user).append(" ").append(message.getString("gallery.orderNotifBodyAsk")).append("\n");
			
		//english notifications
		String subject_en	= message_en.getString("gallery.orderNotifAskSubject");
		messageBody_en 	= messageBody_en.append(user).append(" ").append(message.getString("gallery.orderNotifBodyAsk")).append("\n");

		NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL, subject, messageBody.toString());
		notifMetaData.addLanguage("en", subject_en, messageBody_en.toString());
			
		notifMetaData.addUserRecipients(admins);
		notifMetaData.setLink(URLManager.getURL(null, getComponentId())+ "OrderView?OrderId="+orderId);
		notifMetaData.setComponentId(getComponentId());
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.sendAskOrder()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	
	public void sendAskOrderUser(String orderId) throws RemoteException
	{
		// envoyer une notification au lecteur pour le prévenir du traitement de sa demande
		// 1. création du message
		...
		//french notifications
		String subject	= message.getString("gallery.orderNotifAskSubject");
		messageBody 	= messageBody.append(user).append(" ").append(message.getString("gallery.orderNotifBodyAskOk")).append("\n");
			
		//english notifications
		String subject_en	= message_en.getString("gallery.orderNotifAskSubject");
		messageBody_en 	= messageBody_en.append(user).append(" ").append(message.getString("gallery.orderNotifBodyAskOk")).append("\n");
		...
		{
			throw new GalleryRuntimeException("GallerySessionController.sendAskOrderUser()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
	...
	public void addToBasket() throws RemoteException
	{
		...
		SilverTrace.debug("gallery","GallerySessionController.addToBasket()","root.MSG_GEN_PARAM_VALUE","listSelected = "+listSelected.toString() + " basket = " + basket.toString());
		clearListSelected();
	}
		...
		while (it.hasNext())
		{
			...
			if (row.getPhotoId() == Integer.parseInt(photoId))
			{
				...
				if (download.equals("DW"))
				{
					...
					String pathFile = FileRepositoryManager.getAbsolutePath(getComponentId()) + nomRep + File.separator;
				...
				url = FileServerUtils.getUrl(getSpaceId(), getComponentId(), URLEncoder.encode(title), photo.getImageMimeType(), nomRep);
			}
		}
		return url;
	}
	...
	public List getMetaDataKeys()
	{
		...
		while (property != null && !"".equals(property))
		{
			...
			String search = Integer.toString(indice) + "/" + getMetadataSettings().getString("IPTC_" + Integer.toString(indice) + "_SEARCH");
			boolean isSearch = (Integer.toString(indice) + "/" + "true").equalsIgnoreCase(search);
			
			if (isSearch)
			{
				// récupération de son type
				String date = Integer.toString(indice) + "/" + getMetadataSettings().getString("IPTC_" + Integer.toString(indice) + "_DATE");
	...
	public NodeBm getNodeBm() {
		  ...
		  try {
			  ...
			  throw new GalleryRuntimeException("GallerySessionController.getNodeBm()",SilverpeasRuntimeException.ERROR, "gallery.EX_IMPOSSIBLE_DE_FABRIQUER_NODEBM_HOME", e);
		  }
		  return nodeBm;
	  }
  ...
  protected boolean isAdminOrPublisher(String profile) {
    return SilverpeasRole.admin.equals(SilverpeasRole.valueOf(profile)) || SilverpeasRole.publisher.equals(SilverpeasRole.valueOf(profile));
  }
}