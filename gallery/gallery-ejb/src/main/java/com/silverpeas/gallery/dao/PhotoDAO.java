...
public class PhotoDAO
{
	...
	public static PhotoDetail getPhoto(Connection con, int photoId) throws SQLException
	{
		// récupérer une photo
	...
	public static Collection getAllPhoto(Connection con, String albumId, String instanceId, boolean viewAllPhoto) throws SQLException
	{
	...
	public static Collection getPhotoNotVisible(Connection con, String instanceId) throws SQLException
	{
	...
	public static Collection getAllPhotos(Connection con, String instanceId) throws SQLException
	{
	...
	public static Collection getAllPhotoEndVisible(Connection con, int nbDays) throws SQLException
	{
		// récupérer toutes les photos de l'instance ayant une date de visibilité arrivant à terme dans 'nbDays' jours
		ArrayList listPhoto = null;
		...
		SilverTrace.debug("gallery", "PhotoDAO.getAllPhotoEndVisible()", "root.MSG_GEN_PARAM_VALUE", "dateLimite = " + dateLimite);
	...
	public static String createPhoto(Connection con, PhotoDetail photo) throws SQLException, UtilException
	{
	...
	public static String createPath(Connection con, PhotoDetail photo, String albumId) throws SQLException, UtilException
	{
	...
	public static void updatePhoto(Connection con, PhotoDetail photo) throws SQLException
	{
		PhotoDetail updatedPhoto = photo;
		PreparedStatement prepStmt = null;
		try
		{
			String query = "update SC_Gallery_Photo set photoId = ? , title = ? , description = ? , sizeH = ? ," + " sizeL = ? , creationDate = ? , updateDate = ? , vueDate = ? , author = ? ," + " download = ? , albumLabel = ? , status = ? , albumId = ? , creatorId = ? , updateId = ? , instanceId = ? ," + " imageName = ? , imageSize = ? , beginDate = ? , endDate = ? , imageMimeType = ? , keyWord = ? ," + " beginDownloadDate = ?, endDownloadDate = ? " + " where photoId = ? ";
			// initialisation des paramètres
			prepStmt = con.prepareStatement(query);
	...
	public static void removePhoto(Connection con, int photoId) throws SQLException
	{
		PreparedStatement prepStmt = null;
		try
		{
	...
	public static Collection getDernieres(Connection con, String instanceId, boolean viewAllPhoto) throws SQLException
	{
		// récupérer toutes les photos d'un album
		ArrayList listPhoto = null;
		Date today = new Date();
	...
	public static boolean isVisible(PhotoDetail photo, Date today)
	{
		...
		if (!StringUtil.isDefined(String.valueOf(beginDate)) && !StringUtil.isDefined(String.valueOf(endDate)))
		{
			result = true;
		}
		else
		{
			...
			if (StringUtil.isDefined(String.valueOf(beginDate)) && StringUtil.isDefined(String.valueOf(endDate)))
			{
				result = beginDate.compareTo(today) <= 0 && endDate.compareTo(today) >= 0;
			}
		}
		return result;
	}
	
	public static Collection getPathList(Connection con, String instanceId, String photoId) throws SQLException
	{
		// récupérer la liste des emplacements de la photo
		ArrayList listPath = null;

		String query = "select N.NodeId from SC_Gallery_Path P, SB_Node_Node N " + "where P.PhotoId = ? and N.nodeId = P.NodeId and P.instanceId = ? and N.instanceId = P.instanceId ";
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		try
		{
	...
	public static void deletePhotoPath(Connection con, String photoId, String instanceId) throws SQLException
	{
		SilverTrace.debug("gallery", "PhotoDAO.deleteAlbumPath()", "root.MSG_GEN_PARAM_VALUE", "photoId = " + photoId);
	...
	public static void addPhotoPath(Connection con, String photoId, String albumId, String instanceId) throws SQLException
	{
		SilverTrace.debug("gallery", "PhotoDAO.addPhotoPath()", "root.MSG_GEN_PARAM_VALUE", "photoId = " + photoId + " albumId = " + albumId);
		...
		try
		{
			...
			//String query2 = "update SC_Gallery_Photo set albumId = ? where photoId = ? and instanceId = ?";
			// initialisation des paramètres
	...
	private static void initParam(PreparedStatement prepStmt, int photoId, PhotoDetail photo) throws SQLException
	{
		prepStmt.setInt(1, new Integer(photoId).intValue());
