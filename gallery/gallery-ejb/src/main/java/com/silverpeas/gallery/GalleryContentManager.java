...
public class GalleryContentManager implements ContentInterface, java.io.Serializable
{
   ...
   public int getSilverObjectId(String photoId, String peasId) {
		SilverTrace.info("gallery","GalleryContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "photoId = "+photoId);
		try {
			return getContentManager().getSilverContentId(photoId, peasId);
		} catch (Exception e) {
			throw new GalleryRuntimeException("GalleryContentManager.getSilverObjectId()",SilverpeasRuntimeException.ERROR,"gallery.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
		}
	}
	...
	*/
   public int createSilverContent(Connection con, PhotoDetail photoDetail, String userId) throws ContentManagerException
   {
	  //SilverContentVisibility scv = new SilverContentVisibility(photoDetail.getBeginDate(), photoDetail.getEndDate());
	  //SilverTrace.info("gallery","GalleryContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  return getContentManager().addSilverContent(con, photoDetail.getPhotoPK().getId(), photoDetail.getPhotoPK().getComponentName(), userId);
   }
   ...
   {
	  int contentId = getContentManager().getSilverContentId(photoPK.getId(), photoPK.getComponentName());
	  if (contentId != -1)
	  {
		  SilverTrace.info("gallery","GalleryContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "photoId = "+photoPK.getId()+", contentId = "+contentId);
		  getContentManager().removeSilverContent(con, contentId, photoPK.getComponentName());
	  }
   }
	...
	private List getHeaders(List ids)
	{
		...
		if (ids != null)
		{
			...
			while (it.hasNext())
			{
				...
				{
					throw new GalleryRuntimeException("GalleryContentManager.getHeaders()",SilverpeasRuntimeException.ERROR,"gallery.MSG_PHOTO_NOT_EXIST", e);				
				}
	...
	private GalleryBm getGalleryBm() 
	{
		if (currentGalleryBm == null) {
			try {
				...
				throw new GalleryRuntimeException("GalleryContentManager.getGalleryBm()",SilverpeasRuntimeException.ERROR, "gallery.EX_IMPOSSIBLE_DE_FABRIQUER_GALLERYBM_HOME", e);
			}
		}
