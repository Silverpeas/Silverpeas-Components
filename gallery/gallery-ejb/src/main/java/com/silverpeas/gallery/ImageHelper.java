...
public class ImageHelper
{
	...
	static ResourceLocator gallerySettings = new ResourceLocator("com.silverpeas.gallery.settings.gallerySettings", "");
	...
	public static boolean isImage(String name)
	{
		...
		return (type.equalsIgnoreCase("gif") || type.equalsIgnoreCase("jpg") || type.equalsIgnoreCase("bmp") || 
				type.equalsIgnoreCase("tiff") || type.equalsIgnoreCase("jpeg") || type.equalsIgnoreCase("png") ||
	...
	{
		String type = FileRepositoryManager.getFileExtension(name);
		return (type.equalsIgnoreCase("gif") || type.equalsIgnoreCase("jpg") || type.equalsIgnoreCase("tiff") || 
	 ...
	 * @throws Exception
	 */
	public static void processImage(PhotoDetail photo, FileItem image, String subDirectory, boolean watermark, String watermarkHD, String watermarkOther) throws Exception
	{
		String name = null;
		...
		if (image != null)
		{
			...
			if (name != null)
			{
				...
				if (runOnUnix)
				{
					...
					SilverTrace.info("gallery", "ImageHelper.processImage", "root.MSG_GEN_PARAM_VALUE", "fileName on Unix = " + name);
				}
				...
				if (isImage(name))
				{
					dir = new File(FileRepositoryManager.getAbsolutePath(instanceId) + subDirectory + photoId + File.separator + name);
	 ...
	 * @throws Exception
	 */
	public static void processImage(PhotoDetail photo, File image, boolean watermark, String watermarkHD, String watermarkOther) throws Exception
	{
		String name = null;
		String mimeType = null;
		long size = 0;
		...
		if (image != null)
		{
			...
			if (name != null)
			{
				...
				if (isImage(name))
				{
					...
					dir = FileRepositoryManager.getAbsolutePath(instanceId) + subDirectory + photoId + File.separator + name;
	...
	private static void createImage(String name, File dir, PhotoDetail photo, String subDirectory, boolean watermark, String watermarkHD, String watermarkOther) throws Exception
	{
		String type = FileRepositoryManager.getFileExtension(name);
		String photoId = photo.getPhotoPK().getId();
		String instanceId = photo.getPhotoPK().getInstanceId();
		...
		if (isValidExtension(name))
		{
			...
			String pathFile = FileRepositoryManager.getAbsolutePath(instanceId) + subDirectory + photoId + File.separator;
			// ajout du watermark (si le paramètre est activé) QUE POUR LES IMAGES JPEG
			String nameAuthor = "";
			String nameForWatermark = "";
			if (StringUtil.isDefined(watermarkHD) && watermark && (type.equalsIgnoreCase("jpg") || type.equalsIgnoreCase("jpeg")))
			{
				// création d'un duplicata de l'image originale avec intégration du watermark
			...
			if (StringUtil.isDefined(watermarkOther) && watermark && (type.equalsIgnoreCase("jpg") || type.equalsIgnoreCase("jpeg")))
			{
	...
	public static void setMetaData(PhotoDetail photo, ResourceLocator settings) throws Exception
	{
		setMetaData(photo, settings, null);
	}
		
	public static void setMetaData(PhotoDetail photo, ResourceLocator settings, ResourceLocator metaDataBundle) throws Exception
	{
		String photoId = photo.getPhotoPK().getId();
		...
		{
			File file = new File(FileRepositoryManager.getAbsolutePath(photo.getInstanceId()) + settings.getString("imagesSubDirectory") + photoId + File.separator + name);
			if (file != null && file.exists())
			{
				...
				while (property != null && !"".equals(property))
				{
					...
					String label = settings.getString("METADATA_" + Integer.toString(indice) + "_LABEL");
					...
					if (value != null)
					{
						if ( currentMetadata == ExifDirectory.TAG_WIN_AUTHOR || currentMetadata == ExifDirectory.TAG_WIN_COMMENT 
								|| currentMetadata == ExifDirectory.TAG_WIN_KEYWORDS || currentMetadata == ExifDirectory.TAG_WIN_SUBJECT 
						...
						SilverTrace.debug("gallery", "GallerySessionController.addMetaData()", "root.MSG_GEN_ENTER_METHOD", "METADATA EXIF label = " + label + " value = " + value);

					}
					// lecture de la property suivante
					indice = indice + 1;
					property = settings.getString("METADATA_" + Integer.toString(indice)+"_TAG");
				}
				
				indice = indice + 1;
				//indice = 1;
				// 2. Traitement des metadata IPTC
				property = settings.getString("IPTC_" + Integer.toString(indice)+"_TAG");
				metadata = JpegMetadataReader.readMetadata(file);
				Directory iptcDirectory = metadata.getDirectory(IptcDirectory.class);
				while (property != null && !"".equals(property))
				{
					// récupération de la valeur
					String label = settings.getString("IPTC_" + Integer.toString(indice) + "_LABEL");
					if (metaDataBundle != null)
						label = metaDataBundle.getString(label);
					...
					if (value != null)
					{
						...
						boolean isDate = settings.getBoolean("IPTC_" + Integer.toString(indice) + "_DATE", false);
						if (isDate)
						{
						...
						photo.addMetaData(metaData);
						SilverTrace.debug("gallery", "GallerySessionController.addMetaData()", "root.MSG_GEN_ENTER_METHOD", "METADATA IPTC label = " + label + " value = " + value);
					}
	...
	private static void getDimension(File inputFile, PhotoDetail photo) throws IOException
	{
	...
	private static void createVignettes(PhotoDetail photo, String path, File dir, boolean watermark, String nameWatermark) throws IOException
	{
		String fileId = photo.getId();

		// création d'une preview sans watermark (pour être utilisée pour créer les vignettes)
		String previewFile = path + fileId + thumbnailSuffix_Xlarge;
		int vignetteFile = 600;
		// on ne redimensionne l'image en preview que si l'image est plus grande que la preview
		...
		if (largeWidth > previewWidth)
			redimPhoto(dir, previewFileWatermark, previewWidth, watermark, nameWatermark, sizeWatermarkPreview);
		else
			redimPhoto(dir, previewFileWatermark, largeWidth, watermark, nameWatermark, sizeWatermarkPreview);
		...
		if (largeWidth > vignetteWidth1)
			redimPhoto(dir, vignetteFile1, vignetteWidth1, watermark, nameWatermark, sizeWatermark266x150);
		else
			redimPhoto(dir, vignetteFile1, largeWidth, watermark, nameWatermark, sizeWatermark266x150);
		...
		if (largeWidth > vignetteWidth2)
			redimPhoto(dir, vignetteFile2, vignetteWidth2, watermark, nameWatermark, sizeWatermark133x100);
		else
			redimPhoto(dir, vignetteFile2, largeWidth, watermark, nameWatermark, sizeWatermark133x100);
		...
		if (largeWidth > vignetteWidth3)
			redimPhoto(dir, vignetteFile3, vignetteWidth3, watermark, nameWatermark, sizeWatermark50x66);
		else
			redimPhoto(dir, vignetteFile3, largeWidth, watermark, nameWatermark, sizeWatermark50x66);
	}
	
	public static String[] getWidthAndHeight(String instanceId, String subDir, String imageName, int baseWidth) throws IOException
	{
	...
	private static void redimPhoto(File inputFile, String outputFile, int widthParam, boolean watermark, String nameWatermark, int sizeWatermark) throws IOException
	{
		...
		BufferedImage scaledImage = getScaledInstance(inputBuf, width, height, RenderingHints.VALUE_INTERPOLATION_BICUBIC, higherQuality);
		...
		if (watermark)
		{
			...
			g.drawString(nameWatermark, ((int) width - (int) rect.getWidth()) - sizeWatermark, ((int) height - (int) rect.getHeight()) - sizeWatermark);
			...
			g.drawString(nameWatermark, ((int) width - (int) rect.getWidth()) - sizeWatermark / 2, ((int) height - (int) rect.getHeight()) - sizeWatermark / 2);
	...
	private static void createWatermark(String fileId, String name, String path, File dir, int percentSizeWatermark) throws IOException
	{
		...
		BufferedImage outputBuf = new BufferedImage((int) inputBufWidth, (int) inputBufHeight, BufferedImage.TYPE_INT_RGB);
		// Ajout du watermark (passage par le graphique pour mettre à jour le buffer)
		Graphics2D g = (Graphics2D) outputBuf.getGraphics();
		g.drawImage(inputBuf, 0, 0, (int) inputBufWidth, (int) inputBufHeight, null);
		...
		g.drawString(name, ((int) inputBufWidth - (int) rect.getWidth()) - size, ((int) inputBufHeight - (int) rect.getHeight()) - size);
		...
		g.drawString(name, ((int) inputBufWidth - (int) rect.getWidth()) - size / 2, ((int) inputBufHeight - (int) rect.getHeight()) - size / 2);
	...
	public static void pasteImage(PhotoPK fromPK, PhotoDetail image, boolean cut)
	{
		...
		try {
			...
			SilverTrace.error("gallery", "ImageHelper.pasteImage", "root.MSG_GEN_PARAM_VALUE", "Unable to create dir : "+toAbsolutePath+nameRep, e);
		}
	...
	private static void pasteFile(String fromImage, String toImage, boolean cut)
	{
		...
		else
		{
			try {
				...
				SilverTrace.error("gallery", "ImageHelper.pasteFile", "root.MSG_GEN_PARAM_VALUE", "Unable to copy file : fromImage = "+fromImage+", toImage = "+toImage, e);
			}
		}
	}
