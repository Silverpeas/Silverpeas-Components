...
public class ScheduledAlertUser implements SchedulerEventHandler {
	...
	public void initialize()
	{
		try
		{
			...
			SimpleScheduler.getJob(this, GALLERYENGINE_JOB_NAME, cron, this, "doScheduledAlertUser");
		}
		catch (Exception e)
		{
			SilverTrace.error("gallery", "ScheduledAlertUser.initialize()", "gallery.EX_CANT_INIT_SCHEDULED_ALERT_USER", e);
		}
	}
	...
	public void handleSchedulerEvent(SchedulerEvent aEvent) {
		switch (aEvent.getType())
        {
			case SchedulerEvent.EXECUTION_NOT_SUCCESSFULL:
				SilverTrace.error("gallery", "ScheduledAlertUser.handleSchedulerEvent", "The job '" + aEvent.getJob().getJobName() + "' was not successfull");
	            break;
			case SchedulerEvent.EXECUTION_SUCCESSFULL:
	            SilverTrace.debug("gallery", "ScheduledAlertUser.handleSchedulerEvent", "The job '" + aEvent.getJob().getJobName() + "' was successfull");
	            break;
			default:
	            SilverTrace.error("gallery", "ScheduledAlertUser.handleSchedulerEvent", "Illegal event type");
	            break;
        }
	}
	...
	{
		SilverTrace.info("gallery", "ScheduledAlertUser.doScheduledAlertUser()", "root.MSG_GEN_ENTER_METHOD");
		...
		try
		{
			...
			SilverTrace.info("gallery", "ScheduledAlertUser.doScheduledAlertUser()", "root.MSG_GEN_PARAM_VALUE", "Photos=" + photos.toString());
	        ...
	        while (it.hasNext())
	        {
	        	...
	        	{
	        		//construire la liste des images pour cette instance (a mettre dans le corps du message)
	        		messageBody.append(message.getString("gallery.notifName")).append(" : ").append(photo.getName()).append("\n");
	        		messageBody_en.append(message_en.getString("gallery.notifName")).append(" : ").append(photo.getName()).append("\n");
	        		SilverTrace.info("gallery", "ScheduledAlertUser.doScheduledAlertUser()", "root.MSG_GEN_PARAM_VALUE", "body=" + messageBody.toString());
 	        	}
	        	else
	        	{
	        		if (currentInstanceId != null)
	        		{
	       			...
	       			SilverTrace.info("gallery", "ScheduledAlertUser.doScheduledAlertUser()", "root.MSG_GEN_PARAM_VALUE", "currentInstanceId = " + currentInstanceId);
	        		
	       			// initialisation du corps du message avec la première photo de l'instance en cours
	        		messageBody.append(message.getString("gallery.notifTitle")).append(nameInstance).append("\n").append("\n");
	        		messageBody.append(message.getString("gallery.notifName")).append(" : ").append(photo.getName()).append("\n");
	        ...
	        //Création du message à envoyer aux admins pour la dernière instance en cours
    		UserDetail[] admins = orga.getUsers("useless", currentInstanceId, "admin");
		...
		{
			throw new GalleryRuntimeException("ScheduledAlertUser.doScheduledAlertUser()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		
		SilverTrace.info("gallery", "ScheduledAlertUser.doScheduledAlertUser()", "root.MSG_GEN_EXIT_METHOD");
	}
	
	private void createMessage(ResourceLocator	message, StringBuffer messageBody, ResourceLocator	message_en, StringBuffer messageBody_en, PhotoDetail photo, UserDetail[] admins)
	{
		// 1. création du message

		//french notifications
		String subject	= message.getString("gallery.notifSubject");
		String body 	= messageBody.append("\n").append(message.getString("gallery.notifUserInfo")).append("\n\n").toString();
		...
		{
			throw new GalleryRuntimeException("ScheduledAlertUser.createMessage()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
	}
		
	private String getPhotoUrl(PhotoDetail photoDetail)
	{
		return URLManager.getURL(null, photoDetail.getInstanceId())+photoDetail.getURL();
	}
	private GalleryBm getGalleryBm()
	{
		...
		{
			throw new GalleryRuntimeException("ScheduledAlertUser.getGalleryBm()", SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
		}
		return galleryBm;
	}
}