...
public class PhotoDetail implements SilverContentInterface, Serializable
{
	...
	{
		// contrôle si la photo est téléchargeable et si la date du jour est comprise dans la période de visibilité
		boolean ok;
		Date date = new Date();
		...
		else
		{
			if (endDownloadDate == null)
				...
				ok = download && date.after(beginDownloadDate) && date.before(endDownloadDate);
		}
		return ok;
	}
