...
public class NewsEditoRequestRouter extends ComponentRequestRouter
{
	...
	public ComponentSessionController createComponentSessionController(MainSessionController mainSessionCtrl, ComponentContext context) {
		ComponentSessionController component = (ComponentSessionController) new NewsEditoSessionController(mainSessionCtrl, context);
		return component;
	}
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
		...
		SilverTrace.debug("NewsEdito", "NewsEditoRequestRooter.getDestination", "NewsEdito.EX_ENTER", "function = " + function);
        ...
        else if ((function.startsWith("Main")) || (function.startsWith("newsEdito")))
        {
            ...
            // initialisation de la variable indiquant que l'utilisateur est en mode consultation
        ...
        else if (function.startsWith("manageArticles"))
        {
            ...
            String flag = componentSC.getUserRoleLevel();
            // initialisation de la variable indiquant que l'utilisateur est en mode manage
  ...
  /*      else if (function.startsWith("GoToFavorite"))
        {
            // initialisation de isConsulting inutile
            String flag = componentSC.getUserRoleLevel();
            String favoriteId = request.getParameter("FavoriteId");

            destination = rootDest + "newsEdito.jsp?Action=SelectTitle&TitleId=" + favoriteId + "&flag=" + flag;
        }   */
        ...
        else if (function.equals("SelectModel"))
        {
         	try
        	{
				...
				request.setAttribute("CurrentPublicationDetail", newsEdito.getCompletePublication().getPublicationDetail());
        	}
			catch (Exception e) 
        	{
        		SilverTrace.warn("NewsEdito", "NewsEditoRequestRooter.getDestination", "NewsEdito.EX_LIST_ERROR");
        ...
        else if (function.startsWith("searchResult"))
        {
            ...
            if (type.equals("Publication"))
            {
                ...
                destination = rootDest + "newsEdito.jsp?Action=SelectPublication&PublicationId=" + id + "&flag=" + flag;
            }
        ...
        else if (function.startsWith("multicopy"))
        {
            try
            {
                ...
                for (int i = 0; i < Ids.length; i++)
                {
                    if (Ids[i] != null)
                    {
                        pub = ((NewsEditoSessionController) componentSC).getCompletePublication(Ids[i]);
            ...
            destination = rootDest + URLManager.getURL(URLManager.CMP_CLIPBOARD) + "Idle.jsp?message=REFRESHCLIPBOARD";
            return destination;
        }
                ...
                while (clipObjectIterator.hasNext())
                {
                    ...
                    if (clipObject != null)
                    {
                        if (clipObject.isDataFlavorSupported(PublicationSelection.CompletePublicationFlavor))
                        {
                            ...
                            pub = (CompletePublication) clipObject.getTransferData(PublicationSelection.CompletePublicationFlavor);
                            news.createPublication(pub.getPublicationDetail().getName(), pub.getPublicationDetail().getDescription());

                            /*if (pub.getModelDetail() != null)
                            {
                                news.setPublicationModel(pub.getModelDetail().getId());
                            }
                            if (pub.getInfoDetail() != null)
                            {
                                if (pub.getInfoDetail().getInfoImageList() != null)
                                {
                                    for (Iterator i = pub.getInfoDetail().getInfoImageList().iterator(); i.hasNext(); )
                                    {
                                        InfoImageDetail attachment = (InfoImageDetail) i.next();
                                        String          from = FileRepositoryManager.getAbsolutePath(pub.getPublicationDetail().getPK().getComponentName()) + news.getSettings().getString("imagesSubDirectory") + "\\" + attachment.getPhysicalName();
                                        ...
                                        String to = FileRepositoryManager.getAbsolutePath(news.getComponentId()) + news.getSettings().getString("imagesSubDirectory") + "\\" + newName;
                        ...
                        else if (clipObject.isDataFlavorSupported(PublicationSelection.PublicationDetailFlavor))
                        {
                            ...
                            pub = (PublicationDetail) clipObject.getTransferData(PublicationSelection.PublicationDetailFlavor);
...
*/
		...
		SilverTrace.info("newsEdito","NewsEditoRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","destination = " + destination);

        return destination;
    }
    
    private void updateXmlForm(List items, NewsEditoSessionController newsEdito) throws FileUploadException, NewsEditoException, RemoteException, PublicationTemplateException, FormException
    {
		...
		PublicationTemplate pub = PublicationTemplateManager.getPublicationTemplate(newsEdito.getComponentId()+":"+xmlFormShortName);
		...
		PagesContext context = new PagesContext("myForm", "3", newsEdito.getLanguage(), false, newsEdito.getComponentId(), newsEdito.getUserId());
    ...
    private void setXMLForm(HttpServletRequest request, NewsEditoSessionController newsEdito, String xmlFormName) throws PublicationTemplateException, FormException, NewsEditoException
	{
		PublicationDetail pubDetail = newsEdito.getCompletePublication().getPublicationDetail();
		String pubId = pubDetail.getPK().getId();
		...
		else
		{
			...
			PublicationTemplateManager.addDynamicPublicationTemplate(newsEdito.getComponentId()+":"+xmlFormShortName, xmlFormName);
		}
						
		PublicationTemplateImpl pubTemplate = (PublicationTemplateImpl) PublicationTemplateManager.getPublicationTemplate(newsEdito.getComponentId()+":"+xmlFormShortName, xmlFormName);
    ...
    private void putXMLDisplayerIntoRequest(PublicationDetail pubDetail, NewsEditoSessionController newsEdito, HttpServletRequest request) throws PublicationTemplateException, FormException
	{
		String infoId = pubDetail.getInfoId();
		String pubId = pubDetail.getPK().getId();
