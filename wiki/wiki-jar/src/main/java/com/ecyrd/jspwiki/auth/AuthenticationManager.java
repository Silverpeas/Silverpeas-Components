 ...
 * This class uses JAAS to determine how users log in.
 * <p>
 * The login procedure is protected in addition by a mechanism which prevents a hacker to try and
 * force-guess passwords by slowing down attempts to log in into the same account. Every login
 * attempt is recorded, and stored for a while (currently ten minutes), and each login attempt
 * during that time incurs a penalty of 2^login attempts milliseconds - that is, 10 login attempts
 * incur a login penalty of 1.024 seconds. The delay is currently capped to 20 seconds.
...
public final class AuthenticationManager {
  ...
  /**
   * If this jspwiki.properties property is <code>true</code>, logs the IP address of the editor on
   * saving.
   */
  public static final String PROP_STOREIPADDRESS = "jspwiki.storeIPAddress";

  /**
   * If this jspwiki.properties property is <code>true</code>, allow cookies to be used for
   * authentication.
   */
  public static final String PROP_ALLOW_COOKIE_AUTH = "jspwiki.cookieAuthentication";

  /**
   * This property determines whether we use JSPWiki authentication or not. Possible values are
   * AUTH_JAAS or AUTH_CONTAINER.
   * <p>
  ...
  /**
   * Value specifying that the user wants to use the container-managed security, just like in
   * JSPWiki 2.2.
   */
  ...
  /**
   * If this jspwiki.properties property is <code>true</code>, allow cookies to be used to assert
   * identities.
   */
  ...
  /**
   * Creates an AuthenticationManager instance for the given WikiEngine and the specified set of
  ...
  public final void initialize(WikiEngine engine, Properties props) throws WikiException {
    ...
    String loginModuleClassName =
        TextUtil.getStringProperty(props, PROP_LOGIN_MODULE, DEFAULT_LOGIN_MODULE);
    try {
   ...
   * primarily for cosmetic purposes in the JSP tier, and performs no meaningful security function
   * per se. Delegates to
   * {@link com.ecyrd.jspwiki.auth.authorize.WebContainerAuthorizer#isContainerAuthorized()}, if
   * used as the external authorizer; otherwise, returns <code>false</code>.
  ...
  /**
   * <p>
   * Logs in the user by attempting to populate a WikiSession Subject from a web servlet request by
   * examining the request for the presence of container credentials and user cookies. The
   * processing logic is as follows:
   * </p>
   * <ul>
   * <li>If the WikiSession had previously been unauthenticated, check to see if user has
   * subsequently authenticated. To be considered "authenticated," the request must supply one of
   * the following (in order of preference): the container <code>userPrincipal</code>, container
   * <code>remoteUser</code>, or authentication cookie. If the user is authenticated, this method
   * fires event {@link com.ecyrd.jspwiki.event.WikiSecurityEvent#LOGIN_AUTHENTICATED} with two
   * parameters: a Principal representing the login principal, and the current WikiSession. In
   * addition, if the authorizer is of type WebContainerAuthorizer, this method iterates through the
   * container roles returned by
   * {@link com.ecyrd.jspwiki.auth.authorize.WebContainerAuthorizer#getRoles()}, tests for
   * membership in each one, and adds those that pass to the Subject's principal set.</li>
   * <li>If, after checking for authentication, the WikiSession is still Anonymous, this method next
   ...
   * {@link com.ecyrd.jspwiki.event.WikiSecurityEvent#LOGIN_ASSERTED} with two parameters:
   * <code>WikiPrincipal(<em>cookievalue</em>)</code>, and the current WikiSession.</li>
   * <li>If, after checking for authenticated and asserted status, the WikiSession is <em>still</em>
   * anonymous, this method fires event
   * {@link com.ecyrd.jspwiki.event.WikiSecurityEvent#LOGIN_ANONYMOUS} with two parameters:
   * <code>WikiPrincipal(<em>remoteAddress</em>)</code>, and the current WikiSession</li>
   * </ul>
   * @param request servlet request for this user
   * @return always returns <code>true</code> (because anonymous login, at least, will always
   * succeed)
   * @throws com.ecyrd.jspwiki.auth.WikiSecurityException if the user cannot be logged in for any
   * reason
   * @since 2.3
   */
  public final boolean login(HttpServletRequest request) throws WikiSecurityException {
    ...
    if (!session.isAuthenticated()) {
      ...
      try {
        handler =
            new WebContainerCallbackHandler(m_engine, request, authorizationMgr.getAuthorizer());
      ...
      Set<Principal> principals =
          authenticationMgr.doJAASLogin(m_loginModuleClass, handler, m_loginModuleOptions);
      ...
      if (principals.size() == 0 && authenticationMgr.allowsCookieAuthentication()) {
        principals =
            authenticationMgr.doJAASLogin(CookieAuthenticationLoginModule.class, handler, options);
      }

      // If the container logged the user in successfully, tell the WikiSession (and add all of the
      // Principals)
      if (principals.size() > 0) {
    ...
    if (!session.isAuthenticated() && authenticationMgr.allowsCookieAssertions()) {
      ...
      Set<Principal> principals =
          authenticationMgr.doJAASLogin(CookieAssertionLoginModule.class, handler, options);
      if (principals.size() > 0) {
    ...
    if (session.isAnonymous()) {
      Set<Principal> principals =
          authenticationMgr.doJAASLogin(AnonymousLoginModule.class, handler, options);
      if (principals.size() > 0) {
   ...
   * {@link com.ecyrd.jspwiki.auth.login.UserDatabaseLoginModule} class will be used. When the
   * LoginModule's <code>initialize</code> method is invoked, an options Map populated by properties
   * keys prefixed by {@link #PREFIX_LOGIN_MODULE_OPTIONS} will be passed as a parameter.
   * @param session the current wiki session; may not be null.
   * @param username The user name. This is a login name, not a WikiName. In most cases they are the
   * same, but in some cases, they might not be.
   * @param password the password
   * @return true, if the username/password is valid
   * @throws com.ecyrd.jspwiki.auth.WikiSecurityException if the Authorizer or UserManager cannot be
   * obtained
   */
   ...
   * {@link com.ecyrd.jspwiki.auth.authorize.Role}, <code>false</code> otherwise
   */
   ...
   * {@link com.ecyrd.jspwiki.auth.authorize.Role}, <code>true</code> otherwise
   */
  ...
  /**
   * Instantiates and executes a single JAAS {@link javax.security.auth.spi.LoginModule}, and
   * returns a Set of Principals that results from a successful login. The LoginModule is
   * instantiated, then its
   * {@link javax.security.auth.spi.LoginModule#initialize(Subject, CallbackHandler, Map, Map)}
   * method is called. The parameters passed to <code>initialize</code> is a dummy Subject, an empty
   * shared-state Map, and an options Map the caller supplies.
  ...
  /**
   * Returns the first Principal in a set that isn't a {@link com.ecyrd.jspwiki.auth.authorize.Role}
   * or {@link com.ecyrd.jspwiki.auth.GroupPrincipal}.
