...
public class RssAgregatorCache
{
	...
	{
		ResourceLocator res = new ResourceLocator("com.silverpeas.rssAgregator.settings.rssAgregatorSettings", "");
	...
	{
		if (instance == null) instance = new RssAgregatorCache();
		return instance;
	}
	 ...
	 */
	/*public void addContentToCache(String id, StringBuffer content)
	{
		// Store content informations to cache
		cache.put(id, content);

		// Store time of content informations storage
		long currentTime = System.currentTimeMillis();
		cacheLastRefresh.put(id, new Long(currentTime));
	}*/

	/**
	 * Get a cached content.
	 * If content is not cached, return null.
	 */
	 ...
	 * Add or replace a content in the cache
	 */
	/*public void addChannelsToCache(String instanceId, List channels)
	{
		// Store channels to cache
		cache.put(instanceId, channels);

		// Store time of content informations storage
		long currentTime = System.currentTimeMillis();
		cacheNextRefresh.put(instanceId, new Long(currentTime));
	}*/
	...
	public void addChannelToCache(SPChannel spChannel)
	{
		...
		// Store time of content informations storage
		long currentTime = System.currentTimeMillis();
		int refreshRate = spChannel.getRefreshRate()*60*1000; //refresh rate in ms
	 ...
	 * If content is not cached, return null.
	 */
	/*public StringBuffer getContentFromCache(String id)
	{
		return (StringBuffer) cache.get(id);
	}*/

	/**
	 * Return true if a cached content need to be updated
	 * or a content is not cached.
	 */
