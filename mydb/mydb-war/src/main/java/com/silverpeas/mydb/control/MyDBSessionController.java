	...
	extends AbstractComponentSessionController
{
			...
			if (connection != null)
			{
				...
				if (dbTable != null)
				{
					final String query = new StringBuffer(100)
						...
						.append(dbFilter.getQueryFilter(dbTable))
						.toString();
					startConnection();
					try
					{
						...
						while (rs.next())
						{
							...
							if ((!manualFilter)
								|| ((data != null) && (data.toLowerCase().indexOf(filterValue) != -1)))
							{
		...
		if (!dbTable.getPrimaryKey().isEmpty())
		{
			try {
				...
				StringBuffer query = new StringBuffer(100)
					.append("select ").append(primaryKeys[0])
					.append(" from ").append(dbTable.getName())
					.append(" where ");
	...
	public String createDbLine()
	{
		try
		{
			...
			StringBuffer query = new StringBuffer(100).append("insert into ").append(dbTable.getName()).append(" (")
				.append(DbUtil.getListAsString(columnsNames)).append(") values (");
				...
				for (int i = 0; i < columnsCount; i++)
				{
					...
					if (value != null && value.length() > 0)
					{
						...
						query = new StringBuffer(100)
							.append("select ").append(columnName).append(" from ").append(tableName)
						...
						if (resultCount == 1)
						{
							...
							for (int j = 0, m = foreignKeys.length; j < m; j++)
							{
								...
								query = new StringBuffer(100)
									.append("select ").append(fkColumnName).append(" from ").append(fkTableName)
									.append(" where ").append(fkColumnName).append(" = ?")
									.toString();
								prepStmt = connection.prepareStatement(query);
								setValueByType(value, dataType, 1);
								rs = prepStmt.executeQuery();
