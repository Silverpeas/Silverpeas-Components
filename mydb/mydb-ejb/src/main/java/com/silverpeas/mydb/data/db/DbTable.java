...
public class DbTable
{
	...
	public String[] getColumnsNames(boolean autoIncrementFilter)
	{
		if (autoIncrementFilter)
		{
			...
			return (String[])columnsNamesList.toArray(new String[columnsNamesList.size()]);			
		}
		else
		{
	...
	public String[][] getForeignKeyColumnsNames(String keyName)
	{
		...
		for (int i = 0, n = columns.size(); i < n; i++)
		{
			...
			if (column.hasImportedForeignKey())
			{
				...
				{
					columnsNames.add(new String[] {column.getName(), foreignKey.getColumnName()});
				}
			}
		}
		return (String[][])(columnsNames.toArray(new String[columnsNames.size()][2]));
	}
