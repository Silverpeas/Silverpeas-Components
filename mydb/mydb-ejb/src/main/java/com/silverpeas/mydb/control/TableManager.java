...
public class TableManager {
  ...
  public void updateColumn(DbColumn column, int index) {
    ...
    if (primaryKey.isPrimaryKey(newColumnName)
        || unicityKeys.isUnicityKey(newColumnName)) {
  ...
  public boolean isValidColumn(DbColumn column, ResourcesWrapper resources,
    ...
    return (isValidKeyName(column, resources, exceptedIndex)
        && isValidColumnType(column, resources) && isValidColumnDefaultValue(
        column, resources));
  }
  ...
  public boolean isValidKeyName(Object object, ResourcesWrapper resources,
    ...
    for (int i = 0, n = unicityKeys.getSize(); i < n; i++) {
      if (i != exceptedUKIndex
          && nameUpperCase.equals(unicityKeys.get(i).getName().toUpperCase())) {
    ...
    for (int i = 0, n = foreignKeys.getSize(); i < n; i++) {
      if (i != exceptedFKIndex
          && nameUpperCase.equals(foreignKeys.get(i).getName().toUpperCase())) {
  ...
  public String getTableCreationQuery() {
    ...
    for (int i = 0, n = columns.length; i < n; i++) {
      ...
      querySb.append(column.getName()).append(" ").append(
          dataTypeList.getDataTypeName(column.getDataType()));
    ...
    if (!primaryKey.isEmpty()) {
      querySb.append(", CONSTRAINT ").append(primaryKey.getName()).append(
          " PRIMARY KEY ").append("(").append(
          DbUtil.getListAsString(primaryKey.getColumns())).append(")");
    }
    querySb.append(")");
  ...
  public String[] getUnicityKeysQueries() {
    ...
    for (int i = 0, n = unicityKeys.getSize(); i < n; i++) {
      ...
      querySb = new StringBuffer(200).append("ALTER TABLE ").append(
          table.getName()).append(" ADD CONSTRAINT ").append(
          unicityKey.getName()).append(" UNIQUE ").append("(").append(
          DbUtil.getListAsString(unicityKey.getColumns())).append(")");
  ...
  public String[] getForeignKeysQueries() {
    ...
    for (int i = 0, n = foreignKeys.getSize(); i < n; i++) {
      ...
      querySb = new StringBuffer(200).append("ALTER TABLE ").append(
          table.getName()).append(" ADD CONSTRAINT ").append(
          foreignKey.getName()).append(" FOREIGN KEY ").append("(").append(
          DbUtil.getListAsString(foreignKey.getColumns())).append(")").append(
          " REFERENCES ").append(foreignKey.getForeignTable()).append(" (")
          .append(DbUtil.getListAsString(foreignKey.getForeignColumnsNames()))
          .append(")");
      queries.add(querySb.toString());
    }
    return (String[]) queries.toArray(new String[queries.size()]);
