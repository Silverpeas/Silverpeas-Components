...
public class ForeignKeys {
	...
	public String getConstraintName()
	{
		...
		if (StringUtil.isDefined(tableName))
		{
			...
			String foreignKey = ForeignKey.FOREIGN_KEY_PREFIX + tableName + "_" + foreignIndex;
			while (uKeys.contains(foreignKey))
			{
				foreignIndex++;
				foreignKey = ForeignKey.FOREIGN_KEY_PREFIX + tableName + "_" + foreignIndex;
			}
			return foreignKey;
		}
		else
	...
	public ForeignKeyError[] getErrors(DataTypeList dataTypeList, ResourcesWrapper resources)
	{
		...
		for (int i = 0, n = getSize(); i < n; i++)
		{
			...
			for (int j = 0, m = foreignKey.getColumnsCount(); j < m; j++)
			{
				...
				{
					label = MessageFormat.format(resources.getString("ErrorForeignKeyTypes"), new String[] {
						foreignKey.getName(), columnName, dataType.getName(), foreignColumn.getName(),
						foreignKey.getForeignTable(), dataTypeList.getDataTypeName(foreignColumn.getDataType())});
					errors.add(new ForeignKeyError(
						columnName, label, ForeignKeyError.ERROR_TYPE, foreignColumn.getDataType()));
				}
				if (column.getDataSize() != foreignColumn.getDataSize() && dataType.isSizeEnabled())
				{
					String columnSize = (column.hasDataSize()
						? column.getDataSizeAsString() : resources.getString("UndefinedFemale"));
					String foreignColumnSize = (foreignColumn.hasDataSize()
						? foreignColumn.getDataSizeAsString() : resources.getString("UndefinedFemale"));
					label = MessageFormat.format(resources.getString("ErrorForeignKeySizes"), new String[] {
						foreignKey.getName(), columnName, columnSize, foreignColumn.getName(),
						foreignKey.getForeignTable(), foreignColumnSize});
					errors.add(new ForeignKeyError(
						columnName, label, ForeignKeyError.ERROR_SIZE, foreignColumn.getDataSize()));
				}
			}
		}
