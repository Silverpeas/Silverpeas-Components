...
public class MyDBBmEJB
	implements MyDBBmBusinessSkeleton, SessionBean
{
	...
	{
		return SilverpeasBeanDAOFactory.getDAO(BEAN_NAME).findByWhereClause(
			pk, "instanceId = " + "'" + pk.getComponentName() + "'");
	}

	public MyDBConnectionInfoDetail getConnection(MyDBConnectionInfoPK pk)
		throws RemoteException, PersistenceException
	{
		return (MyDBConnectionInfoDetail) SilverpeasBeanDAOFactory.getDAO(BEAN_NAME).findByPrimaryKey(pk);
	}

	public MyDBConnectionInfoPK addConnection(MyDBConnectionInfoDetail detail)
		throws RemoteException, PersistenceException
	{
		return (MyDBConnectionInfoPK) SilverpeasBeanDAOFactory.getDAO(BEAN_NAME).add(detail);
	}
