...
public class DataWarningQueryResult extends Object
{
    ...
    {
		SilverTrace.info("dataWarning", "DataWarningQueryResult.returnTriggerValueFromResult()", "root.MSG_GEN_ENTER_METHOD");
        try
        {
        	if (isPersoEnabled())
        	{
				SilverTrace.info("dataWarning", "DataWarningQueryResult.returnTriggerValueFromResult()", "root.MSG_GEN_PARAM_VALUE","persoColumnNumber="+persoColumnNumber);
        		long maxVal = 0;
        		long theVal;
        		int theCol;
				...
				SilverTrace.info("dataWarning", "DataWarningQueryResult.returnTriggerValueFromResult(perso)", "root.MSG_GEN_PARAM_VALUE", "maxVal = " + maxVal);
    			return maxVal;
        	}
        	else
        	{
        ...
        {
			throw new DataWarningException("DataWarningQueryResult.returnTriggerValueFromResult()", SilverpeasException.ERROR, "DataWarning.EX_DATA_ACCESS_FAILED", e);
        }
    }
    ...
    public long returnTriggerValueFromResult(String userId) throws DataWarningException 
    {
        try
        {
			if (isPersoEnabled())
			{
				SilverTrace.info("dataWarning", "DataWarningQueryResult.returnTriggerValueFromResult(userId,persoEnabled)", "root.MSG_GEN_PARAM_VALUE", "maxVal = " + getValue(userId,0,0));
				return Long.parseLong(getValue(userId,0,0));
			}
			else
			{
				SilverTrace.info("dataWarning", "DataWarningQueryResult.returnTriggerValueFromResult(userId)", "root.MSG_GEN_PARAM_VALUE", "maxVal = " +Long.parseLong(getValue(0,0)));   
				return Long.parseLong(getValue(0,0));
			}
        }
        catch (Exception e)
        {
			throw new DataWarningException("DataWarningQueryResult.returnTriggerValueFromResult()", SilverpeasException.ERROR, "DataWarning.EX_DATA_ACCESS_FAILED", "UserId = "+userId, e);
        }
    }
    ...
    public ArrayList getValues(String userId)
    {
    	if (isPersoEnabled())
    	{
			...
			SilverTrace.debug("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "userPersoValue = " + userPersoValue);
			ArrayList valret = new ArrayList();
			for (int i=0; i<values.size(); i++)
			{
				ArrayList valeur = (ArrayList) getValues().get(i);
				SilverTrace.debug("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "valeur = " + valeur);
				SilverTrace.debug("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "valeur.size = " + valeur.size());
				SilverTrace.debug("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "persoColumnNumber = " + persoColumnNumber);
				SilverTrace.debug("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "userPersoValue = " + userPersoValue);
				if (valeur.size() > 1)
				{
			...
			SilverTrace.info("dataWarning", "DataWarningQueryResult.getValues(userId)", "root.MSG_GEN_PARAM_VALUE", "Taille valret = " + valret.size());
    		return valret;
    	}
    	else 
    		return getValues();
    ...
    protected ArrayList getRow(String userId, int row)
    {
    	if (isPersoEnabled())
    	{
			...
			SilverTrace.info("dataWarning", "DataWarningQueryResult.getRow(userId, row)", "root.MSG_GEN_PARAM_VALUE", "userId=" + userId+" row="+row+" al="+al);
        	if (al.size() > row)
    ...
    public String getValue(String userId, int row, int col)
    {
		...
		SilverTrace.info("dataWarning", "DataWarningQueryResult.getValue(userId, row, col)", "root.MSG_GEN_PARAM_VALUE", "UserId=" + userId +" Row="+row+" Col="+col);
		ArrayList al = getRow(userId, row);
		SilverTrace.info("dataWarning", "DataWarningQueryResult.getValue(userId, row, col)", "root.MSG_GEN_PARAM_VALUE", "al=" + al);
    	if (al.size() > col)
    	{
    		return (String)(al.get(col));
    	}
	...
	public String returnPersoValue(String userId)
	{
		...
		{
            SilverTrace.warn("dataWarning", "DataWarningQueryResult.returnPersoValue()","root.MSG_GEN_ENTER_METHOD", "UserId=" + userId, e);
            return "";
		}
	}
}
