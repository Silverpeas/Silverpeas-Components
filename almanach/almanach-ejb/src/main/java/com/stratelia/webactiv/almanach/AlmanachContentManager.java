...
public class AlmanachContentManager implements ContentInterface {
	...
	public int getSilverObjectId(String pubId, String peasId) throws ContentManagerException {
		SilverTrace.info("almanach","AlmanachContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubId);
     ...
     */
    public int createSilverContent(Connection con, EventDetail eventDetail, String userId) throws ContentManagerException {
        SilverContentVisibility scv = new SilverContentVisibility();
        return getContentManager().addSilverContent(con, eventDetail.getPK().getId(), eventDetail.getPK().getComponentName(), userId, scv);
    }
    ...
    public void deleteSilverContent(Connection con, EventPK eventPK) throws ContentManagerException {
        int contentId = getContentManager().getSilverContentId(eventPK.getId(), eventPK.getComponentName());
        SilverTrace.info("almanach", "AlmanachContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "pubId = " + eventPK.getId() + ", contentId = " + contentId);
		if (contentId != -1)
     ...
     */
    public void updateSilverContentVisibility(EventDetail eventDetail) throws ContentManagerException {
        int silverContentId = getContentManager().getSilverContentId(eventDetail.getPK().getId(), eventDetail.getPK().getComponentName());
        SilverContentVisibility scv = new SilverContentVisibility();
        SilverTrace.info("almanach", "AlmanachContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = " + scv);

		if (silverContentId == -1)
			createSilverContent(null, eventDetail, eventDetail.getDelegatorId());
		else
			getContentManager().updateSilverContentVisibilityAttributes(scv, eventDetail.getPK().getComponentName(), silverContentId);
    ...
    private ContentManager getContentManager() {
        if (contentManager == null) {
            try {
                ...
                SilverTrace.fatal("almanach", "AlmanachContentManager.getContentManager()", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
            }
        }
