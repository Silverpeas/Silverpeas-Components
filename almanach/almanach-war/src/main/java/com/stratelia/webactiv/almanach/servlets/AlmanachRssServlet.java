...
{    	
    ...
    public Collection getListElements(String instanceId, int nbReturned) throws RemoteException {
    	...
    	Collection allEvents = getAlmanachBm().getAllEvents(new EventPK("", "", instanceId));
    	net.fortuna.ical4j.model.Calendar calendarAlmanach = getAlmanachBm().getICal4jCalendar(allEvents, "fr");
    	
    	Calendar currentDay = GregorianCalendar.getInstance();
    	Collection events = getAlmanachBm().getListRecurrentEvent(calendarAlmanach, null, "", instanceId);
    	if(events != null) {
   ...
   private AlmanachBm getAlmanachBm()
	{
	   AlmanachBm almanachBm = null;
		try 
		{
			AlmanachBmHome almanachBmHome = (AlmanachBmHome) EJBUtilitaire.getEJBObjectRef(JNDINames.ALMANACHBM_EJBHOME, AlmanachBmHome.class);
			almanachBm = almanachBmHome.create();
		} 
		catch (Exception e) 
		{
			throw new AlmanachRuntimeException("AlmanachRssServlet.getAlmanachBm()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
		}
		return almanachBm;
	}
}