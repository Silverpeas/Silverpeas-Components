...
public class AlmanachRequestRouter extends ComponentRequestRouter {
	...
	public ComponentSessionController createComponentSessionController(
		...
		return (
			(ComponentSessionController) new AlmanachSessionController(mainSessionCtrl,
			context));
	}
	 ...
	 * @param request
	 */
	private void setGlobalInfo(AlmanachSessionController almanach, HttpServletRequest request) {
	...
	public String getDestination(
		...
		SilverTrace.info(
			"almanach",
			"AlmanachRequestRouter.getDestination()",
		...
		try
		{
			if (function.startsWith("Main") || function.startsWith("almanach") || function.startsWith("portlet")) {	
				...
				//accès première fois, initialisation de l'almanach à la date du jour (utile pour générer le Header)
				...
				while(itVEvent.hasNext()) {
					...
					while(itPeriod.hasNext()) {
					    ...
					    evt = new Event(idEvent, evtDetail.getName(), new java.util.Date(recurrencePeriod.getStart().getTime()), new java.util.Date(recurrencePeriod.getEnd().getTime()), evtDetail.getURL(), evtDetail.getPriority());
					    ...
					    if (almanach.isAgregationUsed()) {
					    	evt.setColor(almanach.getAlmanachColor(evtDetail.getInstanceId()));
					    }
			...
			{		
				...
				if(unity != null && ! "0".equals(unity)) {
					...
					if(periodicityUntilDate != null && periodicityUntilDate.length()>0) {
						periodicity.setUntilDatePeriod(DateUtil.stringToDate(periodicityUntilDate, almanach.getLanguage()));
					}
			...
			{
				String id = request.getParameter("Id"); //peut etre null en cas de création
				String dateIteration = request.getParameter("Date"); //peut etre null en cas de création (yyyy/MM/jj)
			...
			{
				String action = request.getParameter("Action");//ReallyUpdateOccurence | ReallyUpdateSerial | ReallyUpdate
				String id = request.getParameter("Id"); //not null
				String dateDebutIteration = request.getParameter("DateDebutIteration"); //format client
				String dateFinIteration = request.getParameter("DateFinIteration"); //format client
				...
				{
					java.util.Date startDateEvent = DateUtil.stringToDate(periodicityStartDate, almanach.getLanguage());
			...
			{ //??
				destination = "/almanach/jsp/editAttFiles.jsp?Id="+ request.getParameter("Id");
			}
			else if (function.equals("RemoveEvent"))
			{
				...
				if("ReallyDeleteOccurence".equals(action)) {
					...
					almanach.removeOccurenceEvent(almanach.getCurrentUpdateEvent(), dateDebutIteration, dateFinIteration);
					
				} else if ("ReallyDelete".equals(action)) {
					
					//Supprime l'événement et toutes les occurences de la série
					almanach.removeEvent(almanach.getCurrentUpdateEvent().getPK().getId());
				}
			...
			else if (function.equals("UpdateAgregation"))
			{
				...
				SilverTrace.info("almanach","AlmanachRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","instanceIds = " + instanceIds);
			...
			{ 
				String id = request.getParameter("Id");
				SilverTrace.info("almanach","AlmanachRequestRouter.getDestination()","root.MSG_GEN_PARAM_VALUE","id = " + id);
				try
				{
