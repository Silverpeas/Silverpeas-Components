...
public class ConnecteurJDBCRequestRouter extends ComponentRequestRouter
{
     ...
     * @see
     */
    public ComponentSessionController createComponentSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext) throws ConnecteurJDBCRuntimeException
    {
        ComponentSessionController component = (ComponentSessionController) new ConnecteurJDBCSessionController(mainSessionCtrl, componentContext);
    ...
    /**
     * The rooter compute a destination page given :
     * the function called by the client,
     * the current state of the component (embbeded in the session controleur)
     * and parameters from the request object.
     *
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        else if (function.startsWith("UpdateConnection"))
        {
		    ...
		    {
				SilverTrace.warn("connecteurJDBC","ConnecteurJDBCRequestRouter.getDestination()","connecteurJDBC.MSG_CONNECTION_NOT_STARTED",e);
		    }
