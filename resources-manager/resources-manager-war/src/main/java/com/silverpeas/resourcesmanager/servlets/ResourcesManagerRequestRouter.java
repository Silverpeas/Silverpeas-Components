...
public class ResourcesManagerRequestRouter extends ComponentRequestRouter
{
    ...
    private ReservationDetail request2ReservationDetail(HttpServletRequest request,ComponentSessionController resourcesManagerSC )
    {
    	...
    	try{
    		...
    		SilverTrace.info("resourcesManager", "ResourcesManagerRequestRouter.request2ReservationDetail()", "root.MSG_GEN_PARAM_VALUE", "reservation="+reservation);
    		return reservation;
    	}
    	catch (Exception e) {
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        try
        {
            ...
            {
            	List listTemplates = PublicationTemplateManager.getPublicationTemplates();
            ...
            else if (function.equals("ViewResource"))
            {
            	...
            	request.setAttribute("resource", resource);
            	request.setAttribute("ShowComments", Boolean.valueOf(resourcesManagerSC.areCommentsEnabled()));
            ...
            else if (function.equals("NewReservation"))
            {
            	...
            	ReservationDetail reservation = null;
            	List listResourcesProblem = (List)request.getAttribute("listeResourcesProblem");
            	...
            	if(idReservation != null){
            		SilverTrace.info("resourcesManager", "ResourcesManagerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "dans le if,idReservation="+idReservation);
            		listResourceEverReserved = resourcesManagerSC.getResourcesofReservation(idReservation);
            		if (listResourceEverReserved != null){
//            			 boucle permettant de supprimer les réservations déjà réservées qui posent problème, car elles ont déjà été réservées
        	  			for (int i=0;i<listResourceEverReserved.size();i++){
        	  				ResourceDetail resourceReserved = (ResourceDetail)listResourceEverReserved.get(i);
        	  				if(listResourcesProblem != null){
        	  					for(int j=0;j<listResourcesProblem.size();j++){
        	  						ResourceDetail resourceProblem = (ResourceDetail)listResourcesProblem.get(j);
                ...
                maListResourcesReservable = resourcesManagerSC.getResourcesReservable(reservation.getBeginDate(),reservation.getEndDate());
            	SilverTrace.info("resourcesManager", "ResourcesManagerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "listResourcesReservable="+maListResourcesReservable.size());
            	//on envoie l'id de la réservation et l'ensemble des resources associées à celles -ci
            	...
            	//listeReservation est la liste complète des ressources réservées lors d'une création ou d'une édition de réservation
            	//idModifiedReservation est l'id de la réservation à modifié lors d'une édition de réservation            	 
            	// on doit vérifier que celles-ci n'ont pas été prises avant de mettre à jour la réservation.
            	...
            	//si idModifiedReservation n'est pas nulle, on est en train de modifier une réservation
            	//sinon on est en train de créer une réservation
            	if(idModifiedReservation != null){            		
            		// on vérifie que les nouvelles ressources que l'on veut réserver ne sont pas déjà prises. 
            		List listeResourcesProblemeReservationTotal = resourcesManagerSC.getResourcesProblemDate(listeReservation,resourcesManagerSC.getBeginDateReservation(),resourcesManagerSC.getEndDateReservation(),idModifiedReservation);
            			if(listeResourcesProblemeReservationTotal.isEmpty()){
            			resourcesManagerSC.updateReservation(idModifiedReservation,listeReservation);
            			destination = getDestination("Calendar", componentSC, request);
            		}
            		else {            			
            			request.setAttribute("listeResourcesProblem", listeResourcesProblemeReservationTotal);
            			request.setAttribute("reservationId", idModifiedReservation);
            			destination = getDestination("GetAvailableResources", componentSC, request);
            		}
            	}
            	else{
            		resourcesManagerSC.setListReservationCurrent(listeReservation);
            		List listeResourcesProblem = resourcesManagerSC.verificationReservation(listeReservation);
            	...
            	//listResources est la liste des ressources qui peuvent poser problème quand on change la date de réservation
            	...
            	// on envoie la réservation de l'id et la liste des ressources associées ainsi que la liste qui posent problème quand on change les dates
            	request.setAttribute("reservation", reservation);
            	//request.setAttribute("listResourceEverReserved", listResourceEverReserved);
            	request.setAttribute("listResourcesProblem", listResourcesProblem);
            	destination = root+"reservationManager.jsp";
            	...
            	// si on vient de resource.jsp, reservationId a été stocké dans le session controler
            	...
            	List listResourcesofReservation = resourcesManagerSC.getResourcesofReservation(reservationId);
            ...
            //	destination = root + "almanach.jsp";
            	...
            	if((myObjectView != null) && (!myObjectView.equals("myReservation")) && (!myObjectView.equals("PlanningOtherUser")) && (!myObjectView.equals("viewUser"))){
            		List listReservationsOfCategory = resourcesManagerSC.getMonthReservationOfCategory(myObjectView);
            		List listResourcesofCategory = resourcesManagerSC.getResourcesByCategory(myObjectView);
            		...
            		request.setAttribute("listReservationsOfCategory", listReservationsOfCategory);
            	}
            	// on regarde les réservations de quelqu'un :
            	// si userId = null on regarde ses réservations, sinon on regarde les réservations de cette personne
            	else {
            		List listOfReservation = null;
        			if((myObjectView != null) && (myObjectView.equals("PlanningOtherUser"))){
        				...
        				request.setAttribute("firstNameUser", resourcesManagerSC.getFirstNameUserCalandar());
        				request.setAttribute("lastName", resourcesManagerSC.getLastNameUserCalandar());        				
        			}
        			else{
        ...
        SilverTrace.info("resourcesManager", "ResourcesManagerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
	...
	private void setXMLFormIntoRequest(HttpServletRequest request, ResourcesManagerSessionController resourcesManagerSC) throws Exception
	{
		String idResource = resourcesManagerSC.getResourceIdForResource();
        ...
        if (StringUtil.isDefined(xmlFormName))
        {
	        ...
	        PublicationTemplateManager.addDynamicPublicationTemplate(resourcesManagerSC.getComponentId()+":"+xmlFormShortName, xmlFormName);
	        PublicationTemplateImpl pubTemplate = (PublicationTemplateImpl)PublicationTemplateManager.getPublicationTemplate(resourcesManagerSC.getComponentId()+":"+xmlFormShortName, xmlFormName);
	...
	private void putXMLDisplayerIntoRequest(ResourceDetail resource, HttpServletRequest request, ResourcesManagerSessionController  resourcesManagerSC) throws PublicationTemplateException, FormException
    {
       // récupération de l’Id de l’objet en fonction de l’objet "object" 
    	...
    	request.setAttribute("XMLForm", formView);
       	...
       	PagesContext context = new PagesContext("myForm", "0", resourcesManagerSC.getLanguage(), false, resourcesManagerSC.getComponentId(), resourcesManagerSC.getUserId());
     	context.setBorderPrinted(false);	
     	context.setObjectId(resourceId);
       	request.setAttribute("context", context);
    }
	
	private void updateXMLForm(ResourcesManagerSessionController resourcesManagerSC, List items) throws Exception
	{
    	...
    	if (StringUtil.isDefined(xmlFormName))
        {
	    	...
	    	PublicationTemplate pub =  PublicationTemplateManager.getPublicationTemplate(resourcesManagerSC.getComponentId()+":"+xmlFormShortName);
