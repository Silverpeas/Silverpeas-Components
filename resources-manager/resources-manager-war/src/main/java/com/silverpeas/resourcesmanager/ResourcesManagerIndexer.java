...
public class ResourcesManagerIndexer implements ComponentIndexerInterface {
    ...
    public void index(MainSessionController mainSessionCtrl, ComponentContext context) throws Exception {
    	 SilverTrace.info("resourcesManager", "ResourcesManagerIndexer.index()", "root.MSG_GEN_PARAM_VALUE", "index, context.getCurrentComponentId() = " + context.getCurrentComponentId());
    	getResourceManagerBm().indexResourceManager(context.getCurrentComponentId());
    }
    ...
    private ResourcesManagerBm getResourceManagerBm()
	{
		...
		{
			throw new ResourcesManagerRuntimeException("ResourcesManagerSessionController.getResourceManagerBm()",
