...
public class QuickInfoRequestRouter extends ComponentRequestRouter
{
	...
	public ComponentSessionController createComponentSessionController(MainSessionController mainSessionCtrl, ComponentContext componentContext) {
		ComponentSessionController component = (ComponentSessionController) new QuickInfoSessionController(mainSessionCtrl, componentContext);
		return component;
	}

	private void setGlobalInfo(QuickInfoSessionController quickInfo, HttpServletRequest request) {
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request) {
		...
		try
		{
			if ( (function.startsWith("Main")) || (function.startsWith("quickInfoUser")) || (function.startsWith("quickInfoPublisher")) ) {
				   ...
				   if (flag.equals("publisher") || flag.equals("admin") ) {
						...
						if (action.equals("Edit")) {
						...
						} else if ( (action.equals("ReallyAdd")) || (action.equals("ReallyUpdate")) ) {
							...
							destination = getDestination("quickInfoPublisher", componentSC, request);
						}
					   ...
					   PublicationDetail pubDetail;
						...
						while (parameters.hasMoreElements()) {
							...
							if (paramName.startsWith ("selectItem")) {
								...
								if (Id != null) {
								   pubDetail = ((QuickInfoSessionController) componentSC).getDetail(Id);
						...
						try {
						   userClipboard.setMessageError ("copyError", e);
							// SilverTrace : mettre un warning
                        			   ...
                        			   SilverTrace.error("quickinfo", "QuickInfoRequestRouter.getDestination()", "quickinfo.NO_DATATOCOPY", ee);
					   }
					}
				   ...
				   // appel d'une methode static à ClipboardSessionControler
					ClipboardBm userClipboard = componentSC.getClipboard();
					try {
					   PublicationDetail pubDetail = ((QuickInfoSessionController) componentSC).getDetail(request.getParameter("Id"));
                    ...
                    ClipboardBm userClipboard = componentSC.getClipboard();
                    try {
                        SilverTrace.debug("quickinfo", "QuickInfoRequestRouter.getDestination()", "clipboard '" + userClipboard.getName() + "' count=" + userClipboard.getCount());
                        ...
                        while (clipObjectIterator.hasNext()) {
                           ...
                           if ((clipObject != null) &&
                              (clipObject.isDataFlavorSupported (PublicationSelection.PublicationDetailFlavor))) {
                               PublicationDetail pubDetail;
                               pubDetail = (PublicationDetail) clipObject.getTransferData (PublicationSelection.PublicationDetailFlavor);
                               
                               String description = WysiwygController.load(pubDetail.getPK().getInstanceId(), pubDetail.getPK().getId(), null);
                               
                               ((QuickInfoSessionController) componentSC).add (
                                       pubDetail.getName(),
                                        description,
                                        pubDetail.getBeginDate(),
                        ...
                        SilverTrace.error("quickinfo", "QuickInfoRequestRouter.getDestination()", "quickinfo.PASTE_ERROR", e);
                    }
        ...
        SilverTrace.info("quickinfo","QuickInfoRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE","destination" + destination);
		return destination;
	}
