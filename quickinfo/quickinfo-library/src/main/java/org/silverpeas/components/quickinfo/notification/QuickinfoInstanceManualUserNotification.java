...
public class QuickinfoInstanceManualUserNotification  extends
  ...
  protected boolean check(final NotificationContext context) {
    final String newsId = context.getContributionId();
    final News news = QuickInfoService.get().getNews(newsId);
    context.put(NEWS_KEY, news);
    return news.canBeAccessedBy(context.getSender());
  }

  @Override
  public UserNotification createUserNotification(final NotificationContext context) {
    final News news = context.getObject(NEWS_KEY);
    return new NewsManualUserNotification(news, context.getSender()).build();
  }
}
