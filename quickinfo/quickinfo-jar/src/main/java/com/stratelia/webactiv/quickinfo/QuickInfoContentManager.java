...
public class QuickInfoContentManager implements ContentInterface {
     ...
     */
    public List getSilverContentById(List ids, String peasId,
                                     String userId,
                                     List userRoles) {
    ...
    public int getSilverObjectId(String pubId, String peasId) {
		SilverTrace.info("quickinfo","QuickInfoContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubId);
		try {
			return getContentManager().getSilverContentId(pubId, peasId);
		} catch (Exception e) {
			//throw new KmeliaRuntimeException("KmeliaContentManager.getSilverObjectId()",SilverpeasRuntimeException.ERROR,"kmelia.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
			return 0;
		}
	}
     ...
     */
    public int createSilverContent(Connection con, PublicationDetail pubDetail, String userId, boolean isVisible) throws ContentManagerException {
        SilverTrace.info("quickinfo", "QuickInfoContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", String.valueOf(pubDetail));
        SilverContentVisibility scv = new SilverContentVisibility(pubDetail.getBeginDate(), pubDetail.getEndDate(), isVisible);
        return getContentManager().addSilverContent(con, pubDetail.getPK().getId(), pubDetail.getPK().getComponentName(), userId, scv);
    }
    ...
    public void updateSilverContentVisibility(PublicationDetail pubDetail, boolean isVisible) throws ContentManagerException {
        int silverContentId = getContentManager().getSilverContentId(pubDetail.getPK().getId(), pubDetail.getPK().getComponentName());
		if (silverContentId != -1) {
	        SilverContentVisibility scv = new SilverContentVisibility(pubDetail.getBeginDate(), pubDetail.getEndDate(), isVisible);
			SilverTrace.info("quickinfo", "QuickInfoContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", String.valueOf(scv));
			getContentManager().updateSilverContentVisibilityAttributes(scv, pubDetail.getPK().getComponentName(), silverContentId);
    ...
    public void deleteSilverContent(Connection con, PublicationPK pubPK) throws ContentManagerException {
		...
		if (contentId != -1)
		{
		   SilverTrace.info("quickinfo", "QuickInfoContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "pubId = " + pubPK.getId() + ", contentId = " + contentId);
    ...
    private List getHeaders(List ids) {
        ...
        try {
            ArrayList publicationDetails = (ArrayList) getPublicationBm().getPublications(ids);
    ...
    private PublicationBm getPublicationBm() {
        if (currentPublicationBm == null) {
            try {
                ...
                throw new PublicationRuntimeException("QuickInfoContentManager.getPublicationBm()",PublicationRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
            }
        }
    ...
    private ContentManager getContentManager() {
        if (contentManager == null) {
            try {
                ...
                SilverTrace.fatal("quickinfo", "QuickInfoContentManager.getContentManager()", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
            }
        }
