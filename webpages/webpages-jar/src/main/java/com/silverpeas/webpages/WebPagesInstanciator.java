...
public class WebPagesInstanciator implements ComponentsInstanciatorIntf {
  ...
  private void removeWysiwygContent(String componentId) throws InstanciationException {
    try {
      ...
      throw new InstanciationException("WebPagesInstanciator.delete", SilverpeasException.ERROR,
          "webPages.WYSIWYG_DELETION_FAILED", e);
    }
  }
    ...
    if (StringUtil.isDefined(xmlFormName)) {
      ...
      try {
        PublicationTemplateManager.getInstance()
            .removePublicationTemplate(componentId + ":" + xmlShortName);
