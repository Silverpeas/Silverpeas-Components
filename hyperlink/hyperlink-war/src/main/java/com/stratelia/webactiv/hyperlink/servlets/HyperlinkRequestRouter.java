...
public class HyperlinkRequestRouter extends ComponentRequestRouter {
	...
	public ComponentSessionController createComponentSessionController(MainSessionController mainSessionCtrl, ComponentContext context) {
		ComponentSessionController component = (ComponentSessionController) new HyperlinkSessionController(mainSessionCtrl,	context);
		return component;
	}
	...
	public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request) {
		...
		SilverTrace.info("hyperlink", "HyperlinkRequestRooter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "function = " + function);
		...
		else if (function.startsWith("GoToURL"))
  		{
			...
			if (urlParameter != null && !urlParameter.equals(""))
			{
				...
				if ("yes".equalsIgnoreCase(sso))
				{
					...
					request.setAttribute("Password", (String)session.getAttribute("Silverpeas_pwdForHyperlink"));
				...
				{
					try {
						destination = this.getParsedDestination(destination, s_sUserLogin, URLEncoder.encode(hyperlinkSCC.getUserDetail().getLogin(), ENCODING));
						destination = this.getParsedDestination(destination, s_sUserEmail, URLEncoder.encode(hyperlinkSCC.getUserDetail().geteMail(), ENCODING));
						destination = this.getParsedDestination(destination, s_sUserFirstName, URLEncoder.encode(hyperlinkSCC.getUserDetail().getFirstName(), ENCODING));
						destination = this.getParsedDestination(destination, s_sUserLastName, URLEncoder.encode(hyperlinkSCC.getUserDetail().getLastName(), ENCODING));
						destination = this.getParsedDestination(destination, s_sUserFullName, URLEncoder.encode(hyperlinkSCC.getUserDetail().getDisplayedName(), ENCODING));
						destination = this.getParsedDestination(destination, s_sUserId, URLEncoder.encode(hyperlinkSCC.getUserId(), ENCODING));
						destination = this.getParsedDestination(destination, s_sSessionId, URLEncoder.encode(request.getSession().getId(), ENCODING));
						
						if ("true".equalsIgnoreCase(hyperlinkSCC.getSettings().getString("PasswordKeyEnable", "true")))
						{
							// !!!! Add the password : this is an uggly patch that use a session variable set in the "AuthenticationServlet" servlet
							HttpSession session = request.getSession(false);
							destination = this.getParsedDestination(destination, s_sUserPassword, URLEncoder.encode((String)session.getAttribute("Silverpeas_pwdForHyperlink"), ENCODING));
							destination = this.getParsedDestination(destination, s_sUserEncodedPassword, URLEncoder.encode(hyperlinkSCC.getUserFull().getPassword(), ENCODING));
						}
						else
						{
							destination = this.getParsedDestination(destination, s_sUserPassword, URLEncoder.encode("??????", ENCODING));
						}
						
						destination = getParsedDestinationWithExtraInfos(destination, hyperlinkSCC);
					} catch (UnsupportedEncodingException e) {
						SilverTrace.error("hyperlink","HyperlinkRequestRooter.getDestination()","root.MSG_GEN_PARAM_VALUE","destination = "+destination);
					}
				}
			}
			else
		...
		SilverTrace.info("hyperlink", "HyperlinkRequestRooter.getDestination()", "root.MSG_GEN_RETURN_VALUE", "destination = " + destination);
		return destination;
	}

	private String getParsedDestination(
		String sDestination,
		String sKeyword,
		String sValue) {
		SilverTrace.info("hyperlink", "HyperlinkRequestRooter.getParsedDestination()", "root.MSG_GEN_PARAM_VALUE", "sDestination = " + sDestination);
		...
		SilverTrace.info("hyperlink", "HyperlinkRequestRooter.getParsedDestination()", "root.MSG_GEN_RETURN_VALUE", "sDestination = " + sDestination);
		return sDestination;
	}
	
	private String getParsedDestinationWithExtraInfos(String sDestination, HyperlinkSessionController hyperlinkSC) throws UnsupportedEncodingException
	{
		int i = sDestination.indexOf(s_sUserPropertyPrefix);
		while (i != -1)
		{
			String keyword = sDestination.substring(i, sDestination.indexOf("%", i+1)+1);
			String property = extractPropertyName(keyword);
			
			sDestination = getParsedDestination(sDestination, keyword, URLEncoder.encode(hyperlinkSC.getUserFull().getValue(property), ENCODING));
