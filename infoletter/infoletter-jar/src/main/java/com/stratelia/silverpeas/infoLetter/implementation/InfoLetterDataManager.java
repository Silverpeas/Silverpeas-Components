...
public class InfoLetterDataManager implements InfoLetterDataInterface
{
	...
	public InfoLetterDataManager() {
		try {
			infoLetterDAO = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.infoLetter.model.InfoLetter");
			infoLetterPublicationDAO = SilverpeasBeanDAOFactory.getDAO("com.stratelia.silverpeas.infoLetter.model.InfoLetterPublication");
			infoLetterContentManager = new InfoLetterContentManager();
		}
		catch (PersistenceException pe) {
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
	...
	public Vector getInfoLetters(String instanceId) {
		...
		try {
			...
			retour = new Vector(infoLetterDAO.findByWhereClause(new IdPK(), whereClause));
		}
		catch (PersistenceException pe) {
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
				SilverpeasRuntimeException.FATAL, pe.getMessage());
	...
	public Vector getInfoLetterPublications(WAPrimaryKey letterPK) {
		...
		try {
			...
			String whereClause = "instanceId = '" + letter.getInstanceId() + "' and letterId = " + letterPK.getId();
			retour = new Vector(infoLetterPublicationDAO.findByWhereClause(letterPK, whereClause));
		}
		catch (PersistenceException pe) {
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
	...
	public void createInfoLetterPublication(InfoLetterPublicationPdC ilp, String userId) {
		SilverTrace.info("infoLetter","InfoLetterDataManager.createInfoLetterPublication()", "root.MSG_GEN_ENTER_METHOD", "ilp = "+ilp.toString()+" userId="+userId);
		...
		try {
			...
			try
			{
				...
				SilverTrace.error("infoLetter", "InfoLetterDataManager.createInfoLetterPublication()", "root.EX_ERR_ROLLBACK", e);
			}
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
					SilverpeasRuntimeException.FATAL, pe.getMessage(),pe);
		}
		finally
		{
			try
			{
				...
				SilverTrace.error("infoLetter", "InfoLetterDataManager.createInfoLetterPublication()", "root.EX_CONNECTION_CLOSE_FAILED", e);
			}
		}
	}
	...
	public void deleteInfoLetterPublication(WAPrimaryKey pk, String componentId) {
		...
		try {
			...
			try
			{
				...
				SilverTrace.error("infoLetter", "InfoLetterDataManager.deleteInfoLetterPublication()", "root.EX_ERR_ROLLBACK", e);
			}
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
					SilverpeasRuntimeException.FATAL, pe.getMessage());
		}
	...
	public InfoLetterPublicationPdC getInfoLetterPublication(WAPrimaryKey publiPK) {
		...
		try {
			retour = new InfoLetterPublicationPdC( (InfoLetterPublication) infoLetterPublicationDAO.findByPrimaryKey(publiPK) );
		}
		catch (PersistenceException pe) {
			throw new InfoLetterException("com.stratelia.silverpeas.infoLetter.implementation.InfoLetterDataManager", 
				SilverpeasRuntimeException.FATAL, pe.getMessage());
	...
	public int getSilverObjectId(String pubId, String componentId) {
		SilverTrace.info("infoLetter","InfoLetterDataManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubId);
		...
		try {
			...
			if (silverObjectId == -1) {
				...
				silverObjectId	= infoLetterContentManager.createSilverContent(null, infoLetter, infoLetter.getCreatorId());
			}
		} catch (Exception e) {
			throw new InfoLetterException("InfoLetterDataManager.getSilverObjectId()",SilverpeasRuntimeException.ERROR,"kmelia.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
		}
	...
	public Vector getInternalSuscribers(WAPrimaryKey letterPK) {
		...
		try {
			...
			SilverTrace.info("infoLetter", "InfoLetterDataManager.getInternalSuscribers()", "root.MSG_GEN_PARAM_VALUE",
			...
			while (rs.next()) {
				...
				try {
					if (type.equalsIgnoreCase("U")) users.add(ad.getUserDetail(id));
					else groups.add(ad.getGroup(id));
				}
				catch (AdminException ae) {
					SilverTrace.error("infoLetter", "InfoLetterDataManager.getInternalSuscribers()", "root.MSG_GEN_PARAM_VALUE",
			             "id = " + id);
				}
			}
			// rs.close();
	...
	public void setInternalSuscribers(WAPrimaryKey letterPK, Vector abonnes) {
		...
		try {
			...
			SilverTrace.info("infoLetter", "InfoLetterDataManager.setInternalSuscribers()", "root.MSG_GEN_PARAM_VALUE",
			             "query = " + query);
			...
			if (t_groups.length>0) {
				for (i=0; i<t_groups.length; i++) {
					...
					query += " values (" + letterPK.getId() + ", 'G" + t_groups[i].getId() + "', ";
	...
	public Vector getExternalsSuscribers(WAPrimaryKey letterPK) {
		...
		try {
			...
			selectQuery +=	" where instanceId = '" + letter.getInstanceId() + "' ";
			selectQuery +=	" and letter = " + letterPK.getId() + " ";
			SilverTrace.info("infoLetter", "InfoLetterDataManager.getExternalsSuscribers()", "root.MSG_GEN_PARAM_VALUE",
			             "selectQuery = " + selectQuery);
			selectStmt = con.createStatement();
			rs = selectStmt.executeQuery(selectQuery);
			while (rs.next()) retour.add(rs.getString("email"));
			// rs.close();
			// selectStmt.close();
			// con.close();
		}
	...
	public void setExternalsSuscribers(WAPrimaryKey letterPK, Vector emails) {
		...
		try {
			...
			query +=	" where instanceId = '" + letter.getInstanceId() + "' ";
			query +=	" and letter = " + letterPK.getId() + " ";
			SilverTrace.info("infoLetter", "InfoLetterDataManager.setExternalsSuscribers()", "root.MSG_GEN_PARAM_VALUE",
			             "query = " + query);
			stmt = con.createStatement();
			stmt.executeUpdate(query);
			if (emails.size()>0) {
				for (int i=0; i<emails.size(); i++) {
					...
					query += " values (" + letterPK.getId() + ", '" + email + "', '" + letter.getInstanceId() + "')";
					stmt = con.createStatement();
					stmt.executeUpdate(query);
				}
			}
	...
	public void toggleSuscriber(String userId, WAPrimaryKey letterPK, boolean flag) {
		...
		try {
			...
			SilverTrace.info("infoLetter", "InfoLetterDataManager.toggleSuscriber()", "root.MSG_GEN_PARAM_VALUE",
			             "query = " + query);
			stmt = con.createStatement();
			stmt.executeUpdate(query);
			if (flag) {
	...
	public boolean isSuscriber(String userId, WAPrimaryKey letterPK) {
		...
		try {
			...
			query +=	" and letter = " + letterPK.getId() + " ";
			query +=	" and userId = 'U" + userId + "' ";
			SilverTrace.info("infoLetter", "InfoLetterDataManager.isSuscriber()", "root.MSG_GEN_PARAM_VALUE",
			             "query = " + query);
			stmt = con.createStatement();
	...
	public void initTemplate(String spaceId, String componentId, WAPrimaryKey letterPK) {
		try {
			...
			WysiwygController.createFileAndAttachment(basicTemplate, spaceId, componentId, 
					InfoLetterPublication.TEMPLATE_ID + letterPK.getId());
		} catch (Exception e) {
