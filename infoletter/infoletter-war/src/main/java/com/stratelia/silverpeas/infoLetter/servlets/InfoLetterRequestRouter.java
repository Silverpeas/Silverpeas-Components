...
public class InfoLetterRequestRouter extends ComponentRequestRouter
{
     ...
     * @see
     */
    private void setGlobalInfo(InfoLetterSessionController scc, HttpServletRequest request)
    {
        ...
        {
            scc.getSpaceLabel(), scc.getComponentLabel(), scc.getSpaceId(), scc.getComponentId(), scc.getComponentUrl()
        });
    }
     ...
     * @see
     */
    private String setMainContext(InfoLetterSessionController scc, HttpServletRequest request)
    {
        ...
        if ( (scc.getSettings().getString("showHeader")!=null) && (scc.getSettings().getString("showHeader").equals("false")) )
        	showHeader = false;
        ...
        request.setAttribute("userIsSuscriber", String.valueOf(scc.isSuscriber(defaultLetter.getPK())));
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("infoLetter", "infoLetterRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "User=" + infoLetterSC.getUserId() + " Function=" + function);
        ...
        try
        {
            ...
            else if (function.startsWith("Preview"))
            {
                ...
                if (!parution.equals(""))
                {
					...
					request.setAttribute("ReturnUrl", m_context + "/RinfoLetter/" + scc.getComponentId() + "/ParutionHeaders?parution=" + parution);
            ...
            else if (function.startsWith("FilesEdit"))
            {
                ...
                if (!parution.equals(""))
                {
                    ...
                    request.setAttribute("Language", scc.getLanguage());
					//NEWD DLE               
                    //request.setAttribute("ReturnUrl", m_context + "/RinfoLetter/" + scc.getSpaceId() + "_" + scc.getComponentId() + "/ParutionHeaders?parution=" + parution);
					request.setAttribute("ReturnUrl", m_context + "/RinfoLetter/" + scc.getComponentId() + "/ParutionHeaders?parution=" + parution);
					//NEWF DLE               
            ...
            else if (function.startsWith("ValidateParution"))
            {
                ...
                if (!parution.equals(""))
                {
            		...
            		String server = request.getRequestURL().substring(0,request.getRequestURL().toString().indexOf(URLManager.getApplicationURL()));
			...
			else if ( function.startsWith("pdcPositions.jsp") )
			{
                ...
                {
                    String m_context = GeneralPropertiesManager.getGeneralResourceLocator().getString("ApplicationURL");
					String url = "/RinfoLetter/" + scc.getComponentId() + "/FilesEdit?parution=" + parution;
                    IdPK   publiPK = new IdPK();
				...
				request.setAttribute("silverObjectId", String.valueOf(scc.getCurrentSilverObjectId()));
            ...
            else if (function.startsWith("ExportEmailsCsv"))
            {
                ...
                if (exportOk)
                	request.setAttribute("EmailCsvName", getCurrentLetter(infoLetterSC).getName() + InfoLetterSessionController.EXPORT_CSV_NAME);
            ...
            else if (function.startsWith("SendLetterToManager"))
            {
                ...
                request.setAttribute("ComponentName", scc.getComponentLabel());
                request.setAttribute("EmailErrors", emailErrors);
                request.setAttribute("ReturnUrl", request.getParameter("ReturnUrl") + "?parution="+parution);
        ...
        SilverTrace.info("infoLetter", "infoLetterRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
