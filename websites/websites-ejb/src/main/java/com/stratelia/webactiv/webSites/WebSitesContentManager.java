...
public class WebSitesContentManager implements java.io.Serializable, ContentInterface
{
   ...
   public int getSilverObjectId(String pubId, String peasId) {
		SilverTrace.info("webSites","WebSitesContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubId);
		try {
			return getContentManager().getSilverContentId(pubId, peasId);
		} catch (Exception e) {
			throw new WebSitesRuntimeException("WebSitesContentManager.getSilverObjectId()",SilverpeasRuntimeException.ERROR,"webSites.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
		}
	}
	...
	*/
   public int createSilverContent(Connection con, SiteDetail siteDetail, String userId, String prefixTableName, String componentId) throws ContentManagerException, RemoteException
   {
	  SilverContentVisibility scv = new SilverContentVisibility(isVisible(siteDetail));
	  SilverTrace.info("webSites","WebSitesContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  SilverTrace.info("webSites","WebSitesContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "siteDetail = "+siteDetail.toString());
	  return getContentManager().addSilverContent(con, siteDetail.getSitePK().getId(), componentId, userId, scv);
   }
	...
	*/
   public void updateSilverContentVisibility(SiteDetail siteDetail, String prefixTableName, String componentId) throws ContentManagerException, RemoteException
   {
	  int silverContentId = getContentManager().getSilverContentId(siteDetail.getSitePK().getId(),componentId);
	  SilverContentVisibility scv = new SilverContentVisibility(isVisible(siteDetail));
	  SilverTrace.info("webSites","WebSitesContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	...
	*/
   public void deleteSilverContent(Connection con, SitePK sitePK, String prefixTableName, String componentId) throws ContentManagerException, RemoteException
   {
	  int contentId = getContentManager().getSilverContentId(sitePK.getId(), componentId);
	  if (contentId != -1)
	  {
		  SilverTrace.info("webSites","WebSitesContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "siteId = "+sitePK.getId()+", contentId = "+contentId);
		  getContentManager().removeSilverContent(con, contentId, componentId);
	  }
   }

   private boolean isVisible(SiteDetail siteDetail) throws ContentManagerException
   {
	...
	*/
	...
	private List getHeaders(List siteIds, String peasId)
	{
		...
		SilverTrace.info("webSites","WebSitesContentManager.getHeaders()", "root.MSG_GEN_ENTER_METHOD", "siteIds = "+siteIds.toString());
		try
		{
			ArrayList siteDetails = (ArrayList) getWebSiteBm(peasId).getWebSites(siteIds);
			for (int i=0 ; i<siteDetails.size() ; i++)
			{
				...
				//rustine : the getDate() of the SilverContentInterface have to return a date like yyyy/MM/dd
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("webSites", "WebSitesContentManager", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
	...
	private WebSiteBm getWebSiteBm(String componentId) {
		try {
			WebSiteBmHome webSiteBmHome = (WebSiteBmHome) EJBUtilitaire.getEJBObjectRef(JNDINames.WEBSITESBM_EJBHOME, WebSiteBmHome.class);
			currentWebSiteBm = webSiteBmHome.create();
			currentWebSiteBm.setComponentId(componentId);
		} catch (Exception e) {
			throw new WebSitesRuntimeException("WebSitesContentManager.getWebSiteBm()",SilverpeasRuntimeException.ERROR, "webSites.EX_IMPOSSIBLE_DE_FABRIQUER_BOOKMARKBM_HOME", e);
		}
