      ...
      if (action.equals("Search")) {
        ...
        if ("Admin".equals(bestRole) && scc.isPdcUsed()) {
          ...
          operationPane.addOperation(pdcUtilizationSrc, resources.getString("GML.PDCParam"),
              "javascript:onClick=openSPWindow('" + m_context + "/RpdcUtilization/jsp/Main?ComponentId="
              + scc.getComponentId() + "','utilizationPdc1')");
        ...
        while (i.hasNext()) {
          ...
          while (j.hasNext()) {
            ...
            Link l = new Link(subtheme.getName(), "listSite.jsp?Action=Search&Id=" + subtheme.
                getNodePK().getId());
          ...
          //Ajout d'une ligne
          navList.addItemSubItem(themeName, "listSite.jsp?Action=Search&Id=" + themeId, new Integer(
              nbPub).intValue(), listSubDirectory);
        }
        ...
        if (listeSites.size() > 0) {
          ...
          while (j.hasNext()) {
            ...
            liste +=
                ...
                + siteDetail.getPopup() + "', '" + type + "', 'http://" + getMachine(request) + "/"
                + settings.getString("Context") + "/" + componentId + "/" + siteId + "/' , '"
                + EncodeHelper.javaStringToJsString(sitePage) + "')\">" + siteName
                + "</a></td><td align=left>\n";
