...
//Mise a jour de l'espace
  if (action == null) {
    ...
    SilverTrace.info("websites", "JSPorganize", "root.MSG_GEN_PARAM_VALUE", "action = " + action);
  }
    ...
    <form name="liste" action="organize.jsp" method="post">
      <%
        ...
        if (action.equals("Search")) {
          ...
          operationPane.addOperationOfCreation(addFolder, resources.getString("FolderAdd"),
              "javascript:onClick=topicAdd('" + id + "')");
          operationPane.addOperationOfCreation(addSite, resources.getString("AjouterSitesTheme"),
              "javascript:onClick=publicationAdd('" + id + "')");
          operationPane.addOperation(belpou, resources.getString("DeclasserSitesSupprimerThemes"),
              "javascript:onClick=declassify('" + subThemes.size() + "','" + listeSites.size()
              + "');");
      ...
      <view:frame>
        ...
        <%
          //Le tableau de tri
          ArrayPane arrayPane = gef.getArrayPane("foldersList", "organize.jsp?Action=Search&Id="
              + id,
              request, session);
          ...
          if (scc.isSortedTopicsEnabled()) {
            ArrayColumn arrayColumnOrder = arrayPane.
                addArrayColumn(resources.getString("GML.order"));
          ...
          if (subThemes != null) {
            ...
            while (i.hasNext()) {
              ...
              arrayLine.addArrayCellLink(Encode.javaStringToHtmlString(themeName),
                  "organize.jsp?Action=Search&Id=" + themeId);
              ...
              checkIcon1.setProperties(folderUpdate, resources.getString("GML.modify") + " '"
                  + Encode.
                  javaStringToHtmlString(themeName) + "'", "javascript:onClick=topicUpdate('"
                  + themeId
                  + "')");
              ...
              if (scc.isSortedTopicsEnabled()) {
                ...
                if (nbChild != 0) {
                  ...
                  upIcon.setProperties(upIconSrc, resources.getString("TopicUp") + " '" + themeName
                      + "'",
                      "javascript:onClick=topicUp('" + themeId + "')");
                } else {
                ...
                if (nbChild < subThemes.size() - 1) {
                  ...
                  downIcon.setProperties(downIconSrc, resources.getString("TopicDown") + " '"
                      + themeName
                      + "'", "javascript:onClick=topicDown('" + themeId + "')");
                } else {
              ...
              arrayLine.addArrayCellText("<input type=\"checkbox\" name=\"checkbox\" value=\""
                  + themeId
                  + "\"/>");
          ...
          if (listeSites.size() > 0) {
            ...
            while (j.hasNext()) {
              ...
              String siteDescription = EncodeHelper.
                  javaStringToHtmlParagraphe(site.getDescription());
              ...
              liste += "<tr>\n";
              liste +=
                  "<td valign=\"top\" width=\"5%\"><input type=\"checkbox\" name=\"supSite\" value=\""
                  + pubId + "\"/></td>\n";
              if (scc.isSortedTopicsEnabled()) {
                IconPane sortPane = gef.getIconPane();
                if (nbChild != 0) {
                  Icon upIcon = sortPane.addIcon();
                  upIcon.setProperties(upIconSrc, resources.getString("PubUp") + " '" + siteName
                      + "'",
                      "javascript:onClick=pubUp('" + pubId + "')");
                } else {
                  Icon upIcon = sortPane.addEmptyIcon();
                }
                ...
                if (nbChild < listeSites.size() - 1) {
                  Icon downIcon = sortPane.addIcon();
                  downIcon.
                      setProperties(downIconSrc, resources.getString("PubDown") + " '" + siteName
                      + "'",
                      "javascript:onClick=pubDown('" + pubId + "')");
                } else {
                  Icon downIcon = sortPane.addEmptyIcon();
                }
              ...
              liste +=
                  "<td valign=\"top\">&#149;&nbsp;<a class=\"textePetitBold\" href=\"javascript:onClick=publicationGoTo('"
                  + type + "', 'http://" + getMachine(request) + "/" + settings.getString("Context")
                  + "/"
                  + componentId + "/" + siteId + "/' , '" + Encode.javaStringToJsString(sitePage)
                  + "')\">"
                  + siteName + "</a><br/>\n";

              liste += "<span class=\"txtnote\">&nbsp;&nbsp;" + siteDescription
                  + "</span><br/><br/></td>\n";
              liste += "</tr>\n";
