...
<%!  /**
  ...
  private boolean appartient(String siteId, Collection liste) {
    SilverTrace.info("websites", "JSPlisteSite_reader.appartient()", "root.MSG_GEN_PARAM_VALUE",
        "id = " + siteId);
  ...
  private boolean iconAppartient(IconDetail iconDetail, Collection c) {
    SilverTrace.info("websites", "JSPlisteSite_reader.iconAppartient()", "root.MSG_GEN_PARAM_VALUE",
        "appartient");
    boolean ok = false;
    ...
    while (i.hasNext() && !ok) {
      ...
      SilverTrace.info("websites", "JSPlisteSite_reader.iconAppartient()",
          "root.MSG_GEN_PARAM_VALUE", "id= " + id);
      if (theId.equals(id)) {
        ok = true;
      }
      SilverTrace.info("websites", "JSPlisteSite_reader.iconAppartient()",
          "root.MSG_GEN_PARAM_VALUE", "ok= " + ok);
    }
    return ok;
  }
...
<HTML>
  <HEAD>
    ...
    <script Language="JavaScript">
      ...
      if ((listeSites != null) && (listeSites.size() == 1)) {
        ...
        out.println("publicationGoTo('" + siteDetail.getPopup() + "', '" + siteDetail.getType()
  ...
  <%
    ...
    SilverTrace.info("webSites", "JSPlisteSite_reader", "root.MSG_GEN_PARAM_VALUE", "action = "
        + action);
    ...
    if (action.equals("Search")) {
      ...
      while (i.hasNext()) {
        ...
        while (j.hasNext()) {
          ...
          Link l = new Link(subtheme.getName(), "listSite_reader.jsp?Action=Search&Id=" + theme.
              getNodePK().getId());
        ...
        //Ajout d'une ligne
        navList.addItemSubItem(themeName, "listSite_reader.jsp?Action=Search&Id=" + themeId,
            new Integer(nbPub).intValue(), listSubDirectory);
      }
      ...
      if (listeSites.size() > 0) {
        ...
        while (j.hasNext()) {
          ...
          while (k.hasNext()) {
            ...
            if (iconAppartient(icon, icones)) {
              if (icon.getName().equals("Icon0")) {
                ...
                listeIcones += "<A href=\"#\" onclick=\"openDictionnary()\"><img src=\"" + icon.
                    getAddress() + "\" alt=\"" + resources.getString(icon.getName())
                    + "\" border=0 align=absmiddle title=\"" + resources.getString(icon.getName())
                    + "\"></A>&nbsp;\n";
              }
            }
          }
          ...
          if (rouge) {
            liste += "<td valign=\"top\"><img src=\"" + redFlag
        ...
        /*liste += "<td valign=\"top\" align=left nowrap>&#149;&nbsp;<a class=\"textePetitBold\" href=\"javascript:onClick=publicationGoTo('" + siteDetail.getPopup() + "', '" + siteDetail.getType() + "', 'http://"+getMachine(request)+"/"+settings.getString("Context")+"/"+scc.getComponentId()+"/"+siteId+"/' , '"+Encode.javaStringToJsString(sitePage)+"')\">"+siteName+"</a></td><td align=left>\n";*/
          liste +=
              "<td valign=\"top\" align=left nowrap>&#149;&nbsp;<a class=\"textePetitBold\" href=\"javascript:onClick=publicationGoTo('"
              + siteDetail.getPopup() + "', '" + siteDetail.getType() + "', 'http://" + getMachine(
              request) + "/" + settings.getString("Context") + "/" + componentId + "/" + siteId
              + "/' , '" + Encode.javaStringToJsString(sitePage) + "')\">" + siteName
              + "</a></td><td align=left>\n";
          liste += listeIcones;
          liste +=
              "</td></tr><tr><td class=intfdcolor51>&nbsp;</td><td colspan=2 width=\"100%\" class=intfdcolor51><span class=\"txtnote\">"
              + siteDescription + "</span></td></tr><tr><td colspan=3><img src=\"" + pxmag
              + "\" height=3 width=200></td>\n";
        }
        liste += "</td></tr></table>\n";
      } else {
        liste =
            "<TABLE CELLPADDING=0 CELLSPACING=0 ALIGN=CENTER BORDER=0 WIDTH=\"98%\" class=intfdcolor4><tr><td><table border=0 cellspacing=0 cellpadding=5  WIDTH=\"100%\" class=contourintfdcolor><tr><td><BR><center>"
            + resources.getString("NoLinkAvailable")
