...
public class SurveyContentManager implements ContentInterface {
   ...
   */
  /*
   * public int createSilverContent(Connection con, QuestionContainerHeader qC, String userId)
   * throws ContentManagerException { SilverContentVisibility scv = new
   * SilverContentVisibility(isVisible(qC));
   * SilverTrace.info("survey","SurveyContentManager.createSilverContent()",
   * "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString()); return
   * getContentManager().addSilverContent(con, qC.getPK().getId(), qC.getPK().getComponentName(),
   * userId, scv); }
   */
   ...
   */
  /*
   * public void updateSilverContentVisibility(QuestionContainerHeader qC, int silverObjectId)
   * throws ContentManagerException { SilverContentVisibility scv = new
   * SilverContentVisibility(isVisible(qC)); SilverTrace.info("survey",
   * "SurveyContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD",
   * "SilverContentVisibility = "+scv.toString());
   * getContentManager().updateSilverContentVisibilityAttributes(scv, qC.getPK().getComponentName(),
   * silverObjectId); ClassifyEngine.clearCache(); }
   */
   ...
   */
  /*
   * public void deleteSilverContent(Connection con, QuestionContainerPK pk) throws
   * ContentManagerException { int contentId = getContentManager().getSilverContentId(pk.getId(),
   * pk.getComponentName()); SilverTrace.info("survey","SurveyContentManager.deleteSilverContent()",
