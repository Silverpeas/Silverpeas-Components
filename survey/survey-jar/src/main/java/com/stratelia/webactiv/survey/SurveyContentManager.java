...
public class SurveyContentManager implements ContentInterface
{
	...
	*/
   /*public int createSilverContent(Connection con, QuestionContainerHeader qC, String userId) throws ContentManagerException
   {
	  SilverContentVisibility scv = new SilverContentVisibility(isVisible(qC));
	  SilverTrace.info("survey","SurveyContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	...
	*/
   /*public void updateSilverContentVisibility(QuestionContainerHeader qC, int silverObjectId) throws ContentManagerException
   {
	  SilverContentVisibility scv = new SilverContentVisibility(isVisible(qC));
	  SilverTrace.info("survey","SurveyContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  getContentManager().updateSilverContentVisibilityAttributes(scv, qC.getPK().getComponentName(), silverObjectId);
	...
	*/
   /*public void deleteSilverContent(Connection con, QuestionContainerPK pk) throws ContentManagerException
   {
	  int contentId = getContentManager().getSilverContentId(pk.getId(), pk.getComponentName());
	  SilverTrace.info("survey","SurveyContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "id = "+pk.getId()+", contentId = "+contentId);
	  getContentManager().removeSilverContent(con, contentId, pk.getComponentName());
   }*/

	//this content is always visible
   /*private boolean isVisible(QuestionContainerHeader qC) 
   {	
		return true;
   }*/
	...
	*/
	private List getHeaders(List pks)
	{
		...
		try
		{
			ArrayList questionContainerHeaders = (ArrayList) getQuestionContainerBm().getQuestionContainerHeaders((ArrayList) pks);
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("survey", "SurveyContentManager.getContentManager()", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
	...
	private QuestionContainerBm getQuestionContainerBm() {
		if (currentQuestionContainerBm == null) {
			try {
				...
				SilverTrace.fatal("survey", "SurveyContentManager.getQuestionContainerBm()", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
