...
public class GoToSurvey extends GoTo
{
    public String getDestination(String objectId, HttpServletRequest req, HttpServletResponse res) throws Exception
	{
    	...
    	QuestionContainerHeader survey = getQuestionContainerBm().getQuestionContainerHeader(questionContainerPK);
    	
    	if (survey != null)
    	{
    		String 	componentId = survey.getInstanceId();
            
            SilverTrace.info("survey", "GoToSurvey.getDestination", "root.MSG_GEN_PARAM_VALUE", "survey = "+survey.getId()+"componentId = "+componentId);
