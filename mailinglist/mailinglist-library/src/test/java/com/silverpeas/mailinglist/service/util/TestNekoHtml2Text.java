...
public class TestNekoHtml2Text {
  ...
  public void testParseBigContent() throws Exception {
    Reader reader = new InputStreamReader(TestNekoHtml2Text.class
        .getResourceAsStream("lemonde.html"), CharEncoding.UTF_8);
    parser.parse(reader);
    String summary = parser.getSummary();
    assertThat(summary, is(notNullValue()));
    assertThat(summary, is("Politique Recherchez depuis sur Le Monde.fr A la Une "
        + "Le Desk Vidéos International *Elections américaines Europe "
        + "Politique *Municipales & Cantonales 2008 Société Carnet "
        + "Economie Médias Météo Rendez-vou"));

  }

  @Test
  public void testParseInraContent() throws Exception {
    Reader reader = new InputStreamReader(TestNekoHtml2Text.class.getResourceAsStream(
        "mailInra.html"), CharEncoding.UTF_8);
    parser.parse(reader);
    String summary = parser.getSummary();
    assertThat(summary, is(notNullValue()));
