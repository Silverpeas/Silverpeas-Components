...
public class QuestionReplySessionController extends AbstractComponentSessionController
{
	...
	*/
	...
	public Collection getQuestionsByCategory(String categoryId) throws QuestionReplyException
	{
		Collection questions = getQuestionManager().getAllQuestionsByCategory(getComponentId(), categoryId);
		return questions;
	}
	
	public Collection getAllQuestions() throws QuestionReplyException
	{
		Collection questions = getQuestionManager().getAllQuestions(getComponentId());
		return questions;
	}
	/*
	*	Recupère la question et ses réponses selon le profil de l'utilisateur courant, ainsi que ses destinataires
	*	met la question en session
	*/
	...
	*/
	...
	/*
	* Retourne une nouvelle question (instanceId, creatorId, creationDate)
	* met la question en session : newQuestion
	*/
	...
	*/
	public Reply getNewReply()
	{
		...
		else reply = new Reply(getUserId());
		newReply = reply;
		return newReply;
	}
	/*
	...
	*/
	...
	/*
	* enregistre la nouvelle réponse de la question courante
	* met en session la question modifiée
	*/
	...
	*/
	...
	/*
	* Modifie la réponse courante
	* => supprime la réponse publique => deletePublicReplies()
	* => crée une nouvelle réponse publique et privée
	* met à jour en session la question courante
	*/
	...
	*/
	public void deleteQuestions(Collection questionsIds) throws QuestionReplyException
	{
		try
		{
			getQuestionManager().deleteQuestionAndReplies(questionsIds);
			/*if (userProfil.equals("publisher"))
				deletePrivateQuestions(questionsIds);
			else if ((userProfil.equals("writer")) || (userProfil.equals("admin")))
				deletePublicQuestions(questionsIds);*/
		}
		catch( QuestionReplyException e )
		{
			throw new QuestionReplyException("QuestionReplySessionController.deleteQuestions",SilverpeasException.ERROR,"questionReply.EX_DELETE_QUESTION_FAILED","",e);
		}
	}
	/*
	* Supprime une liste de reponses selon le profil de l'utilisateur courant
	* i.e. suppression des réponses publiques ou privées
	* si ReplyNumber =0 et que la question est close, la question sera supprimée => reSetCurrentQuestion
	* appel de deletePublicReplies ou deletePrivateReplies
	* si le nombre de R publiques ou privées restantes est egal à 0 et que la question est close, la question n'est plus visible => reSetCurrentQuestion
	* sinon met en session la question
	*/
	public void deleteReplies(Collection replyIds) throws QuestionReplyException
	{
		try
		{
			...
			if ((((getCurrentQuestion().getReplyNumber()) == 0)||(rest == 0)) && (getCurrentQuestion().getStatus() == 2))
				reSetCurrentQuestion();
			else getQuestion(((IdPK)getCurrentQuestion().getPK()).getIdAsLong());
		}
		catch( QuestionReplyException e )
		{
			throw new QuestionReplyException("QuestionReplySessionController.deleteReplies",SilverpeasException.ERROR,"questionReply.EX_DELETE_REPLY_FAILED","",e);
		}
	}
	...
	*/
	...
	/*
	* Clos une question
	* si replyNumber = 0, la question sera supprimée => reSetCurrentQuestion
	* sinon met en session la question
	*/
	public void closeQuestion(long questionId) throws QuestionReplyException
	{
		...
		getQuestionManager().closeQuestions(questionIds);
		/*if ((getCurrentQuestion().getReplyNumber() == 0)||(getCurrentQuestion().getPublicReplyNumber() == 0))
			reSetCurrentQuestion();
		else
	...
	* => getQuestionManager().updateRepliesPublicStatus()
	* retourne le nombre de réponses publiques restantes
	*/
	...
	* => getQuestionManager().updateRepliesPrivateStatus()
	* retourne le nombre de réponses privées restantes
	*/
	...
	/*
	* Retourne la liste des questions de l'utilisateur de rôle User
	* i.e. liste des questions avec réponses publiques
	...
	*/
	...
	/*
	* Retourne la liste des questions de l'utilisateur de rôle Publisher (demandeur)
	* i.e. liste des questions dont il est l'auteur non close ou close avec réponses privées
	...
	*/
	private Collection getCurrentQuestionPublicReplies() throws QuestionReplyException
	{
		return getQuestionManager().getQuestionPublicReplies(((IdPK)getCurrentQuestion().getPK()).getIdAsLong());
	}
	/*
	* liste les réponses privées d'une question
	*/
	private Collection getCurrentQuestionPrivateReplies() throws QuestionReplyException
	{
		return getQuestionManager().getQuestionPrivateReplies(((IdPK)getCurrentQuestion().getPK()).getIdAsLong());
	}
	/*
	...
	*/
	...
	{
		return getQuestionManager().getQuestionReplies(((IdPK)getCurrentQuestion().getPK()).getIdAsLong());
	}
	...
	*/
    public ContainerPositionInterface getSilverContentIdPosition() throws QuestionReplyException {
		...
		try {
	    	 ...
	    	 position = (ContainerPositionInterface) containerContext.getSilverContentIdSearchContext(new Integer(getCurrentQuestionContentId()).intValue(), getComponentId());
	   	}
    	catch (Exception e) {
    		throw new QuestionReplyException("QuestionReplySessionController.getCurrentQuestionWriters()",SilverpeasException.ERROR,"questionReply.EX_CANT_GET_EXPERTS","",e);
    	}
    ...
    public String genericWriters() throws QuestionReplyException
    {
		...
		//NEWD DLE
        //String theURL = context + "/RquestionReply/"+getSpaceId()+"_"+getComponentId()+"/EffectiveRelaunch";
        //String cancelURL = context + "/RquestionReply/"+getSpaceId()+"_"+getComponentId()+"/ConsultQuestionQuery?questionId="+((IdPK)getCurrentQuestion().getPK()).getId();
		//PairObject hostComponentName = new PairObject(getComponentLabel(),  context + "/RquestionReply/"+getSpaceId()+"_"+getComponentId()+"/Main");
		//PairObject hostPath1 = new PairObject(getCurrentQuestion().getTitle(), "/RquestionReply/"+getSpaceId()+"_"+getComponentId()+"/ConsultQuestionQuery?questionId="+((IdPK)getCurrentQuestion().getPK()).getId());
		String theURL = context + "/RquestionReply/"+getComponentId()+"/EffectiveRelaunch";
		String cancelURL = context + "/RquestionReply/"+getComponentId()+"/ConsultQuestionQuery?questionId="+((IdPK)getCurrentQuestion().getPK()).getId();
		PairObject hostComponentName = new PairObject(getComponentLabel(),  context + "/RquestionReply/"+getComponentId()+"/Main");
		PairObject hostPath1 = new PairObject(getCurrentQuestion().getTitle(), "/RquestionReply/"+getComponentId()+"/ConsultQuestionQuery?questionId="+((IdPK)getCurrentQuestion().getPK()).getId());
		//NEWF DLE
	...
	*/
	public void relaunchRecipients() throws QuestionReplyException
	{
		...
		if (uids != null)
		{
            ...
            {
				Recipient recipient = new Recipient(((IdPK)getCurrentQuestion().getPK()).getIdAsLong(), uids[i]);
				recipients.add(recipient);
            }
		}
		getCurrentQuestion().writeRecipients(recipients);
	...
	*/
    public Collection getCurrentQuestionWriters() throws QuestionReplyException {
		...
		try {
	    	  ...
	    	  //recupere la liste de tous les experts du domaine de classement de la question
	...
	*/
    public Collection getCurrentQuestionAvailableWriters() throws QuestionReplyException {
	...
	*
	*/
   private void notify(String intro, String content, UserDetail[] users) throws QuestionReplyException
   {
   		try {
			...
			NotificationMetaData notifMetaData = new NotificationMetaData(NotificationParameters.NORMAL, subject, message);
			...
			getNotificationSender().notifyUser(notifMetaData);
	   			...
	   			throw new QuestionReplyException("QuestionReplySessionController.notify()",SilverpeasException.ERROR,"questionReply.EX_NOTIFICATION_MANAGER_FAILED","",e);
	   	}
   }
   ...
   {
			String message = getString("questionReply.Question") + getCurrentQuestion().getTitle() + "\n"+ getCurrentQuestion().getContent() + "\n\n" +
					getString("questionReply.Reply") +  reply.getTitle() + "\n"+ reply.getContent() + "\n";

			UserDetail user = getOrganizationController().getUserDetail(getCurrentQuestion().getCreatorId());
	...
	*
	*/
	public QuestionReplySessionController(MainSessionController mainSessionCtrl, ComponentContext context, String multilangBaseName, String iconBaseName)
	{
      ...
      if (currentQuestion != null)
		{
		   try
		   {
            ...
            contentId = "" + contentManager.getSilverContentId(
		         currentQuestion.getPK().getId(),
		         currentQuestion.getInstanceId());
		   }
		   catch (ContentManagerException ignored)
		   {
             SilverTrace.error("questionReply",
	...
	*/
	...
	public Collection getAllCategories() throws QuestionReplyException
	{
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.getAllCategories()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_CATEGORIES_NOT_EXIST", e);
		}
	}
	...
	public synchronized void createCategory(Category category) throws QuestionReplyException
	{
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.createCategory()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_CATEGORIES_NOT_CREATE", e);
		}
	}
	...
	public Category getCategory (String categoryId) throws QuestionReplyException
	{
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.getCategory()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_CATEGORY_NOT_EXIST", e);
		}
	}
	...
	public synchronized void updateCategory(Category category) throws QuestionReplyException
	{
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.updateCategory()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_CATEGORY_NOT_EXIST", e);
		}
	}
	...
	public synchronized void deleteCategory(String categoryId) throws QuestionReplyException
	{
		try
		{
			// pour cette catégorie, rechercher les questions et mettre "" dans la catégorie
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.deleteCategory()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_CATEGORY_NOT_EXIST", e);
		}
	}
	...
	private NodeBm getNodeBm() throws QuestionReplyException
	 {
		...
		{
			throw new QuestionReplyException("QuestionReplySessioncontroller.getNodeBm()", SilverpeasRuntimeException.ERROR, "QuestionReply.MSG_NODEBM_NOT_EXIST", e);
		}
		return nodeBm;
	 }
}