...
public class QuestionReplyRequestRouter extends ComponentRequestRouter
{
     ...
     * ex : for questionReply, returns "questionReply"
     */
    ...
    {
        ComponentSessionController component = (ComponentSessionController) new QuestionReplySessionController(mainSessionCtrl, componentContext,"com.silverpeas.questionReply.multilang.questionReplyBundle", "com.silverpeas.questionReply.settings.questionReplyIcons");

        return component;
    }
    
    /**
	  * Extract the container context from the request
	  * and save it in the session controller.
	  *
	  * If this context is null then get the last one from
    ...
    private void resetContainerContext(QuestionReplySessionController scc, HttpServletRequest request)
	 {
		 ...
		 {
			SilverTrace.info("questionReply", "QuestionReplyRequestRouter.resetContainerContext()", "root.MSG_GEN_PARAM_VALUE", "returnURL != null");
	...
	private void resetReturnURL(QuestionReplySessionController scc, HttpServletRequest request)
	 {
		 ...
		 {
			SilverTrace.info("questionReply", "QuestionReplyRequestRouter.resetReturnURL()", "root.MSG_GEN_PARAM_VALUE", "returnURL != null");
     ...
     * @return The complete destination URL for a forward (ex : "/questionReply/jsp/questionReply.jsp?flag=user")
     */
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        SilverTrace.info("questionReply", "QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", function);
        String  destination = "";
        
        QuestionReplySessionController scc = (QuestionReplySessionController) componentSC;
        
       	String flag = scc.getUserProfil();
        SilverTrace.info("questionReply", "QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "flag = " + flag);
       	...
       	try
		{
	        ...
	        if (function.startsWith("Main"))
	        {
				...
				/*if (flag.equals("user"))
				{
					// pour le lecteur on affiche les questions et les réponses sous forme de liste DHTML
					destination = "/questionReply/jsp/listQuestionsDHTML.jsp";
				}
				else
				{
					// pour les autres on affiche les questions dans un tableau
	        ...
	        else if (function.equals("MainQuestions"))
	        {
				...
				{
					if (scc.getUserRoleLevel().equals("admin") || scc.getUserRoleLevel().equals("writer"))
	        ...
	        {
			    SilverTrace.info("questionReply", "QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "flag entrée= " + flag);
 
				if((flag.equals("admin")) || (flag.equals("writer")) || (flag.equals("publisher")))
				{
					scc.setUserProfil("publisher");
					flag = scc.getUserProfil();

					SilverTrace.info("questionReply", "QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "flag  appel= " + flag);
	        ...
	        else if (function.equals("CloseQuestion"))
	        {
				if((flag.equals("admin")) || (flag.equals("writer")))
				{
					scc.closeQuestion(new Long(request.getParameter("QuestionId")).longValue());
					...
					else destination = getDestination("Main", componentSC, request); */
					destination = getDestination("Main", componentSC, request);
				}
            	else destination = "/admin/jsp/errorpage.jsp";	  		        
			}
	        else if (function.equals("ConsultQuestionQuery"))
	        {
	        	...
	        	SilverTrace.info("questionReply","QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","questionId = " + questionId);

	        	Question question = scc.getQuestion(Long.parseLong(questionId));
	        	scc.setCurrentQuestion(question);
				/*if (request.getParameter("questionId") != null)
					scc.getQuestion(new Long(request.getParameter("questionId")).longValue());
				else
					scc.getQuestion(new Long((String)request.getAttribute("questionId")).longValue()); */
				//destination = getDestination("ConsultQuestion", componentSC, request);
	        	
	        	// redirection vers la liste des question en ouvrant la question recherchée
	        ...
	        else if (function.equals("ConsultQuestion"))
	        {
				...
				/* if(flag.equals("admin"))
					destination = "/questionReply/jsp/consultQAdmin.jsp";	
				else if(flag.equals("user"))
					destination = "/questionReply/jsp/consultQUser.jsp";
				else if(flag.equals("publisher"))
				{
					if (scc.getCurrentQuestion().getCreatorId().equals(scc.getUserId())){// le publisher est l'auteur de la question, 
						if (question.getStatus() == 2 && question.getPrivateReplyNumber() == 0) {// le publisher est un simple user
							scc.setUserProfil("user");
							flag = scc.getUserProfil();
							destination = getDestination("ConsultQuestion", componentSC, request);
						}
						else
							destination = "/questionReply/jsp/consultQPublisher.jsp";
					}
					else // le publisher est un simple user
					{
						scc.setUserProfil("user");
						flag = scc.getUserProfil();
						destination = getDestination("ConsultQuestion", componentSC, request);
				...
				else if(flag.equals("writer"))
				{
					...
					if (isRecipient) // le writer est destinataire de la question
						destination = "/questionReply/jsp/consultQExpert.jsp";
					else // le writer est potentiellement un publisher
					{
						scc.setUserProfil("publisher");
						flag = scc.getUserProfil();
						destination = getDestination("ConsultQuestion", componentSC, request);
	        ...
	        {
				if((flag.equals("admin")) || (flag.equals("publisher")) || (flag.equals("writer")))
				{
	        ...
	        else if (function.equals("EffectiveUpdateQ"))
	        {
				...
				scc.updateCurrentQuestion(request.getParameter("title"), request.getParameter("content"), request.getParameter("CategoryId"));
				//destination = getDestination("ConsultQuestionQuery", componentSC, request);
	        ...
	        {
				if((flag.equals("admin")) || (flag.equals("writer")) || (flag.equals("publisher")) || (flag.equals("user")))
				{
	        ...
	        else if (function.equals("EffectiveCreateR"))
	        {
				/* if((flag.equals("admin")) || (flag.equals("writer")))
				{
					...
					scc.setNewReplyContent(request.getParameter("title"), request.getParameter("content"), publicReply, 1);
					scc.saveNewReply();
					request.setAttribute("urlToReload", "ConsultQuestion");
					destination = "/questionReply/jsp/closeWindow.jsp";
				}
	        ...
	        else if (function.equals("EffectiveUpdateR"))
	        {
	        	/* if((flag.equals("admin")) || (flag.equals("writer")))
				{
					...
					request.setAttribute("urlToReload", "ConsultReply");
					destination = "/questionReply/jsp/closeWindow.jsp";
				}
            	else destination = "/admin/jsp/errorpage.jsp";	 */
				scc.updateCurrentReply(request.getParameter("title"), request.getParameter("content"));
	        ...
	        else if (function.equals("EffectiveCreateQR"))
	        {
				//if((flag.equals("admin")) || (flag.equals("writer")))
				//{
					scc.setNewQuestionContent(request.getParameter("title"), request.getParameter("content"), request.getParameter("CategoryId"));
					scc.setNewReplyContent(request.getParameter("titleR"), request.getParameter("contentR"), 1, 0);
			...
			else if (function.equals("ViewPdcPositions"))
		    {
				...
				//NEWD DLE
				//request.setAttribute("ReturnURL", "/RquestionReplyPDC/"+scc.getSpaceId()+"_"+scc.getComponentId()+"/ViewPdcPositions");
				request.setAttribute("ReturnURL", "/RquestionReplyPDC/"+scc.getComponentId()+"/ViewPdcPositions");
				//NEWF DLE
			...
			else if (function.startsWith("searchResult"))
			{
				...
				SilverTrace.info("questionReply","QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","type = " + type + " id = " + id);
				...
				else if (type.startsWith("Reply"))
				{
					...
					SilverTrace.info("questionReply","QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","questionId = " + questionId + " replyId = " + id);
        ...
        SilverTrace.info("questionReply","QuestionReplyRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","destination " + destination);
		return destination;  
	}
}      	        
