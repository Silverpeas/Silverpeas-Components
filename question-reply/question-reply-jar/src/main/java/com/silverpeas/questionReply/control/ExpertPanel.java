...
public class ExpertPanel extends PanelProvider
{
    ...
    public void initAll(String language, Collection allExperts)
    {
        ...
        m_rs = new ResourceLocator("com.silverpeas.questionReply.multilang.questionReplyBundle",m_Language);
    ...
    public void refresh(String[] filters)
    {
        ...
        while (it.hasNext())
        {
            ...
            {
                if ((user.getFirstName() == null) || (filters[FILTER_FIRSTNAME].length() > user.getFirstName().length()) || (! user.getFirstName().substring(0,filters[FILTER_FIRSTNAME].length()).equalsIgnoreCase(filters[FILTER_FIRSTNAME])))
                {
                    keepit = false;
                }
            }
            ...
            {
                if ((user.getLastName() == null) || (filters[FILTER_LASTNAME].length() > user.getLastName().length()) || (! user.getLastName().substring(0,filters[FILTER_LASTNAME].length()).equalsIgnoreCase(filters[FILTER_LASTNAME])))
                {
                    keepit = false;
                }
            }
    ...
    public PanelLine getElementInfos(String id)
    {
        ...
        SilverTrace.info("questionReply", "ExpertPanel.getElementInfos()", "root.GEN_MSG_PARAM_VALUE", "id=" + id);
