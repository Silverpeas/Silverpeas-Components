...
public class QuestionReplyContentManager implements ContentInterface {
  ...
  private List getHeaders(ArrayList ids, String instanceId) {
    ...
    Question question;
    try {
      ArrayList questions = (ArrayList) QuestionManager.getInstance()
          .getQuestionsByIds((ArrayList) ids);
      for (int i = 0; i < questions.size(); i++) {
        question = (Question) questions.get(i);
        headers.add(new QuestionHeader(new Long(question.getPK().getId())
            .longValue(), question, instanceId, question.getCreationDate(),
