...
public class WhitePagesContentManager implements ContentInterface
{
   ...
   private boolean isAdmin(List userRoles)
	...
	private List getHeaders(List ids, boolean isAdmin, String instanceId)
	{
		...
		try
		{
			ArrayList cards = (ArrayList) CardManager.getInstance().getCardsByIds((ArrayList) ids);
			for (int i=0 ; i<cards.size() ; i++)
			{
				card = (Card) cards.get(i);
    			headers.add(new CardHeader(new Long(card.getPK().getId()).longValue(), card, instanceId, card.getCreationDate(), new Integer(card.getCreatorId()).toString()));
			}
		}
	...
	public int getSilverObjectId(String id, String peasId) throws WhitePagesException {
		SilverTrace.info("whitePages","WhitePagesContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "id = "+id);
		try {
			return getContentManager().getSilverContentId(id, peasId);
		} catch (Exception e) {
			throw new WhitePagesException("WhitePagesContentManager.getSilverObjectId()",SilverpeasException.ERROR,"whitePages.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
		}
	}
   ...
   public int createSilverContent(Connection con, Card card) throws ContentManagerException
   {
	  ...
	  SilverTrace.info("whitePages","WhitePagesContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  return getContentManager().addSilverContent(con, card.getPK().getId(), card.getInstanceId(), card.getUserId(), scv);
   }
   ...
   {
	  int silverContentId = getContentManager().getSilverContentId(card.getPK().getId(), card.getPK().getComponentName());
	  SilverContentVisibility scv = new SilverContentVisibility(isVisible(card));
	  SilverTrace.info("whitePages","WhitePagesContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  getContentManager().updateSilverContentVisibilityAttributes(scv, card.getPK().getComponentName(), silverContentId);
   ...
   public void deleteSilverContent(Connection con, IdPK pk) throws ContentManagerException
   {
	  ...
	  SilverTrace.info("whitePages","WhitePagesContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "id = "+pk.getId()+", contentId = "+contentId);
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("whitePages", "WhitePagesContentManager", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
