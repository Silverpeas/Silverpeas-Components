...
public class CardManager 
{
   ...
   public long create(Card card, String spaceId, String creatorId) throws WhitePagesException
   {
		...
		try
		{
			dao = SilverpeasBeanDAOFactory.getDAO( "com.silverpeas.whitePages.model.Card" );
   ...
   public Card getCard(long id) throws WhitePagesException
   {
		...
		try
		{
			...
			throw new WhitePagesException("CardManager.getCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_CARD","",e);
		}
		return result;
   }
   ...
   public Collection getCards(String instanceId) throws WhitePagesException 
   {
		...
		try
		{
			...
			throw new WhitePagesException("CardManager.getCards",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_CARDS","",e);
		}
		return cards;
   }
   ...
   public Collection getVisibleCards(String instanceId) throws WhitePagesException 
   {
		...
		try
		{
			...
			throw new WhitePagesException("CardManager.getVisibleCards",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_CARDS","",e);
		}
		return cards;
   }

	public Collection getUserCards(String userId, Collection instanceIds) throws WhitePagesException  
	{
				...
				try
				{
					...
					if (cards != null)
					{
						...
						while (itCard.hasNext())
						{
							...
							wpcards.add(new WhitePagesCard(new Long(card.getPK().getId()).longValue(), card.getInstanceId()));
						}
					}
				}
				catch(PersistenceException e) {
					throw new WhitePagesException("CardManager.getUserCards",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_USERCARDS","",e);
				}
			}
	...
	public Collection getHomeUserCards(String userId, Collection instanceIds, String instanceId) throws WhitePagesException  
	{
		String where = " userId = '" + userId + "' and ((instanceId = '" + instanceId + "') or (hideStatus = 0";
		Collection wpcards = new ArrayList();
		if (instanceIds != null)
		{
			Iterator it = instanceIds.iterator();
			...
			{			
				...
				try
				{
					...
					throw new WhitePagesException("CardManager.getHomeUserCards",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_USERCARDS","",e);
				}
			}
	...
	public void setHideStatus (Collection ids, int status) throws WhitePagesException 
	{
		if (ids != null)
		{
			Iterator it = ids.iterator();
			try
			{
				...
				throw new WhitePagesException("CardManager.setHideStatus",SilverpeasException.ERROR,"whitePages.EX_UPDATE_CARDS_FAILED","",e);
			}
		}
	}
	...
	public void reverseHide (Collection ids) throws WhitePagesException 
	{
		if (ids != null)
		{
			...
			try
			{
				...
				throw new WhitePagesException("CardManager.reverseHide",SilverpeasException.ERROR,"whitePages.EX_UPDATE_CARDS_FAILED","",e);
			}
		}
	}
	...
	public boolean existCard(String userId, String instanceId) throws WhitePagesException 
	{
		...
		try
		{
			...
			throw new WhitePagesException("CardManager.existCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_CARDS","",e);
		}
		return exist;
	}
	...
	public boolean isPublicationClassifiedOnPDC(Card card) throws ContentManagerException, PdcException
	{
		...
		int contentId = contentManager.getSilverContentId(card.getPK().getId(), card.getInstanceId());
	...
	public Card getUserCard(String userId, String instanceId) throws WhitePagesException 
	{
		...
		try
		{
			...
			throw new WhitePagesException("CardManager.getUserCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_CARDS","",e);
		}
		return card;
	}
   ...
   {
   		Iterator 	cards 	= getVisibleCards(instanceId).iterator();
   		Card 		card	= null;
   		while (cards.hasNext())
   		{
   ...
   public void indexCard(Card card)
   {
      ...
      try {
    	  PublicationTemplate pub = PublicationTemplateManager.getPublicationTemplate(card.getInstanceId());
   ...
   private void deleteIndex(WAPrimaryKey pk, String spaceId)
   {
      IndexEngineProxy.removeIndexEntry(
         new IndexEntryPK(pk.getComponentName(), "card", pk.getId() ));
   }
   ...
   private void rollback(Connection con, Exception e) throws WhitePagesException 
   {
		...
		{
			throw new WhitePagesException("CardManager.create",SilverpeasException.ERROR,"whitePages.EX_CREATE_CARD_FAILED","Error in rollback",e1);
		}
			
		throw new WhitePagesException("CardManager.create",SilverpeasException.ERROR,"whitePages.EX_CREATE_CARD_FAILED","",e);
   }
