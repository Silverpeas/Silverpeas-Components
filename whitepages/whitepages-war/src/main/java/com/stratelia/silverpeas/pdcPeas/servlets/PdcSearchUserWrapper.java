...
public class PdcSearchUserWrapper extends ComponentRequestRouter
{
    ...
    {
        return new PdcSearchUserWrapperSessionController(
    ...
    {
    	SilverTrace.info("pdcPeas", "PdcSearchUserWrapper.getDestination()", "root.MSG_GEN_PARAM_VALUE", " Function=" + function);
        ...
        try
        {
			if (function.equals("Open"))
			{
				...
				pdcSearchUserWrapperScc.setElementName(request.getParameter("elementName"));
				pdcSearchUserWrapperScc.setSelectedUserIds(request.getParameter("selectedUsers"));
				pdcSearchUserWrapperScc.initPdcSearchUser(); //selection par défaut d'éléments
				//return "/RpdcSearch/jsp/ToSearchToSelect?ComponentName=expertLocator&ReturnURL=/RpdcSearchUserWrapper/jsp/Close";
			...
			else if (function.equals("Close"))
			{
				...
				request.setAttribute("elementId", pdcSearchUserWrapperScc.getElementId());
				request.setAttribute("elementName", pdcSearchUserWrapperScc.getElementName());
