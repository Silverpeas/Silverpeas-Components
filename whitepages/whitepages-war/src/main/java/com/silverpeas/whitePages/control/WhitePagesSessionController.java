...
public class WhitePagesSessionController extends AbstractComponentSessionController
{
	...
	private Collection currentUserCards = new ArrayList(); // liste des fiches (Collection de WhitePagesCard) inter-instance du user de la fiche courante (currentCard.getUserId())
	private Collection userInstanceIds = null; // liste des id des instances d'annuaire pour lequel l'utilisateur courant (getUserId()) a des droits (admin ou user)
	private UserTemplate userTemplate = null; // permet de gèrer le modèle d'affichage de l'identité d'un user à partir d'un modèle html
	private PublicationTemplate cardTemplate = null;  // permet la gestion du modèle des fiches
	private String[] hostParameters = null;
	private String returnURL = "";
	private ContainerContext containerContext;
	private Card notifiedUserCard;
	private final static ResourceLocator whitePagesSettings = new ResourceLocator("com.silverpeas.whitePages.settings.settings", "");

	/*
	* Recherche une fiche
	* Retourne currentCard si son id est le même que celui de la fiche recherchée
	* Demande au CardManager la fiche sinon
	* Affecte l'attribut ReadOnly de Card à false si la fiche fait partie de l'instance (instanceId)
	* Recherche et affecte le cardRecord de la fiche (getTemplate(currentCard.getInstanceId()).getRecordset().getRecord(userCardId))
	* Recherche et affecte le userRecord de la fiche (userTemplate.getRecord(userCardId))
	* Affecte le cardViewForm (getTemplate(currentCard.getInstanceId()).getViewForm())
	* Affecte le cardUpdateForm (cardTemplate.getUpdateForm())
	* Affecte le userForm (userTemplate.getViewForm())
	* Appel getWhitePagesCards pour mettre à jour la liste des fiches inter-instance portant sur le même user
	* Met la fiche en session puis la retourne
	...
	*/
	public Card getCard(long userCardId) throws WhitePagesException
	{
		try
		{
			if ((currentCard == null) || (currentCard.getPK() == null) || (currentCard.getPK().getId() == null) || (currentCard.getPK().getId().equals("")) || (new Long(currentCard.getPK().getId()).longValue() != userCardId))
			{
			...
			throw new WhitePagesException("WhitePagesSessionController.getCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.getCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	}
	...
	*/
	public Card getCardReadOnly(long userCardId) throws WhitePagesException
	{
		try
		{
			...
			throw new WhitePagesException("WhitePagesSessionController.getCardReadOnly",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.getCardReadOnly",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	...
	* Récupère le premier élement de la liste des fiches inter-instance portant sur le user (getWhitePagesCards)
	* Appel la recherche fiche (getCardReadOnly) à partir de l'id du premier elèment de currentUserCards
	...
	*/
	...
	/*
	* Charge la liste des fiches inter-instance portant sur le user (et non masquées sauf instance courante)et la met en session si le user est différent du user courant (currentCard.getUserId())
	* Sinon retourne currentUserCards (Collection de WhitePagesCard)
	* @param userId id d'un user
	*
	*/
	private Collection getHomeWhitePagesCards(String userId) throws WhitePagesException
	{
		if ((currentCard == null) || (!currentCard.getUserId().equals(userId)) || (getCurrentUserCards().size() <= 0))
		{
			Collection cards = getCardManager().getHomeUserCards(userId, getUserInstanceIds(), getComponentId());
	...
	/*
	* Charge la liste des fiches inter-instance portant sur le user (et non masquées) et la met en session si le user est différent du user courant (currentCard.getUserId())
	* Sinon retourne currentUserCards (Collection de WhitePagesCard)
	* @param userId id d'un user
	*
	*/
	private Collection getWhitePagesCards(String userId) throws WhitePagesException
	{
		if ((currentCard == null) || (!currentCard.getUserId().equals(userId)) || (getCurrentUserCards().size() <= 0))
		{
			Collection cards = getCardManager().getUserCards(userId, getUserInstanceIds());
			Collections.sort((List) cards);
			setCurrentUserCards(cards);
		}
		return getCurrentUserCards();
	...
	/*
	* Crée une nouvelle fiche (new Card()) et affecte le UserRecord de la fiche et le userForm (userTemplate.getViewForm())
	* Met la fiche en session et la retourne
	* Ajoute un new WhitePages(" fiche en cours de création " ) à la liste des fiches
	...
	*/
	public Card createCard(UserDetail userDetail) throws WhitePagesException
	{
		...
		((ArrayList) getCurrentUserCards()).add(0, new WhitePagesCard("Fiche en cours de création"));
		return getCurrentCreateCard();
	}

	/*
	* Affecte un DataRecord vide (cardTemplate.getRecordset().getEmptyRecord()), le cardViewForm (cardTemplate.getViewForm()) et le cardUpdateForm à la fiche courante
	* Retourne la fiche courante
	*
	*/
	public Card setCardRecord() throws WhitePagesException
	{
		try
		{
			...
			throw new WhitePagesException("WhitePagesSessionController.setCardRecord",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.setCardRecord",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	}
	...
	*
	*/
	public void setCardRecord(HttpServletRequest request) throws WhitePagesException
	{
		try
		{
	...
	*/
	public void updateCardRecord(HttpServletRequest request) throws WhitePagesException
	{
		try
		{
			...
			throw new WhitePagesException("WhitePagesSessionController.updateCardRecord",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(Exception e) {
			throw new WhitePagesException("WhitePagesSessionController.updateCardRecord",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}

	}

	/*
	* Enregistre et crée la fiche courante : enregistrement de la fiche (CardManager.create(currentCard)), recupération de l'id de la fiche créée (userCardId) et set de l'id de la fiche courante
	* Enregistre les données du modèle de la fiche : currentCard.readCardRecord().setId(userCardId),  saveCard()
	*
	*/
	public void insertCard() throws WhitePagesException
	{
		try
		{
			String userCardId = new Long(getCardManager().create(getCurrentCreateCard(), getSpaceId(), getUserId())).toString();
            ...
            SilverTrace.spy("whitePages", "WhitePagesSessionController.insertCard",
                getSpaceId(), getComponentId(), userCardId,
                getUserDetail().getId(), SilverTrace.SPY_ACTION_CREATE);

		}
		catch(PublicationTemplateException e) {
			throw new WhitePagesException("WhitePagesSessionController.insertCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.insertCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	...
	* Enregistre les données du modèle de la fiche (cardTemplate.getRecordset().save(currentCard.readCardRecord()))
	*
	*/
	public void saveCard() throws WhitePagesException
	{
		try
		{
			...
			throw new WhitePagesException("WhitePagesSessionController.saveCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.saveCard",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	...
	* @return la liste de toutes les fiches de l'annuaire (Collection de Card) non masquées (hideStatus = 0)
	*/
	...
	{
		Collection	visibleCards	= setUserRecordsAndCardRecords(getCardManager().getVisibleCards(getComponentId()));
	...
	*/
	private Collection setUserRecords(Collection cards) throws WhitePagesException
	{
		...
		try {
			if (cards != null)
			{
				...
				while (it.hasNext())
				{
					...
					if (getUserTemplate().getRecord(card.getUserId()).getUserDetail() == null) {//l'utilisateur n'existe plus
			...
			throw new WhitePagesException("WhitePagesSessionController.setUserRecords",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
		return listCards;
	}
	...
	*/
	private Collection setUserRecordsAndCardRecords(Collection cards) throws WhitePagesException
	{
		ArrayList listCards = new ArrayList();
		try {
			if (cards != null)
			...
			throw new WhitePagesException("WhitePagesSessionController.setUserRecordsAndCardRecords",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.setUserRecordsAndCardRecords",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
		return listCards;
	}
	...
	*/
	public void delete(Collection userCardIds) throws WhitePagesException
	{
		...
		try
		{
			...
			throw new WhitePagesException("WhitePagesSessionController.delete",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
		catch(FormException e) {
			throw new WhitePagesException("WhitePagesSessionController.delete",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_RECORD","",e);
		}
	...
	* PublicationTemplateManager.getPublicationTemplate(instanceId, getParam("cardTemplate", instanceId))
	*/
	private PublicationTemplate getTemplate(String instanceId) throws WhitePagesException
	{
		try
		{
			return PublicationTemplateManager.getPublicationTemplate(instanceId, getParam("cardTemplate", instanceId));
		}
		catch(PublicationTemplateException e) {
			throw new WhitePagesException("WhitePagesSessionController.getTemplate",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
	}

	/*
	* Retourne le userTemplate d'un annuaire :
	* PublicationTemplateManager.getPublicationTemplate(instanceId, getParam("userTemplate", instanceId))
	*/
	private UserTemplate getUserTemplate(String instanceId) throws WhitePagesException
	{
	...
	*/
	public String initUserPanel()
	{
		...
		PairObject hostComponentName = new PairObject(getComponentLabel(),  m_context + "/RwhitePages/"+getComponentId()+"/Main");
		PairObject[] hostPath = new PairObject[1];
		hostPath[0] = new PairObject(getString("whitePages.usersList"),  "/RwhitePages/"+getComponentId()+"/Main");
	...
	* UserDetail[] UserPanel.getUserDetailSelected()
	*/
	...
	private PublicationTemplate getCardTemplate() throws WhitePagesException
	{
		try
		{
			if (cardTemplate == null)
				cardTemplate = PublicationTemplateManager.getPublicationTemplate(getComponentId(), getParam("cardTemplate"));
			return cardTemplate;
		}
		catch(PublicationTemplateException e) {
			throw new WhitePagesException("WhitePagesSessionController.getCardTemplate",SilverpeasException.ERROR,"whitePages.EX_CANT_GET_PUBLICATIONTEMPLATE","",e);
		}
	}
	...
	private Collection getUserInstanceIds()
	{
		if (userInstanceIds == null)
		{
			...
			CompoSpace[] instances = getOrganizationController().getCompoForUser(getUserId(), "whitePages");
	...
	public WhitePagesSessionController(MainSessionController mainSessionCtrl, ComponentContext context, String multilangBaseName, String iconBaseName)
	{
      ...
      if (currentCard != null)
		{
		   try
		   {
				...
				contentId = "" + contentManager.getSilverContentId(currentCard.getPK().getId(), currentCard.getInstanceId());
		   }
		   catch (ContentManagerException ignored)
		   {
             SilverTrace.error("whitePages", "WhitePagesSessionController", "whitePages.EX_UNKNOWN_CONTENT_MANAGER", ignored);
		    contentId = null;
		   }
		}
	...
	public void sendNotification(String message) throws NotificationManagerException {
		...
		String link = URLManager.getURL(null, getComponentId()) + "consultIdentity?userCardId=" + notifiedUserCard.getPK().getId();
	...
	public Boolean isEmailHidden()
	{
		...
		return new Boolean("yes".equalsIgnoreCase(getComponentParameterValue("isEmailHidden")));
	}

	public Boolean isFicheVisible()
	{
		// pour afficher ou non l'onglet fiche pour les lecteurs
		return new Boolean("no".equalsIgnoreCase(getComponentParameterValue("isFicheVisible")));
	}
}