...
public class WhitePagesRequestRouter extends ComponentRequestRouter
{
     ...
     * ex : for whitePages, returns "whitePages"
     */
    ...
    /*
     * (non-Javadoc)
     * @see com.stratelia.silverpeas.peasCore.servlets.ComponentRequestRouter#createComponentSessionController(com.stratelia.silverpeas.peasCore.MainSessionController, com.stratelia.silverpeas.peasCore.ComponentContext)
     */
    ...
    {
    	SilverTrace.info("whitePages", "WhitePagesRequestRouter.createComponentSessionController()", "", "");
        ComponentSessionController component = (ComponentSessionController) new WhitePagesSessionController(mainSessionCtrl, componentContext,"com.silverpeas.whitePages.multilang.whitePagesBundle", "com.silverpeas.whitePages.settings.whitePagesIcons");
    ...
    /*
     * (non-Javadoc)
     * @see com.stratelia.silverpeas.peasCore.servlets.ComponentRequestRouter#getDestination(java.lang.String, com.stratelia.silverpeas.peasCore.ComponentSessionController, javax.servlet.http.HttpServletRequest)
     */
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        SilverTrace.info("whitePages", "WhitePagesRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", function);
        String  destination = "";
       	...
       	try
		{
		    ...
		    */
	        if (function.startsWith("Main"))
	        {
	        	...
	        	if (flag.equals("admin") || flag.equals("user"))
				{
					...
					/*
					 * If user has been redirected to this page,
					 * he must create his own card, so user is redirected to his own card creation (excepted if card already exists).
					 */
					HttpSession session = request.getSession(true);
					if ( ( session.getAttribute( LoginFilter.ATTRIBUTE_FORCE_CARD_CREATION ) != null ) ) {
						destination = getDestination("ForceCardCreation", componentSC, request);
					}
		    ...
		    */
	        else if (function.startsWith("ForceCardCreation"))
		        {
		        	if (flag.equals("admin") || flag.equals("user"))
					{
						...
						else if ( ! scc.isCardClassifiedOnPdc() ) 
						{
							...
							destination = getDestination("ViewPdcPositions", componentSC, request);
						}
	        ...
	        else if (function.equals("createIdentity"))
	        {
	        	...
	        	/*
	        	 * If forceCardCreation flag is set
	        	 * card is current user's one
	        	 */
	        	HttpSession session = request.getSession(true);
				...
				if ((user == null)||(scc.existCard(user.getId()))) {
					...
					destination = "/whitePages/jsp/errorSelectUser.jsp";
					 ...
					 //création Card à partir d'un UserDetail et set de ViewIdentityForm, currentCard en session
...
//				session.removeAttribute(LoginFilter.ATTRIBUTE_FORCE_CARD_CREATION);
	        ...
	        else if (function.equals("consultIdentity"))
	        {
        		 ...
        		 */
        		if ( (flag.equals("admin")) || (card!=null && card.getUserId().equals(scc.getUserId())) ) {
					...
					else
					{
						...
						//retourne un objet Card contenant les infos de la fiche y compris le form et le dataRecord.
						request.setAttribute("card", card);
						request.setAttribute("whitePagesCards", scc.getCurrentUserCards());
	        ...
	        else if (function.equals("updateCard"))
	        {
				...
				else if ( (flag.equals("admin")) || (card.getUserId().equals(scc.getUserId())) ) {
					 //retourne un objet Card contenant les infos de la fiche y compris le form et le dataRecord.
					Form updateForm = card.readCardUpdateForm();
					PagesContext context = new PagesContext("myForm", "0", scc.getLanguage());
					context.setComponentId(card.getInstanceId());
	        ...
	        else if (function.equals("effectiveUpdate"))
	        {
				...
				scc.saveCard();	
        		destination = "/whitePages/jsp/closeUpdate.jsp?userCardId="+request.getParameter("userCardId");
	        }	  		                	                	  	             	        
	        
	        else if (function.equals("reverseHide"))
	        {
	        	if (flag.equals("admin")) {
	        		String retour = request.getParameter("returnPage"); //consultIdentity ou consultCard
	        		String userCardIdString = request.getParameter("userCardId");
	        ...
	        /* Acces externe via d'autres composants ou 
	           Acces interne role User */
	        ...
	        {	        
					...
					// externe
						...
						SilverTrace.info("whitePages", "WhitePagesRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "userId = "+userId);
	        ...
	        {	        
	             ...
	             ||  function.equals("Consult"))    // pdc search result
           {
               if (flag.equals("admin"))
               {
		    ...
		    else if (function.equals("ViewPdcPositions"))
		    {
				...
				//NEWD DLE
				//request.setAttribute("ReturnURL", "/RwhitePages/"+scc.getSpaceId()+"_"+scc.getComponentId()+"/ViewPdcPositions?userCardId="+userCardId);
				request.setAttribute("ReturnURL", "/RwhitePages/"+scc.getComponentId()+"/ViewPdcPositions?userCardId="+userCardId);
				//NEWF DLE
				request.setAttribute("SilverContentId", new Integer(scc.getSilverObjectId(userCardId)).toString());
        ...
        SilverTrace.info("whitePages","WhitePagesRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE","destination " + destination);
		return destination;  
	}
    ...
    private void resetContainerContext(WhitePagesSessionController scc,
	                                    HttpServletRequest request)
	 {
	    ContainerContext containerContext
		    = (ContainerContext) request.getAttribute("ContainerContext");
	 ...
	 private void resetReturnURL(WhitePagesSessionController scc, HttpServletRequest request)
	 {
		 ...
		 {
			SilverTrace.info("whitePages", "WhitePagesRequestRouter.resetReturnURL()", "root.MSG_GEN_PARAM_VALUE", "returnURL != null");
