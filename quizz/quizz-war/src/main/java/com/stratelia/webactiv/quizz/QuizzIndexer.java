...
public class QuizzIndexer implements ComponentIndexerInterface {
    ...
    public void index(MainSessionController mainSessionCtrl, ComponentContext context) throws QuizzException {
		try{
			...
			while (itQ.hasNext()) {
				QuestionContainerHeader quizzHeader = (QuestionContainerHeader) itQ.next();
				scc.updateQuizzHeader(quizzHeader, quizzHeader.getPK().getId());
			}
		}catch(Exception e){
			throw new QuizzException ("QuizzIndexer.index",QuizzException.WARNING,"Quizz.EX_CANNOT_UPDATE_QUIZZ_HEADER",e);
		}
	}
}