...
public class BlogContentManager implements ContentInterface, java.io.Serializable
{
   ...
   public int getSilverObjectId(String postId, String peasId) {
		SilverTrace.info("blog","BlogContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "postId = "+postId);
		try {
			return getContentManager().getSilverContentId(postId, peasId);
		} catch (Exception e) {
			throw new BlogRuntimeException("BlogContentManager.getSilverObjectId()",SilverpeasRuntimeException.ERROR,"kmelia.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID", e);
		}
	}
   ...
   {
	  //SilverTrace.info("blog","BlogContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	  return getContentManager().addSilverContent(con, pubDetail.getPK().getId(), pubDetail.getPK().getComponentName(), userId);
   }
   ...
   public void deleteSilverContent(Connection con, PublicationPK pubPK) throws ContentManagerException
   {
	  ...
	  if (contentId != -1)
	  {
		  SilverTrace.info("blog","BlogContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubPK.getId()+", contentId = "+contentId);
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("blog", "blogContentManager", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
	...
	private PublicationBm getPublicationBm() {
		if (currentPublicationBm == null) {
			try {
				...
				throw new BlogRuntimeException("BlogContentManager.getPublicationBm()",SilverpeasRuntimeException.ERROR, "blog.EX_IMPOSSIBLE_DE_FABRIQUER_PUBLICATIONBM_HOME", e);
			}
		}
