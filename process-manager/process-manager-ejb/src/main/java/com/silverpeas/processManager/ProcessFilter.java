...
{ 
   /**
	 * Builds a process filter which can be used to select
	 * process intance of a given process model.
	 */
   public ProcessFilter(ProcessModel model, String role, String lang) throws ProcessManagerException
   {
	   ...
	   try {
		   ...
		   throw new ProcessManagerException("ProcessFilter", "processFilter.FAIL_TO_CREATE_CRITERIA_FORM", e1);
	   }
	   ...
	   try
	   {
		   ...
		   for (int f=2; f<fields.length; f++)
		   {
			   ...
			   if (parameters != null && (parameters.containsKey("values") || parameters.containsKey("keys")))
			   {
	   ...
	   {
		   throw new ProcessManagerException("ProcessFilter", "processFilter.FAIL_TO_CREATE_CRITERIA_FORM", e);
	   }
   }
   ...
   /**
	 ...
	 *
	 * We ignore all the form exception, since this copy is only done
	 * to simplify the user life.
	 */
   ...
   /**
	...
	{
	   if ("false".equals(collapse)) this.collapse = "false";
   ...
   /**
	 ...
	 */
	public DataRecord[] filter(ProcessInstance[] allInstances,
	                           String role,
										String lang)
