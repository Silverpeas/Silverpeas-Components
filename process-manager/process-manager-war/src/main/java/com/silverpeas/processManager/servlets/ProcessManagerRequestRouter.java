...
{
    /**
     * Returns the name used by the ComponentRequestRequest
     * to store the session controller in the user session.
     */
    ...
    /**
     * Return a new ProcessManagerSessionController
     * wich will be used for each request made
     * in the given componentContext.
	  *
	  * Returns a ill session controler when the a fatal error
	  * occures. This ill session controller can only display an error page.
     */
    public ComponentSessionController createComponentSessionController(
       MainSessionController mainSessionCtrl,
       ComponentContext componentContext)
    {
		SilverTrace.info("kmelia","ProcessManagerRequestRouter.createComponentSessionController()", "root.MSG_GEN_ENTER_METHOD");
	     try
		  {
     ...
     */
	public String getDestination(String function, ComponentSessionController sessionController, HttpServletRequest request)
	{
		SilverTrace.info("processManager","ProcessManagerRequestRouter.getDestination()", "root.MSG_GEN_ENTER_METHOD", "function = "+function);
    ...
    {
       super.init(config);
       if (handlerMap == null) initHandlers();
    }
    
    private Map getHandlerMap()
    {
		SilverTrace.info("processManager","ProcessManagerRequestRouter.getHandlerMap()", "root.MSG_GEN_ENTER_METHOD");
		if (handlerMap == null)
			initHandlers();
	 ...
	 */
	/*static private FunctionHandler adminListProcessHandler = new FunctionHandler()
	{
		...
		throws ProcessManagerException
		{
			...
			return "/processManager/jsp/admin/listProcess.jsp";
		}
	};*/
     ...
     */
    /*static private FunctionHandler adminViewProcessHandler = new FunctionHandler()
    {
       public String getDestination(String function,
                                    ProcessManagerSessionController session,
                                    HttpServletRequest request)
			 throws ProcessManagerException
       {
			...
			setSharedAttributes(session, request);
			return "/processManager/jsp/admin/viewProcess.jsp";
       }
    };*/
	...
	/**
     * The doReAssign handler for the supervisor
	 * Get the new users affected and creates tasks
     */
    static private FunctionHandler adminDoReAssignHandler = new FunctionHandler()
    {
			 ...
			 throws ProcessManagerException
       {
			...
			{
				throw new ProcessManagerException("ProcessManagerRequestRouter", "processManager.ILL_CREATE_FORM", e);
			}
       }
    };

	/**
	 * The listProcess handler.
	 * Used as the Main handler too.
	 */
	...
	/**
	* The listProcess handler
	* (modified in order to skip the list re-computation).
	*/
	...
	*/
    ...
    static private FunctionHandler viewProcessHandler = new FunctionHandler()
    {
			 ...
			 throws ProcessManagerException
       {
			...
			/*String[] actors = session.getActiveUsers();
    ...
    static private FunctionHandler searchResultHandler = new FunctionHandler()
    {
			 ...
			 throws ProcessManagerException
       {
			...
			if (type==null || (!type.equals("com.stratelia.webactiv.calendar.backbone.TodoDetail") && !type.equals("ProcessInstance")))
				return listProcessHandler.getDestination(function, session, request);
			...
			try
			{
				...
				for (int a=0; a<attachmentIds.size(); a++)
				{
					...
					if (session.isVersionControlled())
					{
						...
						documentPK = new DocumentPK(Integer.parseInt(attachmentId), "useless", session.getComponentId());
	...
	*/
      ...
      throws ProcessManagerException
	{
		...
		{
			throw new ProcessManagerException("ProcessManagerRequestRouter", "processManager.ERR_ILL_FILTER_FORM", e);
		}
	}
	...
	*/
	static private void setSharedAttributes(ProcessManagerSessionController session, HttpServletRequest request)
	{
   ...
   static int intValue(String parameter, int defaultValue)
	{
	   try
		{
		   if (parameter != null) return (new Integer(parameter)).intValue();
   ...
   static private PagesContext getFormContext(String formName,
									...
									boolean printTitle)
	{
		PagesContext pagesContext = new PagesContext(formName, formIndex, session.getLanguage(), printTitle, session.getComponentId(), session.getUserId());
