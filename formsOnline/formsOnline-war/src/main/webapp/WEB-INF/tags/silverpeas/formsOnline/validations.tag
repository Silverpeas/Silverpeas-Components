...
<c:set var="nextStep" value="false"/>
<c:forEach var="validation" items="${validations}">
  <formsOnline:validation validation="${validation}" nextStep="${nextStep}" />
  <c:if test="${validation.pendingValidation}">
    <c:set var="nextStep" value="true"/>
  </c:if>
</c:forEach>