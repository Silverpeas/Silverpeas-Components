...
public class DefaultFormsOnlineService implements FormsOnlineService, Initialization {
  ...
  public FormDetail saveForm(FormDetail form,
    ...
    final Map<String, Pair<List<String>, List<String>>> filteredRights =
        userAndGroupIdsByRightTypes.entrySet().stream()
  ...
  public RequestsByStatus getValidatorRequests(RequestsFilter filter, String validatorId,
    ...
    final Map<String, Set<FormInstanceValidationType>> possibleValidatorValidationTypesByFormId =
        getValidatorFormIdsWithValidationTypes(filter.getComponentId(), validatorId,
            filter.getFormIds());
    final List<FormDetail> availableForms =
        getDAO().getForms(possibleValidatorValidationTypesByFormId.keySet());
    ...
    for (final FormDetail form : availableForms) {
      ...
      for (final ValidationMergeRuleByStates rule : VALIDATION_MERGING_RULES_BY_STATES) {
        ...
        final SilverpeasList<FormInstance> result = getDAO().getReceivedRequests(form, states,
            validationCriteria,
    ...
    if (pendingValidationTypeFilter.isPresent()) {
      resultStream = resultStream
          .filter(r -> {
            ...
            final Set<FormInstanceValidationType> possibleTypes =
                r.getForm().getPossibleRequestValidations().keySet();
    ...
    for (FormDetail form : forms) {
      if (form.isHierarchicalValidation() && (isEmpty(formIds) || formIds.contains(form.getPK().getId()))) {
        ...
        final Set<String> creatorIds =
            requests.stream().map(FormInstance::getCreatorId).collect(toSet());
  ...
  private void updateRequest(FormInstance request, List<FileItem> items,
    ...
    try {
      ...
      // Save data form
      PagesContext aContext = new PagesContext("dummy", "0",
          UserDetail.getById(request.getCreatorId()).getUserPreferences().getLanguage(), false,
          request.getComponentInstanceId(), request.getCreatorId());
      aContext.setObjectId(String.valueOf(request.getId()));
      form.update(items, data, aContext);
      set.save(data);
    } catch (Exception e) {
  ...
  protected PublicationTemplate getPublicationTemplate(FormInstance request)
      ...
      try {
        ...
        if (form.isDeleteAfterRequestExchange()) {
          ...
          MessageNotifier.addSuccess(
              messages.getStringWithParams("formsOnline.request.exchange.successAndSummary",
                  form.getTitle()));
        } else {
          MessageNotifier.addSuccess(
              messages.getStringWithParams("formsOnline.request.exchange.success",
                  form.getTitle()));
        }
      } catch (Exception e) {
  ...
  protected Pair<String, List<SimpleDocument>> prepareMailContents(final FormInstance request,
    ...
    try {
      ...
      final Map<String, String> values = dataRecord.getValues(i18n.getDefaultLanguage());
      for (final FieldTemplate field : fields) {
        final String value = values.get(field.getFieldName());
        if (StringUtil.isDefined(value)) {
          // only defined fields are sent
          content.addElement(field.getLabel(i18n.getDefaultLanguage()));
          content.addElement(" : ");
