...
public class SilverCrawlerFileServer extends HttpServlet {
  ...
  public void init(ServletConfig config) {
    try {
      ...
      SilverTrace.fatal("silverCrawler", "FileServer.init()",
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res)
    ...
    if ("link".equals(typeUpload)) {
      ...
      type = Statistic.FILE;
      fileStat = filePath;
      SilverTrace.debug("silverCrawler", "FileServer",
          "root.MSG_GEN_PARAM_VALUE", "file, type = " + type + " file = "
              + fileStat);
    } else {
      filePath = FileRepositoryManager.getTemporaryPath(null, componentId)
          + sourceFile;

      type = Statistic.DIRECTORY;
      fileStat = rootPath + separator + path;
      SilverTrace.debug("silverCrawler", "FileServer",
          "root.MSG_GEN_PARAM_VALUE", "directory, type = " + type + " file = "
              + fileStat);
    }
  ...
  private void outputFile(HttpServletResponse res, String filePath)
      throws IOException {
    OutputStream out2 = res.getOutputStream();
    int read;
    BufferedInputStream input = null; // for the html document generated
    SilverTrace.info("silverCrawler", "FileServer.displayHtmlCode()",
    ...
    try {
      ...
      try {
        ...
        SilverTrace.warn("silverCrawler", "FileServer.displayHtmlCode",
  ...
  private void displayWarningHtmlCode(HttpServletResponse res)
    ...
    try {
      read = sr.read();
      while (read != -1) {
        SilverTrace.info("silverCrawler", "FilServer.displayHtmlCode()",
            "root.MSG_GEN_ENTER_METHOD", " StringReader read " + read);
        out2.write(read); // writes bytes into the response
        read = sr.read();
      }
    } catch (Exception e) {
      SilverTrace.warn("silverCrawler", "FileServer.displayWarningHtmlCode",
