...
public class SilverCrawlerRequestRouter extends ComponentRequestRouter
{
    ...
    public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request)
    {
        ...
        SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "User=" + componentSC.getUserId() + " Function=" + function);
        ...
        try
        {
            ...
            else if (function.equals("ViewDirectory"))
            {
            	...
            	request.setAttribute("IsRootPath", new Boolean(silverCrawlerSC.isRootPath()));
            	request.setAttribute("IsAllowedNav", silverCrawlerSC.isAllowedNav());
            	request.setAttribute("RootPath", silverCrawlerSC.getRootPath());
            	
            	// récupération des paramètres pour la taille des tableaux (nb maxi de répertoires et de fichiers à afficher par page)
            	request.setAttribute("MaxDirectories", silverCrawlerSC.getNbMaxDirectoriesByPage());
            ...
            else if (function.equals("ViewDownloadHistory"))
            {
            	...
            	request.setAttribute("Downloads", silverCrawlerSC.getHistoryByFolder(name));
            ...
            else if (function.equals("ViewDownloadHistoryFromResult"))
            {
            	...
            	request.setAttribute("Downloads", silverCrawlerSC.getHistoryByFolderFromResult(path));
            ...
            else if (function.equals("ViewHistoryByUser"))
            {
            	...
            	request.setAttribute("DownloadsByUser", silverCrawlerSC.getHistoryByUser(folderName, userId));
           ...
           {
   		 	SilverTrace.debug("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "IndexSelected");

   		 		if (flag.equals("admin")) 
        	   	{
	           		String[] checkdirectories = request.getParameterValues("checkedDir");
	           		SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "checkdirectories = " + checkdirectories.length);
					...
					if (checkdirectories != null) 
					{
						for (int i=0;i<checkdirectories.length;i++) 
						{
							String dirName = checkdirectories[i];
				       		SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "dirName = " + dirName);
           ...
           else if (function.equals("IndexFileSelected"))
           {
   		 	SilverTrace.debug("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "IndexSelected");
   		 		...
   		 		if (flag.equals("admin")) 
        	   	{
					String[] checkfiles = request.getParameterValues("checkedFile");
	           		SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "checkfiles = " + checkfiles.length);
	           		...
	           		if (checkfiles != null) 
					{
						for (int i=0;i<checkfiles.length;i++) 
						{
							String fileName = checkfiles[i];
				       		SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "fileName = " + fileName);
           ...
           else if (function.equals("Search"))
            {
            	String wordSearch = (String) request.getParameter("WordSearch");
    		 	SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "word =" + wordSearch);
            	...
            	destination = rootDestination + "viewResultSearch.jsp?ArrayPaneAction=ChangePage&ArrayPaneTarget=docs&ArrayPaneIndex=0";
            }
        ...
        SilverTrace.info("silverCrawler", "SilverCrawlerRequestRouter.getDestination()", "root.MSG_GEN_PARAM_VALUE", "Destination=" + destination);
        return destination;
    }
}
