...
public class YellowpagesSessionController extends AbstractComponentSessionController {
  ...
  public TopicDetail getTopic(String id) {
    ...
    List<NodeDetail> thePath = getNodeService().getPath(getNodePK(id));
  ...
  public void addSubTopic(NodeDetail nd) {
    nd.getNodePK().setComponentName(getComponentId());
    nd.setCreatorId(getUserId());
    getYellowpagesService().addToTopic(getCurrentTopic().getNodeDetail(), nd);
  }
  ...
  public String initUserPanel() {
    ...
    Selection sel = getSelection(hostSpaceName, hostComponentName, hostPath, hostUrl, cancelUrl);
  ...
  private Selection getSelection(String hostSpaceName, Pair<String, String> hostComponentName, Pair<String, String>[] hostPath, String hostUrl, String cancelUrl) {
  ...
  public List<String> getArrayHeaders() {
    ...
    for (String nameProperty : properties) {
      ...
      try {
        ...
        SilverLogger.getLogger(this).warn("Property '"+nameProperty+"' is missing", e);
      }
  ...
  private void exportFormData(ContactFatherDetail contactFatherDetail, CSVRow csvRow) {
    ...
    try {
      ...
      if (StringUtil.isDefined(modelId) && modelId.endsWith(".xml")) {
        String xmlFormShortName = modelId.substring(0, modelId.indexOf('.'));
        PublicationTemplateImpl pubTemplate =
            (PublicationTemplateImpl) getPublicationTemplateManager()
                .getPublicationTemplate(getComponentId() + ":" + xmlFormShortName, modelId);
