...
public class YellowpagesRequestRouter extends ComponentRequestRouter {
  ...
  {
		ComponentSessionController component = (ComponentSessionController) new YellowpagesSessionController(mainSessionCtrl, componentContext);
		return component;
  }
  ...
  public String getDestination(String function, ComponentSessionController componentSC, HttpServletRequest request) {
	...
	try {
		...
		else if (function.equals("GoTo"))
		{
			...
			if (id == null || (id != null && !id.startsWith("group_")))
			{
			    ...
			    {//id != null
		    		...
		    		if(id.equals("0") && action == null) {//racine : affiche les contacts courants
		...
		else if (function.equals("Search"))
		{
			...
			String typeSearch = request.getParameter("Action"); // All || LastName || FirstName ||  LastNameFirstName
		...
		else if (function.equals("PrintList"))
		{
			...
			if ("ViewContactInTopic".equals(action)) {
				...
				if (modelId.endsWith(".xml")) {
		            ...
		            PublicationTemplateManager.addDynamicPublicationTemplate(scc.getComponentId()+":"+xmlFormShortName, xmlFormName);
		            PublicationTemplateImpl pubTemplate = (PublicationTemplateImpl)PublicationTemplateManager.getPublicationTemplate(scc.getComponentId()+":"+xmlFormShortName, xmlFormName);
		            ...
		            PagesContext  context = new PagesContext("modelForm", "0", scc.getLanguage(), false, scc.getComponentId(), scc.getUserId());
		...
		else if (function.equals("ModelUsed"))
		{
			try {
				...
				SilverTrace.info("yellowPages","YellowPagesRequestRouter.getDestination(ModelUsed)", "root.MSG_GEN_PARAM_VALUE","", e);
			}
		...
		else if (function.startsWith("modelManager"))
		{
			...
			String modelId = FileUploadUtil.getParameter(items, "ModelId"); //Id Node de rubrique ou Id de contact
			
			if("ModelChoice".equals(action)) {
				//List listTemplate = PublicationTemplateManager.getPublicationTemplates();
				List listTemplates = new ArrayList();
				ArrayList usedTemplates = new ArrayList(scc.getModelUsed());
				try {
					List allTemplates = PublicationTemplateManager.getPublicationTemplates();
					...
					SilverTrace.info("yellowpages","YellowpagesRequestRouter.getDestination(modelManager)", "root.MSG_GEN_PARAM_VALUE","", e);
				}
			...
			} else if("Add".equals(action)) { //met à jour le choix de formulaire XML
	        	if (StringUtil.isDefined(modelId))
	        	{
		            ...
		            PublicationTemplate pubTemplate =  PublicationTemplateManager.getPublicationTemplate(scc.getComponentId()+":"+xmlFormShortName);
		            Form formUpdate = pubTemplate.getUpdateForm();
		            RecordSet recordSet = pubTemplate.getRecordSet();
		            DataRecord data = recordSet.getRecord(contactId);
		            if (data == null) 
	...
	SilverTrace.info("yellowpages","YellowpagesRequestRooter.getDestination()", "root.MSG_GEN_EXIT_METHOD","destination = "+destination);
	return destination;
  }
