...
<%!
  String displayPath(YellowpagesSessionController Scc, Collection<NodeDetail> path, boolean linked, int
      beforeAfter) {
    ...
    while (iterator.hasNext()) {
      ...
      if ((i <= beforeAfter) || (i + beforeAfter >= nbItemInPath - 1)) {
        ...
        linkedPathString.append("<a href=\"javascript:onClick=topicGoTo('").append(nodeInPath.getNodePK().getId()).append("')\">").append(nodeName).append("</a>");
  ...
  TopicDetail displayTopicsToAdmin(YellowpagesSessionController yellowpagesScc, String id, String
      separator, GraphicElementFactory gef, PageContext pageContext, ServletRequest request, HttpSession session, MultiSilverpeasBundle resources, JspWriter out) throws Exception {
    return displayTopicsToAdmin(yellowpagesScc, id, separator, gef, pageContext, request, session, resources, out, true);
  }

  TopicDetail displayTopicsToAdmin(YellowpagesSessionController yellowpagesScc, String id, String
      separator, GraphicElementFactory gef, PageContext pageContext, ServletRequest request, HttpSession session, MultiSilverpeasBundle resources, JspWriter out, boolean displayOperations) throws Exception {
    ...
    if (nodeDetail.getChildrenNumber() > 0) {
      ...
      String nbContact;
      String link;
      Collection<NodeDetail> nodes = nodeDetail.getChildrenDetails();
      Iterator<NodeDetail> iteratorN = nodes.iterator();
      Collection<Integer> nbContactByTopic = CurrentTopic.getNbContactByTopic();
      Iterator<Integer> iteratorNbContact = nbContactByTopic.iterator();
      while (iteratorN.hasNext()) {
        node = iteratorN.next();
        nbContact = "?";
        if (iteratorNbContact.hasNext())
          nbContact = iteratorNbContact.next().toString();
        ...
        if (childId.equals("1") || childId.equals("2")) {
          ...
          arrayCellText1.setCompareOn(Integer.valueOf(nbContact));
  ...
  TopicDetail displayTopicsToUsers(YellowpagesSessionController yellowpagesScc, String id, String separator, String profile, GraphicElementFactory gef, PageContext pageContext, ServletRequest request, HttpSession session, MultiSilverpeasBundle resources, JspWriter out) throws Exception {
