...
/** 
 * This is the Yellowpages EJB-tier controller of the MVC.
 ...
 public class YellowpagesBmEJB implements YellowpagesBmSkeleton, SessionBean {
    ...
    if (currentNodeBm == null) {
        try {
            ...
            currentNodeBm = nodeBmHome.create();
              ...
              throw new YellowpagesRuntimeException("YellowpagesBmEJB.getNodeBm()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
          }
    }
  ...
  public ContactBm getContactBm() {
    if (currentContactBm == null) {
        try {
            ContactBmHome contactBmHome = (ContactBmHome) EJBUtilitaire.getEJBObjectRef(JNDINames.CONTACTBM_EJBHOME, ContactBmHome.class);
            currentContactBm = contactBmHome.create();
         } catch (Exception re) {
              throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactBm()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACTBM_HOME_FAILED",re);
          }
    }
    return currentContactBm;
  }
  
  private ArrayList getRecursiveNbContact(NodeDetail nodeDetail) throws RemoteException {
	  ...
	  if(childrenPKs != null) {
	      ...
	      while (iterator.hasNext()) {
	            ...
	            if (!childPK.getId().startsWith("group_")) //rubrique
	            {
	            	...
	            	nbContactsBySubTopic = getRecursiveNbContact(getNodeBm().getDetail(childPK));
	            ...
	            {
	            	groupId = childPK.getId().substring(childPK.getId().indexOf("_")+1, childPK.getId().length());
    ...
    */
    public TopicDetail goTo(String id)  {
      ...
      try {
	      ...
	      if (id.equals("2")) {        
	          ...
	          ContactPK contactPK = new ContactPK("unknown", this.space, this.componentId);
	          contactDetails = contactBm.getUnavailableContactsByPublisherId(contactPK, currentUser.getId(), "1");
	      } else {
	      ...
	      if (contactDetails != null)
	      {
	          ...
	          while (it.hasNext()) {
	              ...
	              if (contactDetail.getUserId() != null) //contact de type user Silverpeas
	              {
	            	  ...
	            	  {
	            		  SilverTrace.warn("yellowpages","YellowpagesBmEJB.goTo()", "yellowpages.EX_GET_USER_DETAIL_FAILED","contactDetail = "+contactDetail.toString(),e);
	            	  }
	              }
	              else 
	              {
	      ...
	      this.currentTopic = new TopicDetail(newPath, nodeDetail, contactDetails2userPubs(contactDetailsR), nbContactsByTopic);
      
      } catch (Exception re) {
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.goTo()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
      }
    ...
    private Collection getPathFromAToZ(NodeDetail nd) {
        ...
        try {
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.getPathFromAToZ()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_PATH_FAILED","pk = "+nd.getNodePK().toString(),re);
        }
        return newPath;
    }
    ...
    */
    private Collection getNewPath(NodeDetail nd) {
      ...
      if (find) {
          ...
          if (nd.getFatherPK().getId().equals(currentTopic.getNodeDetail().getNodePK().getId())) {
			...
			try {
			  ...
			  throw new YellowpagesRuntimeException("YellowpagesBmEJB.getNewPath()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_PATH_FAILED","pk = "+nd.getNodePK().toString(),re);
			}
          }
      }  
      return newPath;
    ...
    */
	...
	public List getTree() {
		...
		try {
			...
			for (int n=0; n<tree.size(); n++)
			{
				...
				List groupIds = (List) GroupDAO.getGroupIds(con, node.getNodePK().getId(), componentId);
				String groupId = null;
				Group group = null;
			...
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getTree()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_TREE_FAILED",re);
		}
		finally
		{
			closeConnection(con);
	...
	public List addGroup(List tree, Group group, int level)
	{
		if (group != null)
		{
			...
			Group[] subGroups = getOrganizationController().getAllSubGroups(group.getId());
			Group subGroup = null;
    ...
    */
    public NodePK addToTopic(String fatherId, NodeDetail subTopic) {
      ...
      if (isSameTopicSameLevelOnCreation(subTopic)) {
            ...
            try {
              theNodePK = getNodeBm().createNode(subTopic, currentTopic.getNodeDetail());
            } catch (Exception re) {
                  throw new YellowpagesRuntimeException("YellowpagesBmEJB.addToTopic()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
            }
            return theNodePK;
      }
    }
    ...
    private boolean isSameTopicSameLevelOnCreation(NodeDetail subTopic) {
		try {
		  ...
		  return result;
			  ...
			  throw new YellowpagesRuntimeException("YellowpagesBmEJB.isSameNameSameLevelOnCreation()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
		}
    }
    ...
    private boolean isSameTopicSameLevelOnUpdate(NodeDetail subTopic) {
		try {
		  ...
		  return result;
			 ...
			 throw new YellowpagesRuntimeException("YellowpagesBmEJB.isSameTopicSameLevelOnUpdate()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
		}
    }
  ...
  /**
    ...
    */
     public NodePK addSubTopic(NodeDetail subTopic) {
        ...
        SilverTrace.info("yellowpages","YellowpagesBmEJB.addSubTopic()","root.MSG_GEN_RETURN_VALUE","topicPk = "+pk.toString());
      ...
      if (isSameTopicSameLevelOnUpdate(topic)) {
        ...
        try {
          getNodeBm().setDetail(topic);
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.updateTopic()",SilverpeasRuntimeException.ERROR,"root.EX_UPDATE_TOPIC_FAILED","topic = "+topic.toString(),re);
        }
        SilverTrace.info("yellowpages","YellowpagesBmEJB.addSubTopic()","root.MSG_GEN_RETURN_VALUE","topic.getNodePK() = "+topic.getNodePK());
        SilverTrace.info("yellowpages","YellowpagesBmEJB.addSubTopic()","root.MSG_GEN_EXIT_METHOD");
    ...
    public NodeDetail getSubTopicDetail(String subTopicId) {
      ...
      NodePK pk = new NodePK(subTopicId, this.space, this.componentId);
        ...
        subTopic = getNodeBm().getDetail(pk);
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.getSubTopicDetail()",SilverpeasRuntimeException.ERROR,"root.EX_GET_TOPIC_INFOS_FAILED","subTopic = "+subTopicId,re);
      }
      return subTopic;
    }
    ...
    public boolean isDescendant(String descId, String nodeId) {
        ...
        try {
            ...
            if (descendants != null)
            {
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.isDescendant()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_IS_DESCENDANT_FAILED","descId = "+descId+" , nodeId = "+nodeId,re);
        }
        return isDesc;
    }
  ...
  /**
    * Delete a topic and all descendants. Delete all links between descendants and contacts.
    * This contacts will be visible in the Declassified zone. Delete All subscriptions and favorites on this topics
    * and all descendants
    ...
    */
    public void deleteTopic(String topicId) {     
      ...
      SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteTopic()","root.MSG_GEN_PARAM_VALUE","topicId="+ topicId);
      ...
      try {
        ...
        unreferenceOrphanContacts(contactPK);
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.deleteTopic()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_DELETE_CONTACTS_FAILED","topicId = "+topicId,re);
      }
      ...
      try {
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.deleteTopic()",SilverpeasRuntimeException.ERROR,"root.EX_DELETE_TOPIC_FAILED","pk = " + pkToDelete.toString(),re);
      }
    ...
    private Collection contactDetails2userPubs(Collection contactDetails) {
          ...
          i = 0;
          while (iterator.hasNext()){
              UserContact uPub = new UserContact((UserDetail) userDetails[i], (ContactDetail) iterator.next());
              i++;
              list.add(uPub);
          }
          return list;
    ...
    */
    public ContactDetail getContactDetail(String ContactId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactDetail()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactDetail()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId);
      ...
      try {
          ...
          SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactDetail()","root.MSG_GEN_EXIT_METHOD");
          return contactDetail;
      } catch (Exception re) {
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactDetail()",SilverpeasRuntimeException.ERROR,"root.EX_GET_CONTACT_FAILED","ContactId = " + ContactId,re);
      }      
    }
    
    private void setContactAttributes(ContactDetail contactDetail, UserDetail userDetail, OrganizationController orga, boolean filterExtraInfos)
    {
        ...
        ResourceLocator yellowpagesSettings = new ResourceLocator("com.stratelia.webactiv.yellowpages.settings.yellowpagesSettings", "fr");
        ...
        {
        	if ("Yes".equalsIgnoreCase(yellowpagesSettings.getString("showTelNumber")) || "Yes".equalsIgnoreCase(yellowpagesSettings.getString("showFaxNumber")))
        		userFull = orga.getUserFull(contactDetail.getUserId());
        }
    ...
    {
    	SilverTrace.info("yellowpages", "YellowpagesBmEJB.getContactDetailsByLastName()", "root.MSG_GEN_ENTER_METHOD", "query = "+query);
    	
    	Collection contactDetails = null;
    	try {
    		contactDetails = getContactBm().getDetailsByLastName(pk, query);   		
		} catch (Exception e) {
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactDetailsByLastName()", SilverpeasRuntimeException.ERROR,"root.EX_GET_CONTACTS_FAILED", e);
		}
		return contactDetails;
    }
    
    public Collection getContactDetailsByLastNameOrFirstName(ContactPK pk, String query)
    {
    	SilverTrace.info("yellowpages", "YellowpagesBmEJB.getContactDetailsByLastNameOrFirstName()", "root.MSG_GEN_ENTER_METHOD", "query = "+query);
    	
    	Collection contactDetails = null;
    	try {
    		contactDetails = getContactBm().getDetailsByLastNameOrFirstName(pk, query);   		
		} catch (Exception e) {
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactDetailsByLastNameOrFirstName()", SilverpeasRuntimeException.ERROR,"root.EX_GET_CONTACTS_FAILED", e);
		}
		return contactDetails;
    }
    
    public Collection getContactDetailsByLastNameAndFirstName(ContactPK pk, String lastName, String firstName)
    {
    	SilverTrace.info("yellowpages", "YellowpagesBmEJB.getContactDetailsByLastNameAndFirstName()", "root.MSG_GEN_ENTER_METHOD", "lastName = "+lastName+", firstName = "+firstName);
    	
    	Collection contactDetails = null;
    	try {
    		contactDetails = getContactBm().getDetailsByLastNameAndFirstName(pk, lastName, firstName);   		
		} catch (Exception e) {
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactDetailsByLastNameAndFirstName()", SilverpeasRuntimeException.ERROR,"root.EX_GET_CONTACTS_FAILED", e);
		}
		return contactDetails;
    }
    
    public Collection getAllContactDetails(NodePK nodePK) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getAllContactDetails()","root.MSG_GEN_ENTER_METHOD");
      Collection contactDetails = null;
      ...
      try {
          ...
          contactDetails = getContactBm().getDetailsByFatherPKs(nodePKsWithout12, pk, nodePK);
            if (contactDetails != null)
            {
              Iterator it = contactDetails.iterator();
              while (it.hasNext()) {
                  ContactFatherDetail contactFatherDetail = (ContactFatherDetail) it.next();
                  ...
                  if (contactDetail.getUserId() != null) //contact de type user Silverpeas
                  {
                    ...
                    {
                        SilverTrace.warn("yellowpages","YellowpagesBmEJB.getAllContactDetails()", "yellowpages.EX_GET_CONTACTS_FAILED","contactDetail = "+contactDetail.toString(),e);
                    }
                  }
                  else contactDetailsR.add(contactFatherDetail);
              }
            }
          SilverTrace.info("yellowpages","YellowpagesBmEJB.getAllContactDetails()","root.MSG_GEN_EXIT_METHOD");
          return contactDetailsR;
      } catch (Exception re) {
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.getAllContactDetails()",SilverpeasRuntimeException.ERROR,"root.EX_GET_CONTACTS_FAILED",re);
      }      
    }      
    ...
    public Collection getPathList(String ContactId) {
      ...
      //get all nodePK whick contains this contact
          fatherPKs = contactBm.getAllFatherPK(contactPK);
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.getPathList()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACTBM_HOME_FAILED",re);
      }
      try {
            NodeBm nodeBm = getNodeBm();
         ...
         //add this path
          ...
          return pathList;
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.getPathList()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_NODEBM_HOME_FAILED",re);
      }
    }
  ...
  /**
    ...
    */
    public String createContact(ContactDetail contactDetail) {
      ...
      SilverTrace.info("yellowpages","YellowpagesBmEJB.createContact()","root.MSG_GEN_PARAM_VALUE","contactDetail = "+contactDetail.toString());
      ...
      try {
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.createContact()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_CREATE_CONTACT_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.createContact()","root.MSG_GEN_RETURN_VALUE","id = "+contactPK.getId());
  ...
  /**
    ...
    * @since 1.0
    ...
    public void updateContact(ContactDetail contactDetail) {
      ...
      SilverTrace.info("yellowpages","YellowpagesBmEJB.updateContact()","root.MSG_GEN_PARAM_VALUE","contactDetail = "+contactDetail.toString());
      contactDetail.getPK().setSpace(this.space);
      contactDetail.getPK().setComponentName(this.componentId);
      ...
      try {
        ...
        if (fatherId.equals("2")||fatherId.equals("1"))             
			deleteIndex(contactPK);
             ...
             throw new YellowpagesRuntimeException("YellowpagesBmEJB.updateContact()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_UPDATE_CONTACT_FAILED",re);
      }      
  ...
  /**
    * Delete a contact
    * If this contact is in the basket or in the DZ, it's deleted from the database
    ...
    */
    public void deleteContact(String ContactId) {
        ...
        SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteContact()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId);
        //if the contact is in the basket or in the DZ
        //this contact is deleted from the database
        if (currentTopic.getNodeDetail().getNodePK().getId().equals("1") || currentTopic.getNodeDetail().getNodePK().getId().equals("2")) {
            ContactPK contactPK= new ContactPK(ContactId, this.space, this.componentId);
            try {
                ContactBm contactBm = getContactBm();
				...
				contactBm.removeContact(contactPK);
                 ...
                 throw new YellowpagesRuntimeException("YellowpagesBmEJB.deleteContact()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_DELETE_CONTACT_FAILED",re);
            }
        } else {
  ...
  /**
    ...
    */
    private void sendContactToBasket(String ContactId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.sendContactToBasket()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteContact()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId);
      ContactPK contactPK = new ContactPK(ContactId, this.space, this.componentId);
      try {
            ContactBm contactBm = getContactBm();
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.sendContactToBasket()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_SEND_CONTACT_TO_BASKET_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.sendContactToBasket()","root.MSG_GEN_EXIT_METHOD");
    }
    
    public void emptyDZByUserId() {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.emptyDZByUserId()","root.MSG_GEN_ENTER_METHOD");
      ...
      try {
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.emptyDZByUserId()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_DELETE_ORPHEAN_CONTACTS_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.emptyDZByUserId()","root.MSG_GEN_EXIT_METHOD");
    }
    ...
    public void addContactToTopic(String ContactId, String fatherId) {
      ...
      try {
        ...
        createIndex(contactPK);
            ...
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.addContactToTopic()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_ADD_CONTACT_TO_TOPIC_FAILED",re);
      }
}
  ...
  /**
    ...
    */
    public void deleteContactFromTopic(String ContactId,String fatherId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteContactFromTopic()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteContactFromTopic()","root.MSG_GEN_PARAM_VALUE","ContactId="+ContactId+" , fatherId="+fatherId);
      ContactPK contactPK = new ContactPK(ContactId, this.space, this.componentId);
      NodePK fatherPK = new NodePK(fatherId, this.space, this.componentId);
      try {
			getContactBm().removeFather(contactPK, fatherPK);
      } catch (Exception re) {
            throw new YellowpagesRuntimeException("YellowpagesBmEJB.deleteContactFromTopic()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_DELETE_CONTACT_FROM_TOPIC_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteContactFromTopic()","root.MSG_GEN_EXIT_METHOD");
    }
  ...
  /**
    ...
    */
    public void createInfoModel(String ContactId, String modelId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.createInfoModel()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.createInfoModel()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId+" , modelId = "+modelId);
      try {
        ContactBm contactBm = getContactBm();
        ...
        contactBm.createInfoModel(contactPK, modelId);
          ...
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.createInfoModel()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_CREATE_INFO_MODEL_DETAIL_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.createInfoModel()","root.MSG_GEN_EXIT_METHOD");
    }
  ...
  /**
    ...
    */
    public UserCompleteContact getCompleteContact(String ContactId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContact()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContact()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId);
      ContactPK contactPK = null;
      CompleteContact completeContact = null;
		...
		try {
			...
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getCompleteContact()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACT_FAILED",re);
		}
		...
		if (contactDetail.getUserId() != null) //contact de type user Silverpeas
		{
			...
			{
				SilverTrace.warn("yellowpages","YellowpagesBmEJB.getCompleteContact()", "yellowpages.EX_GET_USER_DETAIL_FAILED","ContactId = "+ContactId,e);
			}
		}
		OrganizationController orga = getOrganizationController();
		...
		SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContact()","root.MSG_GEN_EXIT_METHOD");
     ...
     */
     public UserCompleteContact getCompleteContactInNode(String ContactId, String nodeId) {
       SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContactInNode()","root.MSG_GEN_ENTER_METHOD");
       SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContactInNode()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId+", NodeId="+nodeId);
       ContactPK contactPK = null;
       NodePK nodePK = null;
       CompleteContact completeContact = null;
 		...
 		try {
 			...
 			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getCompleteContactInNode()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACT_FAILED",re);
 		}
 		ContactDetail contactDetail = completeContact.getContactDetail();
 		if (contactDetail.getUserId() != null) //contact de type user Silverpeas
 		{
 			...
 			{
 				SilverTrace.warn("yellowpages","YellowpagesBmEJB.getCompleteContactInNode()", "yellowpages.EX_GET_USER_DETAIL_FAILED","ContactId = "+ContactId,e);
 			}
 		}
 		OrganizationController orga = getOrganizationController();
 		UserDetail userDetail = orga.getUserDetail(contactDetail.getCreatorId());
 		UserCompleteContact userCompleteContact = new UserCompleteContact(userDetail, completeContact);
 		this.currentContact = userCompleteContact;
 		SilverTrace.info("yellowpages","YellowpagesBmEJB.getCompleteContactInNode()","root.MSG_GEN_EXIT_METHOD");
 		return this.currentContact;
     }
    
    public TopicDetail getContactFather(String ContactId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactFather()","root.MSG_GEN_ENTER_METHOD");
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactFather()","root.MSG_GEN_PARAM_VALUE","ContactId = "+ContactId);
      ContactPK contactPK = new ContactPK(ContactId, this.space, this.componentId);
      ...
      try {
          ...
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactFather()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACT_FATHER_FAILED",re);
      }
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactFather()","root.MSG_GEN_EXIT_METHOD");
      return fatherDetail;
    }
  ...
  /**
    ...
    */
    public Collection getContacts(Collection contactIds) {
      ...
      ArrayList contactDetailsR = new ArrayList();
      while (iterator.hasNext()) {
            ContactPK contactPK = new ContactPK(((String) iterator.next()), this.space, this.componentId);
        ...
        if (contacts != null)
        {
          ...
          while (it.hasNext()) {
              ...
              if (contactDetail.getUserId() != null) //contact de type user Silverpeas
              {
                ...
                {
                    SilverTrace.warn("yellowpages","YellowpagesBmEJB.getContacts()", "yellowpages.EX_GET_USER_DETAIL_FAILED","contactDetail.getUserId() = "+contactDetail.getUserId(),e);
                }

              }
              else contactDetailsR.add(contactDetail);
          }
        }
      } catch (Exception re) {
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContacts()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_ALL_CONTACTS_FAILED",re);
      }
    ...
    public Collection getContactFathers(String ContactId) {
      SilverTrace.info("yellowpages","YellowpagesBmEJB.getContactFathers()","root.MSG_GEN_ENTER_METHOD", "ContactId = "+ContactId);
      ContactPK contactPK = new ContactPK(ContactId, this.space, this.componentId);
      try {
          //fetch contact fathers
          return getContactBm().getAllFatherPK(contactPK);
      } catch (Exception re) {
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.getContactFathers()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_GET_CONTACT_FATHERS_FAILED",re);
      }
    }  
	...
	public void unreferenceOrphanContacts(ContactPK contactPK) {
      try {
        ...
        while (i.hasNext())
        {
          ...
          throw new YellowpagesRuntimeException("YellowpagesBmEJB.unreferenceOrphanContacts()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_UNREFERENCE_ORPHEAN_CONTACTS_FAILED",e);
      }
    }

    private void createIndex(ContactPK contactPK) {
          SilverTrace.info("yellowpages","YellowpagesBmEJB.createIndex()","root.MSG_GEN_ENTER_METHOD","contactPK = "+contactPK.toString());
          try {
              ContactBm contactBm = getContactBm();
              ...
              if (contactDetail != null) {
                      ...
                      indexEntry = new FullIndexEntry(contactPK.getComponentName(), "Contact", contactDetail.getPK().getId());
              ...
              SilverTrace.warn("yellowpages","YellowpagesBmEJB.createIndex()", "root.EX_INDEX_FAILED","contactPK = "+contactPK.toString(),e);
          }
          SilverTrace.info("yellowpages","YellowpagesBmEJB.createIndex()","root.MSG_GEN_EXIT_METHOD");
    }

    private void deleteIndex(ContactPK contactPK) {
          SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteIndex()","root.MSG_GEN_ENTER_METHOD","contactPK = "+contactPK.toString());
          IndexEntryPK indexEntry = new IndexEntryPK(contactPK.getComponentName(), "Contact", contactPK.getId());
          IndexEngineProxy.removeIndexEntry(indexEntry);
          SilverTrace.info("yellowpages","YellowpagesBmEJB.deleteIndex()","root.MSG_GEN_EXIT_METHOD");
    }
    ...
    public List getGroupIds(String id)
    {
    	...
    	try {
			...
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.addGroup()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_UNREFERENCE_ORPHEAN_CONTACTS_FAILED",e);
		}
    	finally
    	{
    		closeConnection(con);
    ...
    public void removeGroup(String groupId)
    {
    	...
    	try {
			...
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.removeGroup()",SilverpeasRuntimeException.ERROR,"yellowpages.EX_UNREFERENCE_ORPHEAN_CONTACTS_FAILED",e);
		}
    	finally
    	{
    		closeConnection(con);
	...
	private Connection getConnection()
	{
		...
		catch (UtilException e)
		{
			...
			throw new YellowpagesRuntimeException(
					"YellowpagesBmEJB.getConnection()",
    ...
    public void addModelUsed(String[] models, String instanceId) 
    {
		...
		{
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.addModelUsed()",SilverpeasRuntimeException.ERROR,"kmelia.IMPOSSIBLE_D_AJOUTER_LES_MODELES", e);
		}
		finally
		{
			// fermer la connexion
    ...
    public Collection getModelUsed(String instanceId)
    {
		...
		{
			throw new YellowpagesRuntimeException("YellowpagesBmEJB.getModelUsed()",SilverpeasRuntimeException.ERROR,"kmelia.IMPOSSIBLE_DE_RECUPERER_LES_MODELES", e);
		}
		finally
		{
			// fermer la connexion
