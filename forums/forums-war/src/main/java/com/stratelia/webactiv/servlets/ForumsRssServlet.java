...
public class ForumsRssServlet extends RssServlet
{
    ...
    public Collection getListElements(String instanceId, int nbReturned) throws RemoteException {
    	...
    	SilverTrace.debug("forums", "ForumsRssServlet.getListElements()", "root.MSG_GEN_PARAM_VALUE", "events = " + events);
    ...
    public String getElementTitle(Object element, String userId) {
    	...
    	SilverTrace.debug("forums", "ForumsRssServlet.getElementTitle()", "root.MSG_GEN_PARAM_VALUE", "message.elementAt(1) = " + message.elementAt(1));
    ...
    public String getElementLink(Object element, String userId) {
    	Vector message = (Vector) element;
    	String messageUrl = URLManager.getApplicationURL()+"/ForumsMessage/"+ (String) message.elementAt(0)+ "?ForumId=" + (String) message.elementAt(4);

    	SilverTrace.debug("forums", "ForumsRssServlet.getElementLink()", "root.MSG_GEN_PARAM_VALUE", "messageUrl = " + messageUrl);
    ...
    public String getElementDescription(Object element, String userId) {
    	...
    	SilverTrace.debug("forums", "ForumsRssServlet.getElementDescription()", "root.MSG_GEN_PARAM_VALUE", "message.elementAt(6) = " + message.elementAt(1));
    ...
    public Date getElementDate(Object element) {
		...
		calElement.add(Calendar.HOUR_OF_DAY, 0);  //-1 car bug d'affichage du fil RSS qui affiche toujours 1h en trop*/
		
    	SilverTrace.debug("forums", "ForumsRssServlet.getElementDate()", "root.MSG_GEN_PARAM_VALUE", "messageCreationDate = " + messageCreationDate);
   ...
   private ForumsBM getForumsBM()
	{
		...
		try 
		{
			ForumsBMHome forumsBMHome = (ForumsBMHome) EJBUtilitaire.getEJBObjectRef(JNDINames.FORUMSBM_EJBHOME, ForumsBMHome.class);
			forumsBM = forumsBMHome.create();
		} 
		catch (Exception e) 
		{
			throw new ForumsRuntimeException("RssServlet.getForumsBM()",SilverpeasRuntimeException.ERROR,"root.EX_CANT_GET_REMOTE_OBJECT",e);
		}
		return forumsBM;
	}
}