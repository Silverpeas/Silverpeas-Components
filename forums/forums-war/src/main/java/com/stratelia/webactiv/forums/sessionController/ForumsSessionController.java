...
public class ForumsSessionController extends AbstractComponentSessionController
{
	...
	public Forum[] getForumsList()
	{
		try
		{
			ArrayList forums = forumsBM.getForums(new ForumPK(getComponentId(), getSpaceId()));
		...
		int forumParent, String categoryId, String keywords)
	{
		...
		try
		{
			...
			return forumsBM.createForum(forumPK, truncateTextField(forumName),
				truncateTextArea(forumDescription), forumCreator, forumParent, categoryId, keywords);
		}
		catch (RemoteException re)
		{
			throw new EJBException(re.getMessage());
		...
		String categoryId, String keywords)
	{
		try
		{
			forumsBM.updateForum(getForumPK(forumId), truncateTextField(forumName),
	...
	private void fillMessageList(Vector messageList, Message[] messages, int messageId)
	{
	...
	public Object[] getLastMessage(int forumId, int messageId)
	{
		try
		{
			Message message = (messageId != -1
				? forumsBM.getLastMessage(getForumPK(forumId), messageId)
				: forumsBM.getLastMessage(getForumPK(forumId)));
			if (message != null)
			{
		...
		String keywords)
	{
		...
		try
		{
			...
			messageId = forumsBM.createMessage(messagePK, truncateTextField(title), author, null,
				forumId, parentId, text, keywords);
		}
		catch (RemoteException re)
		{
			throw new EJBException(re.getMessage());
	...
	public void updateMessage(int messageId, int parentId, String title, String text)
	{
		MessagePK messagePK = getMessagePK(messageId);
		try
		{
			forumsBM.updateMessage(messagePK, truncateTextField(title), text, getUserId());
		}
		catch (RemoteException re)
		{
			throw new EJBException(re.getMessage());
	...
	public void sendNotification(String title, String text, int parentId, int messageId)
	...
	public boolean isNewMessage(String userId, int forumId, int messageId)
	{
		...
		try
		{
			isNewMessage = forumsBM.isNewMessage(userId, getForumPK(forumId), messageId);
	...
	{
		UserDetail[] userDetails = CollectionUtil.sortUserDetailArray(
			getOrganizationController().getAllUsers(getComponentId()));
	...
	public String getAuthorName(String userId)
	{
		...
		return (userDetail != null
			? (userDetail.getFirstName() + " " + userDetail.getLastName()).trim()
			: null);
	}
	...
	{
		return (s.length() >= DBUtil.TextFieldLength
			? s.substring(0, DBUtil.TextFieldLength - 1) : s);
	}

	private String truncateTextArea(String s)
	{
		return (s.length() >= DBUtil.TextAreaLength
			? s.substring(0, DBUtil.TextAreaLength - 1) : s);
	}
	...
	public void addMessageStat(int messageId, String userId) throws RemoteException
	{
		getStatisticBm().addStat(userId,
			new ForeignPK(String.valueOf(messageId), getComponentId()), 1, STAT_TYPE);
	}
	
	public int getMessageStat(int messageId) throws RemoteException
	{
		return getStatisticBm().getCount(new ForeignPK(String.valueOf(messageId), getComponentId()), STAT_TYPE);
	}
	...
	{
		return new NotationPK(
			String.valueOf(forumId), getComponentId(), Notation.TYPE_FORUM, getUserId());
	}
	
	private NotationPK getMessageNotationPk(int messageId)
	{
		return new NotationPK(
			String.valueOf(messageId), getComponentId(), Notation.TYPE_MESSAGE, getUserId());
	}
