...
public class ActionUrl
{
	private static final String[] FIELD_NAMES = {
		"Space", "Component", "call", "action", "params", "forumId", "addStat", "changeDisplay"};
	private static final String[] FIELD_DEF_VALUES = {
		"",      "",          "",     "-1",     "-1",     "-1",      "false",   "false"};
	...
	public static String getUrl(String page, String call, int action, int params, int forumId)
	{
		return (new ActionUrl(page, call, action, params, forumId)).toString();
	}
	
	public static String getUrl(String page, String call, int action, int params, int forumId,
		boolean addStat, boolean changeDisplay)
	{
		return (new ActionUrl(page, call, action, params, forumId, addStat, changeDisplay))
			.toString();
	}
	
	public static String getUrl(String spaceId, String componentId, String page, String call,
		int action, int params, int forumId)
	{
		return (new ActionUrl(spaceId, componentId, page, call, action, params, forumId)).toString();
	}
	...
	private ActionUrl(String page, String call, int action, int params, int forumId)
	{
		this.page = page;
		this.call = call;
		this.action = action;
	...
	private ActionUrl(String page, String call, int action, int params, int forumId, boolean addStat,
	...
	private ActionUrl(String spaceId, String componentId, String page, String call, int action,
		int params, int forumId)
	{
	...
	public String toString()
	{
		...
		String[] values = {spaceId, componentId, call, String.valueOf(action),
			String.valueOf(params), String.valueOf(forumId), String.valueOf(addStat),
			String.valueOf(changeDisplay)};
		for (int i = 0; i < FIELD_COUNT; i++)
		{
			if (!values[i].equals(FIELD_DEF_VALUES[i]))
			{
				sb.append(sb.length() > 0 ? "&" : "?").append(FIELD_NAMES[i])
					.append("=").append(values[i]);
			}
		}
		return ((page == null || page.length() == 0) ? DEFAULT_PAGE : page) + ".jsp" + sb.toString();
	}
