...
/**
 * Cette classe est le Business Manager
 * qui gere les forums
 * @author frageade
...
public class ForumsBMEJB implements SessionBean
{
    ...
    /**
     * Verrouille recursivement l'arborescence
     * d'un forum en ecriture a partir de sa primary key
    ...
    /**
     * Deverrouille recursivement un forum en ecriture
     * a partir de sa primary key
     * @param ForumPK la primary key du forum
     * @param int le niveau de verrouillage
    ...
    /**
     * Supprime un forum et tous ses sous-forums
     * a partir de sa primary key
     * @param ForumPK la primary key du forum
     * @author frageade
    ...
    public void deleteForum(ForumPK forumPK)
    {
        ...
        for (int i = 0; i < sonsIds.size(); i++)
        {
            deleteForum(new ForumPK(
            	forumPK.getComponentName(), forumPK.getDomain(), (String)sonsIds.get(i)));
        }
        ...
        try
        {
            ...
            {
                deleteMessage(new MessagePK(
                	forumPK.getComponentName(), forumPK.getDomain(), (String)messagesIds.get(i)));
            }
    	...
    	String forumCreator, int forumParent, String categoryId, String keywords)
    {
        ...
        try
        {
            int forumId = ForumsDAO.createForum(
            	con, forumPK, forumName, forumDescription, forumCreator, forumParent, categoryId);
    ...
    /**
     * Met a jour les informations sur
     * un forum dans la datasource
     * @param ForumPK la primary key du forum
     * @param String nom du forum
     * @param String description du forum
     * @param int l'id du forum parent
    	...
    	int forumParent, String categoryId, String keywords, boolean updateTagCloud)
    {
        ...
        try
        {
            ForumsDAO.updateForum(
            	con, forumPK, forumName, forumDescription, forumParent, categoryId);
            deleteIndex(forumPK);
            createIndex(forumPK);
            if (updateTagCloud)
            {
    ...
    public boolean isNewMessage(String userId, ForumPK forumPK, int messageParentId)
    {
        Connection con = openConnection();
        try
        {
        	...
        	Date dateLastVisit = ForumsDAO.getLastVisit(con, userId, messagesIds);
       		...
       		if (dateLastMessageBySubject == null || dateLastVisit == null
       			|| (dateLastMessageBySubject != null && dateLastVisit != null
       				&& dateLastVisit.before(dateLastMessageBySubject)))
       		{
       			// la date de dernière visite de ce message est antérieure à la date du dernier
    ...
    /**
     * Supprime un message et tous ses sous-messages
     * a partir de sa primary key
     * @param MessagePK la primary key du message
     * @author frageade
     * @since 04 Octobre 2000
    ...
    public void deleteMessage(MessagePK messagePK)
    {
        ...
        if (v.size() > 0)
        {
            ...
            {
                deleteMessage(new MessagePK(
                	messagePK.getComponentName(), messagePK.getDomain(), (String) v.elementAt(i)));
            }
        }
        con = openConnection();
        try
    ...
    public void moveMessage(MessagePK messagePK, ForumPK forumPK)
    {
        ...
        if (v.size() > 0)
        {
            for (int i = 0; i < v.size(); i++)
            {
                moveMessage(new MessagePK(messagePK.getComponentName(), messagePK.getDomain(),
                	(String) v.elementAt(i)), forumPK);
            }
        }
        con = openConnection();
        try
	...
	public int getSilverObjectId(ForumPK forumPK)
	{
		...
		try
		{
			...
			if (silverObjectId == -1)
			{
				...
				silverObjectId = getForumsContentManager().createSilverContent(
					null, forumPK, creatorId);
			}
		}
		catch (Exception e)
		{
			throw new ForumsRuntimeException("ForumsBmEJB.getSilverObjectId()",
				SilverpeasRuntimeException.ERROR, "forums.EX_IMPOSSIBLE_DOBTENIR_LE_SILVEROBJECTID",
				e);
		}
	...
	public void deleteCategory(String categoryId, String instanceId)
	{
		try
		{
			...
			ArrayList forums = getForumsByCategory(new ForumPK(instanceId, null), categoryId);
			Forum forum;
			int forumId;
			for (int i = 0, n = forums.size(); i < n; i++) 
			{
				...
				forumId = forum.getId();
				ForumPK forumPK = new ForumPK(instanceId, null, Integer.toString(forumId));
				updateForum(forumPK, forum.getName(), forum.getDescription(), forum.getParentId(),
					"0", null, false);
			}
	...
	public Collection getAllCategories(String instanceId)
	{
		...
		catch (Exception e)
		{
			throw new ForumsRuntimeException("ForumsBmEJB.getAllCategories()",
				SilverpeasRuntimeException.ERROR, "forums.MSG_CATEGORIES_NOT_EXIST", e);
		}
	}
	...
	private Collection getLastThreads(ForumPK forumPK, int count, boolean notAnswered)
    {
        Connection con = openConnection();
        try
        {
    ...
    private void deleteTagCloud(MessagePK messagePK)
		throws RemoteException
	{
		getTagCloudBm().deleteTagCloud(new TagCloudPK(messagePK.getId(),
			messagePK.getComponentName()), TagCloud.TYPE_MESSAGE);
	}
		...
		throws RemoteException
	{
		getNotationBm().deleteNotation(
			new NotationPK(forumPK.getId(), forumPK.getComponentName(), Notation.TYPE_FORUM));
	}
		...
		throws RemoteException
	{
		getNotationBm().deleteNotation(new NotationPK(
			messagePK.getId(), messagePK.getComponentName(), Notation.TYPE_MESSAGE));
	}
    ...
    private void createWysiwyg(MessagePK messagePK, String text)
    {
		...
		{
			SilverTrace.error("forums", "ForumsBMEJB.createWysiwyg()",
				"spaceId = " + messagePK.getSpaceId()
				+ " ; componentId = " + messagePK.getComponentName()
        ...
        try
        {
			if (WysiwygController.haveGotWysiwyg(spaceId, componentId, messageId))
			{
				WysiwygController.updateFileAndAttachment(
					text, spaceId, componentId, messageId, userId);
			}
			else
			{
				WysiwygController.createFileAndAttachment(
					text, spaceId, componentId, messageId);
			}
		}
		catch (WysiwygException e)
		{
			SilverTrace.error("forums", "ForumsBMEJB.updateWysiwyg()",
				"spaceId = " + messagePK.getSpaceId()
				+ " ; componentId = " + messagePK.getComponentName()
				+ " ; messageId = " + messagePK.getId());
		}
    ...
    private void deleteWysiwyg(MessagePK messagePK)
    {
    	try
        {
    		WysiwygController.deleteFileAndAttachment(
                messagePK.getComponentName(), messagePK.getId());
        }
		catch (WysiwygException e)
		{
