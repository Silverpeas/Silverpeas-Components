...
<%
  ResourceLocator uploadSettings =
      new ResourceLocator("com.stratelia.webactiv.util.uploads.uploadSettings", resources
          .getLanguage());
  ...
  UserCompletePublication userPubComplete =
      (UserCompletePublication) request.getAttribute("Publication");
  ...
  SilverTrace.info("kmelia", "JSPdesign", "root.MSG_GEN_PARAM_VALUE", "ACTION pubManager = " +
      action);
  ...
  if (action.equals("ValidationComplete") || action.equals("ValidationInProgress") ||
    ...
    if (action.equals("ValidationComplete")) {
      screenMessage =
          "<div class=\"inlineMessage-ok\">" + resources.getString("PubValidate") + "</div>";
    } else if (action.equals("ValidationInProgress")) {
      screenMessage =
          "<div class=\"inlineMessage\">" +
              resources.getString("kmelia.PublicationValidationInProgress") + "</div>";
    } else if (action.equals("Unvalidate")) {
      screenMessage =
          "<div class=\"inlineMessage-nok\">" + resources.getString("PublicationRefused") +
              "</div>";
    } else if (action.equals("Suspend")) {
      screenMessage =
          "<div class=\"inlineMessage-nok\">" +
              resources.getString("kmelia.PublicationSuspended") + "</div>";
    }
  ...
  if (action.equals("ValidateView")) {
    ...
    if (profile.equals("admin") ||
        profile.equals("publisher") ||
        profile.equals("supervisor") ||
        (ownerDetail != null && kmeliaScc.getUserDetail().getId().equals(ownerDetail.getId()) && profile
            .equals("writer"))) {
      isOwner = true;

      if (!kmeliaScc.isSuppressionOnlyForAdmin() ||
          (profile.equals("admin") && kmeliaScc.isSuppressionOnlyForAdmin())) {
...
<%
  ...
  operationPane.addOperation(favoriteAddSrc, resources.getString("FavoritesAddPublication") +
      " " + resources.
 ...
 if (isNewsManage && isOwner && delegatedNews == null) {
  ...
  if (!"user".equals(profile)) {
    if (pubDetail.isValidationRequired()) {
      out.println("<img src=\"" + outDraftSrc + "\" alt=\"" +
          resources.getString("PubStateToValidate") + "\" align=\"absmiddle\" id=\"status\"/>");
    } else if (pubDetail.isDraft()) {
      out.println("<img src=\"" + inDraftSrc + "\" alt=\"" +
          resources.getString("PubStateDraft") + "\" align=\"absmiddle\" id=\"status\"/>");
    } else if (pubDetail.isValid()) {
      out.println("<img src=\"" + validateSrc + "\" alt=\"" +
          resources.getString("PublicationValidated") +
          "\" align=\"absmiddle\" id=\"status\"/>");
    } else if (pubDetail.isRefused()) {
      out.println("<img src=\"" + refusedSrc + "\" alt=\"" +
          resources.getString("PublicationRefused") + "\" align=\"absmiddle\" id=\"status\"/>");
    }
  }
...
<%
  ...
  if (WysiwygController.haveGotWysiwyg(spaceId, componentId, id)) {
    ...
    getServletConfig().getServletContext().getRequestDispatcher(
        "/wysiwyg/jsp/htmlDisplayer.jsp?ObjectId=" + id + "&SpaceId=" + spaceId +
            "&ComponentId=" + componentId + "&Language=" + language + "&axisId=" +
            kmeliaScc.getAxisIdGlossary() + "&onlyFirst=" + highlightFirst).include(request,
        response);
    ...
    if (xmlForm != null) {
      PagesContext xmlContext =
          new PagesContext("myForm", "0", resources.getLanguage(), false, componentId,
              kmeliaScc.getUserId());
...
%> <view:highlight axisId="<%=kmeliaScc.getAxisIdGlossary()%>"
   ...
   if (attachmentsEnabled) {
     ...
     try {
       ...
       if (kmeliaScc.isVersionControlled(componentId)) {
         ...
         getServletConfig().getServletContext().getRequestDispatcher(
             "/versioningPeas/jsp/displayDocuments.jsp?Id=" + id + "&ComponentId=" +
                 componentId + "&Alias=" + alias + "&Context=Images&AttachmentPosition=" +
                 ...
                 "&UpdateOfficeMode=" + kmeliaScc.getUpdateOfficeMode() + "&Profile=" +
                 attProfile + "&NodeId=" + kmeliaScc.getSessionTopic().getNodePK().getId() +
                 "&TopicRightsEnabled=" + kmeliaScc.isRightsOnTopicsEnabled() +
                 "&VersionningFileRightsMode=" + kmeliaScc.getVersionningFileRightsMode() +
                 "&CallbackUrl=" + URLManager.getURL("useless", componentId) +
                 "ViewPublication&IndexIt=" + pIndexIt + "&ShowMenuNotif=" + true).include(
             request, response);
       } else {
         ...
         getServletConfig().getServletContext().getRequestDispatcher(
             "/attachment/jsp/displayAttachments.jsp?Id=" + id + "&ComponentId=" + componentId +
                 "&Alias=" + alias + "&Context=Images&AttachmentPosition=" +
                 resources.getSetting("attachmentPosition") + "&ShowIcon=" + showIcon +
                 "&ShowTitle=" + showTitle + "&ShowFileSize=" + showFileSize +
                 "&ShowDownloadEstimation=" + showDownloadEstimation + "&ShowInfo=" + showInfo +
                 "&UpdateOfficeMode=" + kmeliaScc.getUpdateOfficeMode() + "&Language=" +
                 language + "&Profile=" + attProfile + "&CallbackUrl=" +
                 URLManager.getURL("useless", componentId) + "ViewPublication&IndexIt=" +
                 pIndexIt + "&ShowMenuNotif=" + true).include(request, response);
       }
     } catch (Exception e) {
 ...
 %> <span id="authorInfo"><%=resources.getString("GML.author")%>
: <%=pubDetail.getAuthor()%></span><br />
<%
  }
%> <span id="creationInfo"><%=creatorName%> - <%=resources.getOutputDate(pubDetail.getCreationDate())%></span>
<%
  if (updaterId != null) {
%> <span id="lastModificationInfo"> | <%=resources.getString("kmelia.LastModification")%>
: <%=updaterName%> - <%=resources.getOutputDate(pubDetail.getUpdateDate())%></span>
<%
  }
  // Displaying all validator's name and final validation date 
  if (pubDetail.isValid() && StringUtil.isDefined(pubDetail.getValidatorId()) &&
      pubDetail.getValidateDate() != null) {
    String validators = "";
...
%> <span id="validationInfo"> | <%=resources.getString("kmelia.validation")%>
: <%=validators%> - <%=resources.getOutputDate(pubDetail.getValidateDate())%></span>
<%
  }
%> <span id="statInfo"> | <%=resources.getString("kmelia.consulted")%>
<%=pubDetail.getNbAccess()%> <%=resources.getString("kmelia.time")%></span> <%
   if (URLManager.displayUniversalLinks()) {
     String link = null;
     if (!pubDetail.getPK().getInstanceId().equals(contextComponentId)) {
       link =
           URLManager.getSimpleURL(URLManager.URL_PUBLI, pubDetail.getPK().getId(),
               contextComponentId);
     } else {
