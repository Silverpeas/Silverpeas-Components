...
<%
  ...
  ModelDetail modelDetail = (ModelDetail) request.getAttribute("ModelDetail");
    CompletePublication completePublication = (CompletePublication) request.getAttribute(
        "CompletePublication");
...
//Icons
  ...
  Button cancelButton = gef.getFormButton(resources.getString("GML.cancel"),
        "ViewPublication", false);
    Button sendUpdateButton = gef.getFormButton(resources.getString("GML.validate"),
        "javascript:onClick=sendModelData()", false);
%>
<HTML>
  <HEAD>
    <%
    ...
    <%
      ...
      if (notificationAllowed) {
          ...
          operationPane.addOperation(alertSrc, resources.getString("GML.notify"),
              "javaScript:onClick=goToOperationInAnotherWindow('ToAlertUser', '" + pubId
              + "', 'ViewAlert')");
        }

      // definition des boutons du wizard
        Button cancelWButton = gef.getFormButton(resources.getString("GML.cancel"),
            "ToPubliContent?WizardRow=" + wizardRow, false);
        Button nextButton;
        if (isEnd) {
          nextButton = gef.getFormButton(resources.getString("kmelia.End"),
              "javascript:onClick=sendModelData()", false);
        } else {
          nextButton = gef.getFormButton(resources.getString("GML.next"),
              "javascript:onClick=sendModelData()", false);
        }
      ...
      out.println(frame.printBefore());
        ...
        if (imageTrouble) {
          out.println("<Font color=\"red\" size=2>" + kmeliaScc.getString("ImageTrouble")
              + "</font><BR><BR>");
        }
              ...
              while (posit != -1) {
                ...
                if (toParse.startsWith("%WATXTDATA%")) {
                    ...
                    if (imageIterator.hasNext()) {
                      ...
                      String url = FileServerUtils.getUrl(spaceId, componentId, imageDetail
                          .getLogicalName(),
                          imageDetail.getPhysicalName(), imageDetail.getType(), publicationSettings
                          .getString(
                          "imagesSubDirectory"));
                    ...
                    out
                        .print("<INPUT TYPE=\"file\" name=\"" + "WAIMGVAR" + String.valueOf(
                        infoImageCount)
                        + "\">");
                    infoImageCount++;
                    toParse = toParse.substring(10);
