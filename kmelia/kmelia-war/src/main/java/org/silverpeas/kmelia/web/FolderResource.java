...
public class FolderResource extends RESTWebService {
  ...
  public NodeEntity getTreeview(@PathParam("path") String path, @QueryParam("lang") String language) {
    ...
    try {
      List<NodeDetail> nodes = new ArrayList<NodeDetail>(getNodeBm().getPath(new NodePK(nodeId, componentId)));
      Collections.reverse(nodes);
      NodeDetail root = getKmeliaBm().getExpandedPathToNode(new NodePK(nodeId, componentId), getUserDetail().getId());
