...
public class KmeliaRequestRouter extends ComponentRequestRouter<KmeliaSessionController> {
    ...
    try {
      ...
      if (function.startsWith("Main")) {
        ...
        if ("UpdateView".equals(action)) {
          ...
          boolean isPublicationClassifiedOnPDC = !positions.isEmpty();
        ...
        if (!SilverpeasRole.ADMIN.isInRole(kmelia.getUserTopicProfile(topicId))) {
          ...
          setNodeDataIntoRequest(kmelia, topicId, request);
        ...
        if (!SilverpeasRole.ADMIN.isInRole(kmelia.getUserTopicProfile(id)) &&
          ...
          setNodeDataIntoRequest(kmelia, id, request);
        ...
        putXMLDisplayerIntoRequest(request, kmelia, kmeliaPublication.getDetail());
        ...
        if (toolboxMode) {
          ...
          if (!alreadyOpened) {
            if (kmelia.openSingleAttachmentAutomatically() &&
                !kmelia.isCurrentPublicationHaveContent()) {
              request.setAttribute(SINGLE_ATTACHMENT_URL_ATTR,
                  kmelia.getSingleAttachmentURLOfCurrentPublication(alias));
            } else if (attachmentId != null) {
              request.setAttribute(SINGLE_ATTACHMENT_URL_ATTR,
                  kmelia.getAttachmentURL(attachmentId, alias));
            } else if (documentId != null) {
              request.setAttribute(SINGLE_ATTACHMENT_URL_ATTR,
                  kmelia.getAttachmentURL(documentId, alias));
            }
          }
        ...
        String pubId = request.getParameter("PubId");
        kmelia.getPublicationDetail(pubId);
        ...
        kmelia.changeTopicStatus(newStatus, subTopicId, recursive != null && recursive.equals("1"));
        ...
        HashSet<String> list = (HashSet<String>)
            request.getSession().getAttribute(KmeliaConstants.PUB_TO_LINK_SESSION_KEY);
        ...
        if (WysiwygController.haveGotWysiwyg(kmelia.getComponentId(),
          ...
          if (infoId == null || "0".equals(infoId)) {
            List<String> usedModels = kmelia.getModelUsed(true);
        ...
        Collection<String> modelUsed = kmelia.getModelUsed(true);
            ...
            WysiwygRouting.WysiwygRoutingContext.fromComponentSessionController(kmelia)
                ...
                .withContributionId(
                    ContributionIdentifier.from(kmelia.getComponentId(), publication.getId(),
                        publication.getContributionType()))
                .withContentLanguage(kmelia.getContentLanguage())
                .withComeBackUrl(
       ...
       */
        ...
        if (kmelia.isTimeAxisUsed() && StringUtil.isDefined(kmelia.getSessionTimeCriteria())) {
          ...
          if (!kmelia.getSessionTimeCriteria().equals("X")) {
            ...
            if (localizedCriteria.trim().isEmpty()) {
              localizedCriteria = "";
            }
        ...
        final List<KmeliaPublication> authorizedPublications =
            kmelia.getPublicationsForModification(
  ...
  private void setNodeDataIntoRequest(KmeliaSessionController kmelia, String topicId,
      HttpRequest request) {
      ...
      if (fileItem != null) {
        ...
        if (logicalName != null) {
          ...
          logicalName = logicalName
              .substring(logicalName.lastIndexOf(File.separator) + 1);

          // Name of temp folder: timestamp and userId
          tempFolderName = System.currentTimeMillis() + "_" + kmeliaScc.getUserId();
  ...
  private void putXMLDisplayerIntoRequest(HttpServletRequest request,
      KmeliaSessionController kmekia, PublicationDetail pubDetail)
    ...
    if (!StringUtil.isInteger(infoId)) {
      ...
      String language = kmekia.getContentLanguage();
      RecordSet recordSet = pubTemplate.getRecordSet();
      DataRecord data = getDataRecord(recordSet, pubId, language);
  ...
  private void setXMLUpdateForm(HttpRequest request, KmeliaSessionController kmelia,
    ...
    // get displayed language
    String language = kmelia.getContentLanguage();
    DataRecord data = getDataRecord(recordSet, pubId, language);
  ...
  @NonNull
  private static DataRecord getDataRecord(RecordSet recordSet, String pubId, String language)
  ...
  private String checkLanguage(KmeliaSessionController kmelia) {
    return checkLanguage(kmelia.getSessionPublication().getDetail());
  }

  private String checkLanguage(PublicationDetail pubDetail) {
    return pubDetail.getLanguageToDisplay(pubDetail.getLanguage());
  }
  ...
  private void setTemplatesUsedIntoRequest(KmeliaSessionController kmelia,
    ...
    boolean wysiwygValid = modelUsed.contains("WYSIWYG");
