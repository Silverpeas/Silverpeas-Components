...
public class KmeliaTransversal implements PublicationHelper{
	...
	{
		SilverTrace.debug("kmelia", "KmeliaTransversal.getPublications()", "root.MSG_GEN_ENTER_METHOD", "spaceId = "+spaceId+", nbPublis = "+nbPublis);
		...
		else
		{
			...
			SilverTrace.debug("kmelia", "KmeliaTransversal.getPublications()", "root.MSG_GEN_PARAM_VALUE", "#cIds = "+cIds.length);
			for (int c=0; c<cIds.length; c++)
			{
				if (cIds[c].startsWith("kmelia") || cIds[c].startsWith("toolbox") || cIds[c].startsWith("kmax"))
					componentIds.add(cIds[c]);
			}
		}
		
		SilverTrace.debug("kmelia", "KmeliaTransversal.getPublications()", "root.MSG_GEN_PARAM_VALUE", "componentIds = "+componentIds.toString());
		
		List publicationPKs = null;
		try
		{
			publicationPKs = (List) getPublicationBm().getPublicationPKsByStatus("Valid", componentIds);
		}
		catch (Exception e)
		{
            SilverTrace.error("kmelia", "KmeliaTransversal.getPublications()", "kmelia.CANT_GET_PUBLICATIONS_PKS", "spaceId = "+spaceId , e);
		}
		Collection filteredPublicationPKs = filterPublicationPKs(publicationPKs, nbPublis);
		
		try {
			return (List) getPublicationBm().getPublications(filteredPublicationPKs);
		} catch (RemoteException e) {
			SilverTrace.error("kmelia", "KmeliaTransversal.getPublications()", "kmelia.CANT_GET_PUBLICATIONS", "spaceId = "+spaceId , e);
		}
	...
	public List getPublicationsByComponentId(String componentId)
	{
		...
		try
		{
			...
			publications = (List) getPublicationBm().getPublicationsByStatus("Valid", componentIds);
		}
		catch (Exception e)
		{
            SilverTrace.error("kmelia", "KmeliaTransversal.getPublicationsByComponentId()", "kmelia.CANT_GET_PUBLICATIONS", "componentId = "+componentId, e);
		}
	...
	private List filterPublications(List publications, int nbPublis)
	{
		...
		for (int p=0; publications != null && p<publications.size(); p++)
		{
			...
			if (security.isObjectAvailable(pub.getPK().getInstanceId(), userId, pub.getPK().getId(), "Publication"))
