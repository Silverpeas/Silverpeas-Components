...
public class AliasFileServer extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException
    {
        ...
        if (StringUtil.isDefined(documentId))
        {
        	...
        	DocumentVersionPK versionPK = new DocumentVersionPK(Integer.parseInt(versionId), "useless", "useless");
        ...
        if (foreignKey != null)
        {
			...
			for (int a=0; !rightsOK && a<aliases.size(); a++)
			{
				...
				if (!contextComponentId.equals(alias.getInstanceId()))
				{
					...
					rightsOK = security.isAccessAuthorized(alias.getInstanceId(), userId, alias.getId(), "Node");
				}
			}
			
			if (rightsOK)
			{
		        String filePath = FileRepositoryManager.getAbsolutePath(fileComponentId) + directory + File.separator + sourceFile;
    ...
    private void display(HttpServletResponse res, String htmlFilePath) throws IOException
    {
        ...
        SilverTrace.info("kmelia", "AliasFileServer.display()", "root.MSG_GEN_ENTER_METHOD", " htmlFilePath "+htmlFilePath);
        try
        {
        ...
        {
        	SilverTrace.warn("kmelia", "AliasFileServer.doPost", "root.EX_CANT_READ_FILE", "file name=" + htmlFilePath);
        	displayWarningHtmlCode(res);
        }
        finally
        {
            ...
            {
                SilverTrace.warn("kmelia", "AliasFileServer.display", "root.EX_CANT_READ_FILE", "close failed");
            }
        }
    }
    ...
    private void displayWarningHtmlCode(HttpServletResponse res) throws IOException{
		...
		try{
            ...
            SilverTrace.warn("kmelia", "AliasFileServer.displayWarningHtmlCode", "root.EX_CANT_READ_FILE", "warning properties");
		} finally {
			try{
                if (sr != null)
                    sr.close();
                ...
                SilverTrace.warn("kmelia", "AliasFileServer.displayHtmlCode", "root.EX_CANT_READ_FILE", "close failed");
			}
		}
	}
    ...
    private VersioningBm getVersioningBm()
    {
    	try {
			...
			SilverTrace.warn("kmelia", "AliasFileServer.getVersioningBm", "root.EX_CANT_GET_EJB");
			return null;
		}
    }
    ...
    {
    	try {
			PublicationBmHome ejbHome = (PublicationBmHome) EJBUtilitaire.getEJBObjectRef(JNDINames.PUBLICATIONBM_EJBHOME, PublicationBmHome.class);
