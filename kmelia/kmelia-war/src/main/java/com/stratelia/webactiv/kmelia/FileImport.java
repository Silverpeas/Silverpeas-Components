...
public class FileImport {
	...
	public ArrayList importFile()
	{
		...
		//Create publication
		PublicationDetail publicationDetail = processImportFile(attachmentImportExport, versioningImportExport, filesToProcess, KmeliaSessionController.UNITARY_IMPORT_MODE);
	...
	public ArrayList importFiles()
	{
        ...
        String tempFolderName = new Long(new Date().getTime()).toString() + "_" + kmeliaScc.getUserId();

        //Directory Temp for the extracted files
		try {
			String tempFolderPath = FileRepositoryManager.getAbsolutePath(kmeliaScc.getComponentId()) + GeneralPropertiesManager.getGeneralResourceLocator().getString("RepositoryTypeTemp") + File.separator + tempFolderName;
			...
			SilverTrace.info("kmelia", "FileImport.importFiles()", "root.MSG_GEN_PARAM_VALUE", "nbFiles = "+nbFiles);
			ZipManager.extract(fileUploaded, new File(tempFolderPath));
			SilverTrace.info("kmelia", "FileImport.importFiles()", "root.MSG_GEN_PARAM_VALUE", "tempFolderPath.getPath() = "+new File(tempFolderPath).getPath());
			
			//Get files of the concerned upload directory
			File[] filesExtracted = new File(tempFolderPath+File.separator).listFiles();
			
			SilverTrace.info("kmelia", "FileImport.importFiles()", "root.MSG_GEN_PARAM_VALUE", "nb filesExtracted = "+filesExtracted.length);
			
			//Create publication
			PublicationDetail publicationDetail = processImportFile(attachmentImportExport, versioningImportExport, filesExtracted, KmeliaSessionController.MASSIVE_IMPORT_MODE_ONE_PUBLICATION);
	...
	public ArrayList importFilesMultiPubli()
	{
		...
		try {
	        ...
	        //Extraction of the files
			ZipManager.extract(fileUploaded, new File(tempFolderPath));
			SilverTrace.info("kmelia", "FileImport.importFilesMultiPubli()", "root.MSG_GEN_PARAM_VALUE", "tempFolderPath.getPath() = "+new File(tempFolderPath).getPath());
			
			//Get files of the concerned upload directory
			File[] filesExtracted = new File(tempFolderPath+File.separator).listFiles();
			
			SilverTrace.info("kmelia", "FileImport.importFilesMultiPubli()", "root.MSG_GEN_PARAM_VALUE", "nb filesExtracted = "+filesExtracted.length+" File="+fileUploaded.getAbsolutePath());
			...
			for (int i=0; i<filesExtracted.length; i++)
			{
				...
				//Create publications
				PublicationDetail publicationDetail = processImportFile(attachmentImportExport, versioningImportExport, fileToProcess, KmeliaSessionController.MASSIVE_IMPORT_MODE_MULTI_PUBLICATIONS);
				if (publicationDetail != null)
					publicationDetails.add(publicationDetail);
			}
	        FileFolderManager.deleteFolder(tempFolderPath);
		...
		{
      	  //Other exception
			SilverTrace.warn("kmelia","FileImport.importFilesMultiPubli()","root.EX_LOAD_ATTACHMENT_FAILED", e);
		}
	 ...
	 */
	private PublicationDetail processImportFile(AttachmentImportExport attachmentIE, VersioningImportExport versioningIE, File[] filesToProcess, String importMode)
	{
		...
		try {
			...
			String pubId = kmeliaScc.getKmeliaBm().createPublicationIntoTopic(pubDetailToCreate, new NodePK(topicId, componentId));
			pubDetailToCreate.getPK().setId(pubId);
			SilverTrace.info("kmelia", "FileImport.processImportFile()", "root.MSG_GEN_PARAM_VALUE", "componentId = "+componentId);
			...
			if (isVersioningUsed)
			{
				...
				for (int i=0; i<filesToProcess.length; i++)
				{
					...
					SilverTrace.info("kmelia", "FileImport.processImportFile()", "root.MSG_GEN_PARAM_VALUE", "filesExtracted[i].getPath() versioning = "+filesToProcess[i].getAbsolutePath());
					...
					SilverTrace.info("kmelia", "FileImport.processImportFile()", "root.MSG_GEN_PARAM_VALUE", "versioningIE.getVersioningPath(componentId) = "+versioningIE.getVersioningPath(componentId));
				...
				List copiedAttachments = attachmentIE.copyFiles(componentId, attachments, versioningIE.getVersioningPath(componentId));
				SilverTrace.info("kmelia", "FileImport.processImportFile()", "root.MSG_GEN_PARAM_VALUE", "copiedAttachments.size() = "+copiedAttachments);
				versioningIE.importDocuments(pubDetailToCreate.getId(), componentId, copiedAttachments, new Integer(userDetail.getId()).intValue(), versionType, KmeliaHelper.isIndexable(pubDetailToCreate));
			}
			else
			{
				//Add attachments
				...
				{
					AttachmentDetail attDetail = new AttachmentDetail();
					SilverTrace.info("kmelia", "FileImport.processImportFile()", "root.MSG_GEN_PARAM_VALUE", "filesExtracted[i].getPath() Non versionning = "+filesToProcess[i].getAbsolutePath());
					attDetail.setPhysicalName(filesToProcess[i].getAbsolutePath());
					attDetail.setAuthor(userDetail.getId());
				...
				attachmentIE.importAttachments(pubDetailToCreate.getId(),componentId, attachments, userDetail.getId(), KmeliaHelper.isIndexable(pubDetailToCreate));
			}
		} catch(Exception ex) {
	...
	{
		SilverTrace.info("kmelia", "FileImport.getPublicationDetail()", "root.MSG_GEN_PARAM_VALUE", "fileName = "+file.getName()+" filepath="+file.getAbsolutePath());
		...
		PublicationDetail publicationDetail = new PublicationDetail(null, pubName/*nom*/, description/*description*/, new Date()/*date de création*/, new Date()/*date de début de validité*/, null/*date de fin de validité*/, kmeliaScc.getUserDetail().getId()/*id user*/, "1"/*importance*/, null/*version de la publication*/, keywords/*keywords*/, content, null, author);
	...
	private String getOfficeTitle(String file, String value)
	{
		...
		{
			if (!MSdpManager.getTitle(file).equals("") && MSdpManager.getTitle(file).length()>1)
	...
	private String getOfficeSubject(String file, String value)
	{
		...
		{
			if (!MSdpManager.getSubject(file).equals("") && MSdpManager.getSubject(file).length()>1)
	...
	private String getOfficeAuthor(String file, String value)
	{
		...
		{
			if (!MSdpManager.getAuthor(file).equals("") && MSdpManager.getAuthor(file).length()>1)
	...
	private String getOfficeKeywords(String file, String value)
	{
		...
		{
			if (!MSdpManager.getKeywords(file).equals("") && MSdpManager.getKeywords(file).length()>1)
