...
public class DefaultKmeliaService implements KmeliaService {
  ...
  private List<Location> getPublicationLocations(PublicationPK pubPK,
    ...
    if (locations.isEmpty()) {
      ...
      final boolean alwaysVisibleModeActivated = getBooleanValue(getOrganisationController().
  ...
  public List<HistoryObjectDetail> getLastAccess(PublicationPK pk, NodePK nodePK,
    ...
    final Pagination<HistoryObjectDetail> pagination = new Pagination<HistoryObjectDetail>(new PaginationPage(1, maxResult))
        .limitDataSourceCallsTo(10)
        .withMinPerPage(200)
    ...
    try {
      ...
      if (pagination.isNbMaxDataSourceCallLimitReached()) {
        SilverLogger.getLogger(this).warn(
            "Performing too much paginated sql queries to retrieve last accesses from pub {0} on node {1}",
            pk, nodePK);
      }
    }
  }
  ...
  public Collection<Location> getLocations(PublicationPK pubPK) {
    try {
      ...
      if (locations.isEmpty()) {
        ...
        boolean alwaysVisibleModeActivated = getBooleanValue(getOrganisationController().
            getComponentParameterValue(pubPK.getInstanceId(), "publicationAlwaysVisible"));
        if (alwaysVisibleModeActivated) {
  ...
  private NodeDetail copyNode(KmeliaCopyDetail copyDetail, HashMap<Integer, Integer> oldAndNewIds) {
    ...
    if (rightsOnTopicsEnabled && copyDetail.isNodeRightsMustBeCopied()) {
      ...
      setNodeRightDependency(nodeToCopy, father, node);
      // Set topic rights if necessary
      if (nodeToCopy.haveLocalRights()) {
        node = nodeService.getDetail(nodePK);
  ...
  private void setNodeRightDependency(final NodeDetail nodeToCopy, final NodeDetail father,
  ...
  private PublicationDetail copyPublication(final PublicationDetail publiToCopy,
      final NodePK toNodePK, final String toComponentId, final KmeliaCopyDetail copyDetail,
      final String userId) throws PdcException, PublicationTemplateException, FormException {
    ResourceReference toResourceReference =
        new ResourceReference(ResourceReference.UNKNOWN_ID, toNodePK.getInstanceId());
