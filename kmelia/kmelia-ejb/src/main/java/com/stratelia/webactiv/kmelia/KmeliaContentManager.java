...
public class KmeliaContentManager implements ContentInterface, java.io.Serializable
{
   ...
   public int getSilverObjectId(String pubId, String peasId) {
		SilverTrace.info("kmelia","KmeliaContentManager.getSilverObjectId()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubId);
		try {
   ...
   {
	  SilverContentVisibility scv = new SilverContentVisibility(pubDetail.getBeginDate(), pubDetail.getEndDate(), isVisible(pubDetail));
	  SilverTrace.info("kmelia","KmeliaContentManager.createSilverContent()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
	...
	* update the visibility attributes of the content. Here, the type of content is a PublicationDetail
  	...
  	public void updateSilverContentVisibility(PublicationDetail pubDetail, boolean isVisible) throws ContentManagerException
  	{
		int						silverContentId = getContentManager().getSilverContentId(pubDetail.getPK().getId(), pubDetail.getPK().getComponentName());
	  	...
	  	SilverTrace.info("kmelia","KmeliaContentManager.updateSilverContentVisibility()", "root.MSG_GEN_ENTER_METHOD", "SilverContentVisibility = "+scv.toString());
   ...
   public void deleteSilverContent(Connection con, PublicationPK pubPK) throws ContentManagerException
   {
	  ...
	  if (contentId != -1)
	  {
		  SilverTrace.info("kmelia","KmeliaContentManager.deleteSilverContent()", "root.MSG_GEN_ENTER_METHOD", "pubId = "+pubPK.getId()+", contentId = "+contentId);
	...
	*/
	private List getHeaders(List ids, String componentId, String userId)
	{
		...
		try
		{
			...
			ArrayList publicationDetails = (ArrayList) getPublicationBm().getPublications((ArrayList) ids);
	   ...
	   if (contentManager == null)
		{
		   try
			...
			{
			    SilverTrace.fatal("kmelia", "KmeliaContentManager", "root.EX_UNKNOWN_CONTENT_MANAGER", e);
			}
		}
	...
	private PublicationBm getPublicationBm() {
		if (currentPublicationBm == null) {
			try {
				...
				throw new KmeliaRuntimeException("KmeliaContentManager.getPublicationBm()",SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DE_FABRIQUER_PUBLICATIONBM_HOME", e);
			}
		}
