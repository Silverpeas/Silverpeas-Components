...
public class KmeliaSecurity implements ComponentSecurity {
	...
	{
		if (cacheEnabled)
			getCache().put(objectId+objectType+componentId, Boolean.valueOf(available));
	}
	...
	public boolean isAccessAuthorized(String componentId, String userId, String objectId, String objectType) 
	{
		...
		if (objectType != null && ("Publication".equalsIgnoreCase(objectType) || objectType.startsWith("Attachment") || objectType.startsWith("Version")))
		{
			...
			{
				throw new KmeliaRuntimeException("KmeliaSecurity.isAccessAuthorized()", SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DOBTENIR_LA_PUBLICATION", e);
			}
			if (publication != null)
			{
				...
				else if("ToValidate".equalsIgnoreCase(status))
				{
					...
					if ("user".equalsIgnoreCase(profile))
						...
						return userId.equals(publication.getUpdaterId()) || userId.equals(publication.getCreatorId());
					else
						return true;
				}
				else if("UnValidate".equalsIgnoreCase(status) || "Draft".equalsIgnoreCase(status))
				{
					String profile = getProfile(userId, pk);
					if (!"user".equalsIgnoreCase(profile))
						return userId.equals(publication.getUpdaterId()) || userId.equals(publication.getCreatorId());
	...
	public boolean isObjectAvailable(String componentId, String userId, String objectId, String objectType)
	{
		...
		{
			objectAvailable = isNodeAvailable(new NodePK(objectId, componentId), userId);
		}
		else
		{
	...
	{
		String param = controller.getComponentParameterValue(componentId, "rightsOnTopics");
	...
	protected boolean isPublicationAvailable(PublicationPK pk, String userId)
	{
		...
		if (isRightsOnTopicsEnable(pk.getInstanceId()))
		{
			...
			{
				SilverTrace.info("kmelia", "KmeliaSecurity.isPublicationAvailable", "kmelia.EX_IMPOSSIBLE_DOBTENIR_LA_PUBLICATION", "PubId = "+pk.toString());
				objectAvailable = false;
			}
			
			NodePK fatherPK = null;
			for (int f=0; !objectAvailable && fatherPKs != null && f<fatherPKs.size(); f++)
			{
	...
	private boolean isNodeAvailable(NodePK nodePK, String userId)
	{
		...
		if (isRightsOnTopicsEnable(nodePK.getInstanceId()))
		{
			...
			try {
				...
				throw new KmeliaRuntimeException("KmeliaSecurity.isNodeAvailable()", SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DOBTENIR_LA_PUBLICATION", e);
			}
			...
			if (node != null)
			{
				if (!node.haveRights())
					...
					objectAvailable = controller.isObjectAvailable(node.getRightsDependsOn(), ObjectType.NODE, nodePK.getInstanceId(), userId);
			}
			else
			{
				objectAvailable = false;
		...
		writeInCache(nodePK.getId(), "Node", nodePK.getInstanceId(), objectAvailable);
		return objectAvailable;
	}
	...
	private String getProfile(String userId, PublicationPK pubPK)
	{
		...
		else
		{
			...
			try {
				...
				throw new KmeliaRuntimeException("KmeliaSecurity.getProfile()", SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DOBTENIR_LA_PUBLICATION", e);
			}
			...
			for (int n=0; nodePKs != null && n<nodePKs.size(); n++)
			{
				...
				if (node != null)
				{
					SilverTrace.debug("kmelia", "KmeliaSecurity.getProfile", "root.MSG_GEN_PARAM_VALUE", "nodePK = "+nodePK.toString());
					if (!node.haveRights())
					{
					...
					{
						lProfiles.addAll(Arrays.asList(controller.getUserProfiles(userId, pubPK.getInstanceId(), node.getRightsDependsOn(), ObjectType.NODE)));
					}
				}
			}
	...
	private PublicationBm getPublicationBm() 
	{
		if (publicationBm == null) {
			try {
				...
				throw new KmeliaRuntimeException("KmeliaSecurity.getPublicationBm()",SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DE_FABRIQUER_PUBLICATIONBM_HOME", e);
			}
		}
	...
	public NodeBm getNodeBm() {
		if (nodeBm == null)
		{
			try {
				...
				throw new KmeliaRuntimeException("KmeliaSecurity.getNodeBm()",SilverpeasRuntimeException.ERROR, "kmelia.EX_IMPOSSIBLE_DE_FABRIQUER_NODEBM_HOME", e);
			}
		}
		return nodeBm;
	}
