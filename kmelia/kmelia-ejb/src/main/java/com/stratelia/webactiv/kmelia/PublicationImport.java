...
public class PublicationImport {
	 ...
	 * 
	 * @param publiParams
	 *            The parameters of the publication.
	 * @param formParams
	...
	public boolean importPublication(Map publiParams, Map formParams,
		...
		String publicationToUpdateId = getPublicationId(xmlFormName,
		...
		if (publicationToUpdateId != null) {
			// Update
			resultStatus = false;
			pubPK = new PublicationPK(publicationToUpdateId, spaceId,
					componentId);
		...
		try {
			...
			PagesContext context = new PagesContext("myForm", "3", language,
					false, componentId, userId);
	...
	/**
	 * @param parameters
	 *            The parameters defining the publication.
	 * @param language
	 *            The language used to create the publication.
	 * @return A publication detail containing the parameters given as
	 *         parameters.
	 * @throws Exception
	 */
	private PublicationDetail getPublicationDetail(Map parameters,
		...
		PublicationDetail pubDetail = new PublicationDetail(pubId, name,
				description, null, jBeginDate, jEndDate, null, importance,
				version, keywords, "", null, author);
	...
	/**
	 * @param xmlFormName
	 *            The name of the XML form describing the publication.
	 * @param fieldName
	 *            The name of the field searched into the form.
	 * @param fieldValue
	 *            The value of the field searched into the form.
	 * @return The id of the publication corresponding to the XML form name and
	 *         containing a field named fieldName and valued to fieldValue.
	 *         Returns null if no publication is found.
	 */
	private String getPublicationId(String xmlFormName, String fieldName,
		...
		try {
			...
			MatchingIndexEntry[] result = searchEngineBm.getRange(0,
					searchEngineBm.getResultLength());
	...
	public Collection getPublicationsSpecificValues(String componentId, String xmlFormName,
			String fieldName)
		throws RemoteException
	{
		PublicationBm publicationBm = kmeliaBm.getPublicationBm();
		Collection publications = publicationBm.getAllPublications(
    	...
    	while (iter.hasNext())
    	{
    		...
    		if (publication.getInfoId().equals(xmlFormName)
    			/*&& publication.getStatus().equals(PublicationDetail.VALID)*/)
    		{
		...
		throws RemoteException
	{
		...
		if (publicationEndDate == null || publicationEndDate.after(endDate))
		{
			...
			updatePublication(pubDetail);
			//NodePK nodePK = new NodePK(topicId, spaceId, componentId);
			//kmeliaBm.draftOutPublication(publicationPK, nodePK, userId);
		}
	}
