...
public class KmeliaHelper implements Serializable {
	...
	{
		return "/Rkmelia/" + pubDetail.getPK().getInstanceId() + "/searchResult?Type=Publication&Id=" + pubDetail.getPK().getId();
	}
	
	public static String getNodeUrl(NodeDetail nodeDetail)
	{
		return "/Rkmelia/" + nodeDetail.getNodePK().getInstanceId() + "/searchResult?Type=Node&Id=" + nodeDetail.getNodePK().getId();
	}
