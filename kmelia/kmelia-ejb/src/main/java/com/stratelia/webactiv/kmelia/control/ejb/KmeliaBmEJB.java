...
public class KmeliaBmEJB implements KmeliaBmBusinessSkeleton, SessionBean {
  ...
  public TopicDetail goTo(NodePK pk, String userId, boolean isTreeStructureUsed, String userProfile,
    ...
    if (pk.isTrash()) {
      ...
      try {
        ...
        if (nbPublisOnRoot == 0 || !isTreeStructureUsed || KmeliaHelper
            .isToolbox(pk.getInstanceId())) {
  ...
  public void updatePublication(PublicationDetail pubDetail) {
    updatePublication(pubDetail, PUBLICATION_HEADER, false);
  }

  @Override
  public void updatePublication(PublicationDetail pubDetail, boolean forceUpdateDate) {
    updatePublication(pubDetail, PUBLICATION_HEADER, forceUpdateDate);
  }
  ...
  public void externalElementsOfPublicationHaveChanged(PublicationPK pubPK, String userId,
    ...
    if (pubDetail.getPK().getInstanceId().startsWith("kmelia")
      ...
      if (!isDefined(userId)) {
        updatePublication(pubDetail, PUBLICATION_CONTENT, false);
      } else {
  ...
  public void createInfoDetail(PublicationPK pubPK, String modelId, InfoDetail infos) {
    ...
    updatePublication(pubPK, PUBLICATION_CONTENT);
    SilverTrace.info("kmelia", "KmeliaBmEJB.createInfoDetail()",
        "root.MSG_GEN_EXIT_METHOD");
  }
  ...
  public List<KmeliaPublication> getPublicationsToValidate(String componentId) {
    ...
    try {
      ...
      for (PublicationDetail publi : temp) {
        boolean isClone = PublicationDetail.TO_VALIDATE.equals(publi.getStatus()) && !"-1".
            equals(publi.getCloneId());
        if (isClone) {
  ...
  public List<String> getAllValidators(PublicationPK pubPK, int validationType) throws
    ...
    if (validationType == VALIDATION_TARGET_N || validationType == VALIDATION_TARGET_1) {
      PublicationDetail publi = getPublicationBm().getDetail(pubPK);
   ...
   * @throws RemoteException
   */
  private boolean isValidationComplete(PublicationPK pubPK, List<String> allValidators) throws
      RemoteException {
    ...
    try {
      if (force) {
        ...
        switch (validationType) {
          case VALIDATION_CLASSIC:
          case VALIDATION_TARGET_1:
            validationComplete = true;
            break;
          case VALIDATION_COLLEGIATE:
          case VALIDATION_TARGET_N:
            // get all users who have to validate
  ...
  public NotificationMetaData getAlertNotificationMetaData(PublicationPK pubPK,
    ...
    final NotificationMetaData notifMetaData =
        UserNotificationHelper.build(new KmeliaAttachmentSubscriptionPublicationUserNotification(
        topicPK, pubDetail,
  ...
  public NotificationMetaData getAlertNotificationMetaData(PublicationPK pubPK,
    ...
    final NotificationMetaData notifMetaData =
        UserNotificationHelper.build(new KmeliaDocumentSubscriptionPublicationUserNotification(
        topicPK, pubDetail,
  ...
  private void createTodosForPublication(PublicationDetail pubDetail,
    ...
    if (pubDetail.isValidationRequired()
      ...
      if (validationType == VALIDATION_TARGET_N || validationType == VALIDATION_COLLEGIATE) {
