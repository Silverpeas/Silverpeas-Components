...
public class KmeliaPublicationHelper {
  ...
  public static boolean isUserConsideredAsOwner(String instanceId, String currentUserId, String profile, UserDetail ownerDetail) {
    if ("admin".equals(profile) || "publisher".equals(profile) || "supervisor".equals(profile) ||
        (ownerDetail != null && currentUserId.equals(ownerDetail.getId()) && "writer".equals(profile))) {
      return true;
  ...
  public static boolean isCanBeCut(String instanceId, String currentUserId, String profile, UserDetail ownerDetail) {
    return !KmeliaHelper.isKmax(instanceId) && isUserConsideredAsOwner(instanceId, currentUserId, profile, ownerDetail);
  }
  
  public static boolean isCreationAllowed(NodePK pk, String profile) {
    boolean publicationsInTopic =
        !pk.isRoot() ||
            (pk.isRoot() && (isPublicationsOnRootAllowed(pk.getInstanceId()) || !isTreeEnabled(pk
                .getInstanceId())));
  ...
  private static String getParameterValue(String instanceId, String name) {
    return OrganizationControllerFactory.getFactory().getOrganizationController().getComponentParameterValue(instanceId, name);
  }
}
